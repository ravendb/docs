...
description: "Configure ongoing tasks like ETL and backups for high availability with automatic failover across RavenDB cluster nodes."
sidebar_position: 1
---
...
* A **RavenDB Task** can be one of the following:  
  ...
  * Updating a `Rehab` or a `Promotable` [Database Node](./distributed-database.mdx#database-topology)  

* There is no single coordinator handing out tasks to a specific node.  
  Instead, each node decides on its own if it is the [Responsible Node](#responsible-node) of the task.  
...
* In this page:  
  ...
  * [Tasks Relocation](#tasks-relocation)  
  * [Pinning a Task](#pinning-a-task)  
</Admonition>
## License
...
* Scenarios [below](#tasks-relocation) 
...
1. Task is defined per [Database Group](./distributed-database.mdx).  

2. Task is executed by a single `Database Node` only.  
   With Backup Task being an exception in case of a cluster partition, see [Backup task - when the cluster is down](../../../backup/faq.mdx#what-happens-when-the-cluster-is-down) and [Backup task - when the responsible node is down](../../../backup/faq.mdx#what-happens-if-the-node-responsible-for-a-backup-task-is-down).  

3. A `Database Node` can be assigned with many tasks.  

4. The node must be in a [Member](./distributed-database.mdx#database-topology) state in the `Database Group` in order to perform a task.  
...
* Each node checks whether it is the `Responsible Node` for the task by executing a local function that is based on the  
  _unique hash value_ of the task and the current [Database Topology](./distributed-database.mdx#database-topology).  
...
Learn more [here](./distributed-database.mdx#database-topology) 
...
or chosen by the [cluster observer](./cluster-observer.mdx)
...
The [Cluster Observer](./cluster-observer.mdx) will note that it can't reach node B 
and issue a [Raft Command](../rachis/consensus-operations.mdx) in order to move node B to a `Rehab` state.  
...
In the meanwhile, node B which has no communication with the [Cluster Leader](../rachis/cluster-topology.mdx),  
...
* If a node remains offline for the period set by 
  [cluster.timebeforeaddingreplicainsec](../../configuration/cluster-configuration.mdx#clustertimebeforeaddingreplicainsec), 
