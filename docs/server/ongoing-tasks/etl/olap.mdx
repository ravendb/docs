...
* In this article:  
  * [Client API](../../../server/ongoing-tasks/etl/olap.mdx#client-api)  
     * [Add an OLAP connection string](../../../server/ongoing-tasks/etl/olap.mdx#add-an-olap-connection-string)  
...
* An OLAP ETL task writes Parquet files to a destination defined by an **OLAP connection string**.  
  The connection string stores the destination settings, such as a local folder, Amazon S3 bucket, Azure Storage container, Google Cloud Storage bucket, Amazon Glacier vault, or FTP endpoint.

* The ETL task does not store these destination details directly.  
  Instead, its configuration references the connection string by name through the `ConnectionStringName` property.

* An OLAP connection string can be created in any of these ways:
  * **From the Client API**:
  ...
  * **From Studio**:
    ...
    or while creating an [OLAP ETL task](../../../studio/database/tasks/ongoing-tasks/olap-etl-task.mdx#define-an-olap-etl-task).

* The examples below add **per-database** OLAP connection strings from the **Client API**.  
  The connection strings are stored in the current database record and can be used by tasks in that database.  
  The first example uses a local destination, and the second uses Amazon S3.

<Tabs>
<TabItem value="sync" label="Sync" default>
```csharp
// Define an OLAP connection string to a local destination
// =======================================================
var localOlapConStr = new OlapConnectionString
{
    // The name used by the OLAP ETL task configuration
    Name = "olap-local-connection-string-name",

    // The local folder where RavenDB will write the Parquet files
...
// Deploy (send) the local destination connection string to the server
// ===================================================================
var putLocalConnectionStringOp =
    new PutConnectionStringOperation<OlapConnectionString>(localOlapConStr);

PutConnectionStringResult localConnectionStringResult =
    store.Maintenance.Send(putLocalConnectionStringOp);

// Define an OLAP connection string to an Amazon S3 destination
// ============================================================
var s3OlapConStr = new OlapConnectionString
{
    // The name used by the OLAP ETL task configuration
    Name = "olap-s3-connection-string-name",

    // The S3 bucket where RavenDB will write the Parquet files
...
// Deploy (send) the S3 destination connection string to the server
// ================================================================
var putS3ConnectionStringOp =
    new PutConnectionStringOperation<OlapConnectionString>(s3OlapConStr);

PutConnectionStringResult s3ConnectionStringResult =
    store.Maintenance.Send(putS3ConnectionStringOp);
```
</TabItem>
...
PutConnectionStringResult localConnectionStringResult =
    await store.Maintenance.SendAsync(putLocalConnectionStringOp);
...
PutConnectionStringResult s3ConnectionStringResult =
    await store.Maintenance.SendAsync(putS3ConnectionStringOp);
```
</TabItem>
</Tabs>
...
| **LocalSettings** | `LocalSettings` | Settings for a local folder destination. |
| **S3Settings** | `S3Settings` | Settings for an Amazon S3 destination. |
| **AzureSettings** | `AzureSettings` | Settings for an Azure storage destination. |
| **GlacierSettings** | `GlacierSettings` | Settings for an Amazon Glacier destination. |
| **GoogleCloudSettings** | `GoogleCloudSettings` | Settings for a Google Cloud Storage destination. |
| **FtpSettings** | `FtpSettings` | Settings for an FTP destination. |
