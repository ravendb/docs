...
try \{
    ...
    $session->advanced()->getSessionInfo()->setContext("team/9-A");
    ...
    $employee2 = $session->load(Employee::class, "employees/3-A");
\} finally \{
    $session->close();
\}
`}
