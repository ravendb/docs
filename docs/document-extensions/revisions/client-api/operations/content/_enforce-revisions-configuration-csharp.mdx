...
`ContinuationParameters` is only needed when resuming a previous revisions operation.  
Because etags are node-local, a resumed operation must run on the same database node that produced the continuation values.
Use the `NodeTags` returned in the previous result to route the resumed operation to the correct node.    
