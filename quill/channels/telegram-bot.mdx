...
* A Quill **Telegram bot** channel lets your users converse with an [agent](../overview.mdx#ai-agent)
  of your app in a private chat with a bot on Telegram.  
  Users can chat with the bot using any Telegram client, and the agent will answer from the app's internal database.
...
search, can open a private chat with it and ask questions, and the agent will answer from the app's internal database.  
...
queue Quill keeps for this chat has room.  
...
The agent remembers a conversation for one day at most: at midnight UTC the agent forgets the earlier exchange, and the next
message is answered without the earlier context.  
...
The bot answers text-only messages. A photo, a document, a sticker, or a voice message gets no reply.  
...
3. **customerPhone**  
   ...
   Each row shows a parameter's name and the source the parameter is bound to; here, the Telegram phone number.
...
* **One bot per channel, one agent per channel.**  
  See [Channels and agents](../channels/overview.mdx#channels-and-agents) in the channels overview.
...
* **The agent remembers a conversation for one day at most.**  
  A user can make the agent forget sooner by sending `/clear`, as described in [Clearing the conversation](#clearing-the-conversation).  
  See also [Behavior and limitations common to all channels](../channels/overview.mdx#behavior-and-limitations-common-to-all-channels)
...
Symptoms met by the users of every bot type, not just of a Telegram bot, are listed in the channels overview, in
