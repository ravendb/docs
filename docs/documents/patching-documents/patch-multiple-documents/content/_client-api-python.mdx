...
`values` can be a single number or an array of numbers.  
    ...
    query="""from Products as p
             update
             {
                 timeseries(p, $timeSeriesName)
                     .append(new Date($timestamp), [p.PricePerUnit]);
             }""")
...
as shown in [Update by static index query](../../../../documents/patching-documents/patch-multiple-documents/client-api.mdx#update-by-static-index-query).
