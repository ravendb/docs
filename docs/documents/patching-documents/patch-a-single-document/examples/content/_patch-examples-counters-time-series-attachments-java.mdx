...
Map<String, Object> values = new HashMap<>();
values.put("name", "HeartRates");
values.put("from", NetISO8601Utils.format(from, true));
values.put("to", NetISO8601Utils.format(to, true));
patchRequest.setValues(values);
...
values.put("at", NetISO8601Utils.format(DateUtils.addDays(new Date(), 1), true));
patchRequest.setValues(values);
