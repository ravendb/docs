...
* This article explains **optimistic concurrency** in RavenDB,
  focusing on how optimistic concurrency behaves in sessions operating with [single-node transactions](../../cluster-transaction/overview.mdx#cluster-wide-transaction-vs-single-node-transaction) and how to configure it.    
...
* RavenDB supports three [modes of optimistic concurrency](#optimistic-concurrency-modes),  
...
* In this article:
  * [Optimistic concurrency - Overview](#optimistic-concurrency-overview)
    * [Default concurrency behavior with single-node transactions](#default-concurrency-behavior-with-single-node-transactions)
    * [Optimistic concurrency with single-node transactions](#optimistic-concurrency-with-single-node-transactions)
    * [Concurrency control in cluster-wide transactions](#concurrency-control-in-cluster-wide-transactions)
  * [Optimistic concurrency - Modes](#optimistic-concurrency-modes)  
  * [Configure globally](#configure-globally)
  * [Configure per session](#configure-per-session)
  * [Disable for specific document (when enabled on the session)](#disable-for-specific-document-when-enabled-on-session)  
  * [Enable for specific document (when disabled on the session)](#enable-for-specific-document-when-disabled-on-session)
...
* A session in RavenDB typically follows these steps:
  ...
  1. **Load or query** for documents from the database.  
     Unless [Tracking is explicitly disabled](../how-to-disable-tracking.mdx), these documents are automatically [Tracked by the session](../../what-is-a-session-and-how-does-it-work.mdx#unit-of-work-pattern).   
...
By default, sessions using [single-node transactions](../../cluster-transaction/overview.mdx#cluster-wide-transaction-vs-single-node-transaction) do not perform optimistic concurrency checks:
...
* **Optimistic concurrency modes**:   
  ...
  Learn more in the [Optimistic concurrency modes](#optimistic-concurrency-modes) section below.   
...
When the session operates in [Cluster-wide mode](../../cluster-transaction/overview.mdx),  
...
* For more information, see [Cluster Transaction - Overview](../../cluster-transaction/overview.mdx).
...
* When working with a session that uses [Single-node transactions](../../cluster-transaction/overview.mdx#cluster-wide-transaction-vs-single-node-transaction), 
...
* Use the [store.Conventions.OptimisticConcurrencyMode](../../../configuration/conventions.mdx#optimisticconcurrencymode) convention 
...
* This is done by passing `null` as a change vector value to the [Store](../../storing-entities.mdx) method.
...
* This is done by passing `string.Empty` as the change vector value to the [Store](../../storing-entities.mdx) method.  
...
* [Cannot combine _Writes_ and _WritesAndReads_ with _NoTracking_](#cannot-combinewritesandwritesandreadswithnotracking).
* [Cannot combine _Writes_ and _WritesAndReads_ with _TransactionMode.ClusterWide_](#cannot-combineewritesandwritesandreadswithtransactionmodeclusterwide).
* [_UseOptimisticConcurrency_ is deprecated](#useoptimisticconcurrencyis-deprecated).
...
* If you [Disable tracking](../how-to-disable-tracking.mdx),
...
* [Cluster-wide sessions](../../cluster-transaction/overview.mdx) cannot apply _Writes_ or _WritesAndReads_ modes.    
...
**Note**:  
In all cases above, the exception is also thrown if the conflicting condition is inherited from the [store conventions](../../../configuration/conventions.mdx)  
