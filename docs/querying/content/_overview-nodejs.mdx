...
* Queries defined with the `query` method are **translated by the RavenDB client to [RQL](../rql/what-is-rql.mdx)** when sent to the server.
...
* Queries defined with [query](#session-query) are translated by the RavenDB client to [RQL](../rql/what-is-rql.mdx) when sent to the server.  
