...
1. If the time that is defined in [TimeBeforeMovingToRehabInSec](../../../server/configuration/cluster-configuration.mdx#clustertimebeforemovingtorehabinsec) (default: 60 seconds) has passed  
   and the node is still unreachable, the node will be moved to a `Rehab` state.
   
2. If the node remains in `Rehab` for the time defined in [TimeBeforeAddingReplicaInSec](../../../server/configuration/cluster-configuration.mdx#clustertimebeforeaddingreplicainsec) (default: 900 seconds),  
