...
* The audit index can then be queried to find documents by compliance status and specific validation errors.  
...
Define a schema for the audit index. This can be a collection schema or an index schema.  
...
An **index schema** is defined as part of the audit index itself. [See below](../../../documents/schema-validation/auditing-document-compliance/auditing-document-compliance_studio#create-the-audit-index) how to do this.  

* If you define only one of the schemas (collection or index), the audit index will use that schema.  
* If you define both schemas, the audit index will use the **index schema**.  
...
Query the index to find documents by compliance status and specific errors.
...
* Run queries that filter documents based on the `Errors` field.  
  ...
  Click an item's preview (eye) icon to view the document details, including the validation errors embedded in the index.  
