...
description: "AI agents act as customizable server-side mediators between RavenDB clients and AI models, providing secure conversational access to your data."
sidebar_position: 1
---
...
* In this article:
   * [Defining and running AI agents](../../ai-integration/ai-agents/overview.mdx#defining-and-running-an-ai-agent)
      * [The main stages in defining an AI agent](../../ai-integration/ai-agents/overview.mdx#the-main-stages-in-defining-an-ai-agent)
      * [What is a conversation](../../ai-integration/ai-agents/overview.mdx#what-is-a-conversation)
      * [Initiating a conversation](../../ai-integration/ai-agents/overview.mdx#initiating-a-conversation)
   * [AI agent usage flowchart](../../ai-integration/ai-agents/overview.mdx#ai-agent-usage-flowchart)
   * [Streaming LLM responses](../../ai-integration/ai-agents/overview.mdx#streaming-llm-responses)
   * [Reducing throughput and expediting LLM response](../../ai-integration/ai-agents/overview.mdx#reducing-throughput-and-expediting-llm-response)
   * [Multi-agents](../../ai-integration/ai-agents/overview.mdx#multi-agents)
   * [Common use cases](../../ai-integration/ai-agents/overview.mdx#common-use-cases)  

</Admonition>

<Panel heading="Defining and running an AI agent">
...
<Admonition type="note" title="">
* [Learn to create an AI agent using the client API](../../ai-integration/ai-agents/creating-ai-agents/creating-ai-agents_api.mdx)  
* [Learn to create an AI agent using Studio](../../ai-integration/ai-agents/creating-ai-agents/creating-ai-agents_studio.mdx)  
</Admonition>
...
* A **connection string** to the AI model.  
  [Create a connection string using the API](../../ai-integration/ai-agents/creating-ai-agents/creating-ai-agents_api.mdx#creating-a-connection-string)  
  [Create a connection string using Studio](../../ai-integration/ai-agents/creating-ai-agents/creating-ai-agents_studio.mdx#configure-basic-settings)  

* An **agent configuration** that defines the agent.  
  [Define an agent configuration using the API](../../ai-integration/ai-agents/creating-ai-agents/creating-ai-agents_api.mdx#defining-an-agent-configuration)  
  [Define an agent configuration using Studio](../../ai-integration/ai-agents/creating-ai-agents/creating-ai-agents_studio.mdx#configure-basic-settings)  
   ...
   * <a id="query-tools"/> Optional **query tools**.  
      ...
      * **Read-only operations**  
        ...
        To make changes in the database, use [action tools](../../ai-integration/ai-agents/overview.mdx#action-tools).  
...
* The conversation starts when the client invokes the agent and provides it with an [initial context](../../ai-integration/ai-agents/overview.mdx#initiating-a-conversation).  
* The conversation may include multiple "turns" of message exchanges between the client and the LLM, mediated by the agent.  
   ...
   * During the turn, the LLM can trigger the agent to run queries or request the client to perform actions, using [defined query and action tools](../../ai-integration/ai-agents/creating-ai-agents/creating-ai-agents_api.mdx#adding-agent-tools).  
   * The turn ends with an [LLM response](../../ai-integration/ai-agents/creating-ai-agents/creating-ai-agents_api.mdx#conversation-response) to the user prompt.  
...
To start a conversation with the LLM, the agent will send to the LLM an **initial context** that includes -  

* The pre-defined [agent configuration](../../ai-integration/ai-agents/overview.mdx#the-main-stages-in-defining-an-ai-agent) (automatically sent by the agent) with:  
   * The system prompt  
   * A response object that defines the layout for the LLM response  
   * Optional agent parameters  
   * Optional Query tools  
     (and if any query tool is configured as an [initial-context query](../../ai-integration/ai-agents/overview.mdx#initial-context-queries) - results for this query)  
...
* **Values for agent parameters**  
  If agent parameters were defined in the agent configuration, the client is required to provide their values to the agent when starting a conversation.  
     ...
     [See an example that utilizes this agent parameter](../../ai-integration/ai-agents/creating-ai-agents/creating-ai-agents_api.mdx#query-tools).  
...
When the agent has [sub-agents](../../ai-integration/ai-agents/multi-agents.mdx),
parameter values passed at conversation start flow to any sub-agent that
declares a parameter with the same name. You can also pass a parameter that the
**root** agent does not declare - it will still be delivered to a sub-agent that
declares it. See
...
for the full rules, including LLM visibility controls.
...
* Optional **file attachments**, provided by the client alongside the user prompt.  
  The client can attach files (e.g., PDFs, images) to any conversation turn, allowing the LLM to analyze the attachment content as part of its response.  
  [Learn to add attachments using the API](../../ai-integration/ai-agents/creating-ai-agents/creating-ai-agents_api.mdx#adding-attachments)  

</ContentFrame>

</Panel>

<Panel heading="AI agent usage flowchart">
...
3. **Client`<->`Agent** flow  
    * To invoke an agent, the client needs to provide it with an [initial context](../../ai-integration/ai-agents/overview.mdx#initiating-a-conversation).  
...
5. **Agent`<->`Model** flow  
    * **When a conversation is started**, the agent needs to provide the AI model with an [initial context](../../ai-integration/ai-agents/overview.mdx#initiating-a-conversation), partly defined by the agent configuration and partly by the client.  
...
<Panel heading="Streaming LLM responses">
...
[Streaming LLM responses using the API](../../ai-integration/ai-agents/creating-ai-agents/creating-ai-agents_api.mdx#stream-llm-responses)  

</Panel>

<Panel heading="Reducing throughput and expediting LLM response">
...
[Configuring chat trimming using the API](../../ai-integration/ai-agents/creating-ai-agents/creating-ai-agents_api.mdx#set-chat-trimming-configuration)  
[Configuring chat trimming using Studio](../../ai-integration/ai-agents/creating-ai-agents/creating-ai-agents_studio.mdx#configure-chat-trimming)  

You can also distribute work across a
[multi-agents hierarchy](../../ai-integration/ai-agents/multi-agents.mdx)
so each agent's context window stays focused on its own specialty. The root
agent sees only a concise result from each sub-agent, not the sub-agent's full
exchange with the LLM, which keeps token usage on the root conversation low.  
...
[Creating query tools using the API](../../ai-integration/ai-agents/creating-ai-agents/creating-ai-agents_api.mdx#query-tools)  
[Creating query tools using Studio](../../ai-integration/ai-agents/creating-ai-agents/creating-ai-agents_studio.mdx#add-query-tools)  
...
[Setting iterations limit using the API](../../ai-integration/ai-agents/creating-ai-agents/creating-ai-agents_api.mdx#set-maximum-number-of-iterations)  

</ContentFrame>

</Panel>

<Panel heading="Multi-agents">

An AI agent can invoke other agents as tools, forming a **multi-agents hierarchy**.  
A root agent delegates focused tasks to specialist sub-agents; each sub-agent is itself a full agent that can run queries, request actions, and call its own sub-agents.  

Use a multi-agents hierarchy when:  

* **A single agent's system prompt is becoming crowded.**  
  Giving each responsibility its own focused prompt makes tool selection more reliable and easier to audit.  

* **Tool counts are growing.**  
  Grouping related tools under a sub-agent reduces the number of options the LLM has to choose among at any given level.  

* **The root conversation is under context-window pressure.**  
  Sub-calls return a concise result to the root. The detailed exchange a sub-agent had with the LLM stays in the sub-agent's own conversation.  

Each agent in the hierarchy has its own conversation document; parameters can be shared across the hierarchy by name; parameter values can be hidden from the LLM at either the configuration or conversation level; and parameters whose values must be provided by the caller can be marked so that the parent's LLM is not allowed to generate a value for them.  

[Learn more about multi-agents](../../ai-integration/ai-agents/multi-agents.mdx)  

</Panel>

<Panel heading="Common use cases">
