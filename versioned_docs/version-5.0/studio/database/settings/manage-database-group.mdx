...
* A **Database instance** can reside on a single node, on a number of nodes, or on all cluster nodes,  
  depending on the replication factor defined when [creating the database](../../../studio/server/databases/create-new-database/general-flow.mdx).  
...
* At the **Database Group Level**, all the nodes are working cooperatively together.  
  ...
  Instead of selecting a leader, as in the [Cluster Level](../../../studio/server/cluster/cluster-view.mdx), 
...
1. **Node:**  
   ...
   Note: Node should be already part of the Cluster Topology. See [Adding a node to the Cluster](../../../studio/server/cluster/add-node-to-cluster.mdx)  
