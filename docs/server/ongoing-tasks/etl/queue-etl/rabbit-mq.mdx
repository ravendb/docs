...
description: "Stream RavenDB document changes to RabbitMQ exchanges using queue ETL tasks with JavaScript transformations and routing key configuration."
sidebar_position: 2
---
...
* This article focuses on how to create a RabbitMQ ETL task using the Client API.  
  To define a RabbitMQ ETL task from Studio see [Studio: RabbitMQ ETL Task](../../../../studio/database/tasks/ongoing-tasks/rabbitmq-etl-task.mdx)  
...
* In this article:
    ...
    * [Add a RabbitMQ ETL task](../../../../server/ongoing-tasks/etl/queue-etl/rabbit-mq.mdx#add-a-rabbitmq-etl-task)
        ...
        * [Example - delete processed documents](../../../../server/ongoing-tasks/etl/queue-etl/rabbit-mq.mdx#delete-processed-documents)
        * [Syntax](../../../../server/ongoing-tasks/etl/queue-etl/rabbit-mq.mdx#syntax-1)
    * [The transformation script](../../../../server/ongoing-tasks/etl/queue-etl/rabbit-mq.mdx#the-transformation-script)
        * [The loadTo method](../../../../server/ongoing-tasks/etl/queue-etl/rabbit-mq.mdx#the-loadto-method)
        * [Available method overloads](../../../../server/ongoing-tasks/etl/queue-etl/rabbit-mq.mdx#available-method-overloads)

</Admonition>

## Add a RabbitMQ connection string

* A RabbitMQ ETL task uses a **RabbitMQ connection string** to connect to its destination RabbitMQ broker.  
  The connection string stores the RabbitMQ connection URI used to reach the broker.
...
* A RabbitMQ connection string can be created in any of these ways:
  * **From the Client API**:
  ...
  * **From Studio**:
    ...
    or while creating a [RabbitMQ ETL task](../../../../studio/database/tasks/ongoing-tasks/rabbitmq-etl-task.mdx#define-rabbitmq-etl-task).

* The example below adds a **per-database** RabbitMQ connection string from the **Client API**.  
  The connection string is stored in the current database record and can be used by tasks in that database.

<Tabs>
<TabItem value="sync" label="Sync" default>
```csharp
// Define a RabbitMQ connection string
// ===================================
var rabbitMqConnectionString = new QueueConnectionString
{
    // The name used by the RabbitMQ ETL task configuration
    Name = "rabbitmq-connection-string-name",

    // Set the queue broker type to RabbitMQ
...
var putConnectionStringOp =
    new PutConnectionStringOperation<QueueConnectionString>(rabbitMqConnectionString);
...
<Tabs groupId="rabbitmq-connection-string-syntax-gpt">
...
{
    // The RabbitMQ connection URI, for example: amqp://guest:guest@localhost:49154
...
<TabItem value="rabbitMq_delete_documents" label="rabbitMq_delete_documents">
<CodeBlock language="csharp">
{`var etlTask = new QueueEtlConfiguration()
\{
    BrokerType = QueueBrokerType.RabbitMq,
...
The [basic characteristics](../../../../server/ongoing-tasks/etl/basics.mdx) of a RabbitMQ ETL script are similar to those of other ETL types.  
...
The two methods are equivalent, offering alternative syntax:
  ...
  * **`loadTo('ExchangeName', obj, 'routingKey', {attributes})`**  
    ...
    * Separating the target name from the `loadTo` command makes it possible to include symbols like `'-'` and `'.'` in target names.
      ...
      | **attributes**   | object  | An object with [CloudEvents attributes](../../../../server/ongoing-tasks/etl/queue-etl/overview.mdx#cloudevents).                |
