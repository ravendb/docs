...
* In this article:
   ...
   * [Configuration](../../monitoring/alerts-and-notifications/notification-center.mdx#configuration)
      * [Notification type values](../../monitoring/alerts-and-notifications/notification-center.mdx#notification-type-values)
      * [Performance-hint kind values](../../monitoring/alerts-and-notifications/notification-center.mdx#performance-hint-kind-values)
      * [Alert kind values](../../monitoring/alerts-and-notifications/notification-center.mdx#alert-kind-values)
...
#### Example: Filter out a hint kind and an alert kind
...
List a notification type to filter out all the notifications of this type:
...
### Performance-hint kind values

List a performance-hint kind to filter out only the hints of this kind:

| Kind | Filters out hints about |
| ---- | ----------------------- |
| `Paging` | Requests that return an excessive number of results |
| `RequestLatency` | Queries whose duration exceeds the configured threshold |
| `UnusedCapacity` | A server that uses fewer cores than the machine provides |
| `SlowIO` | Extremely slow disk writes |
| `HugeDocuments` | Documents that exceed the configured size |
| `SqlEtl_SlowSql` | Slow SQL statements in a SQL ETL task |
| `Indexing` | Indexing performance issues, like a high fanout ratio |
| `Indexing_References` | A high rate of `LoadDocument()` calls per referenced item |
| `Replication` | Tombstone accumulation caused by a disabled replication destination |
...
List an alert kind to filter out only the alerts of this kind.  

An alert's kind is not shown on its card. To find the kind's name, fetch the server's stored
notifications from the `/admin/server/notifications` endpoint (requires `Operator` clearance),
