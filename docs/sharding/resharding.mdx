...
description: "Redistribute data across RavenDB shards by splitting buckets during resharding to rebalance storage, query load, and cluster resources."
...
* **Resharding** is the relocation of data stored on one [shard](./overview.mdx#shards) 
  to another shard, to maintain an overall balanced database in which all 
  shards handle about the same volume of data.  
* The resharding process moves all the data related to a certain [bucket](./overview.mdx#buckets) 
  (including documents, document extensions, [tombstones](../monitoring/tombstones/overview.mdx), etc.), to a different 
...
* In this page:  
  ...
  * [The Resharding Process](#the-resharding-process)  
      * [Following Resharding Progress](#following-resharding-progress)  
      * [Racing](#racing)  
  * [Change Vector on a Sharded Database](#change-vector-on-a-sharded-database)  
  * [Resharding and Other Features](#resharding-and-other-features)  
  * [Executing Resharding](#executing-resharding)  
...
3. Shard `#0` remains the owner of the bucket until all data has been propagated to 
   all shard `#2` [replicas](./overview.mdx#shard-replication).  
...
* **Studio Popup Messages**  
  When [Studio is used for resharding](#executing-resharding) 
...
During [external replication](./external-replication.mdx):  
...
[ETL tasks](./etl.mdx) cannot determine whether a document that 
...
Our promise to [data subscription](./subscriptions.mdx) workers is that we 
...
[may show](#bucket-ownership) 
both the bucket's source and destination shards.  
However, if documents stored in this bucket are [queried](./querying.mdx) 
