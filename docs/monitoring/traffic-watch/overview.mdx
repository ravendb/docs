...
* **CPU & memory overhead**: Negligible.  
  Traffic Watch logging is processed asynchronously by [NLog](../logging/overview.mdx)'s `AsyncTargetWrapper`.  
...
* **Disk storage**: Proportional to request volume.
  ...
  * The size and retention limits are set by the standard [logging configuration](../logging/configuration.mdx):  
    [`Logs.ArchiveAboveSizeInMb`](../logging/configuration.mdx#logsarchiveabovesizeinmb) (default: 128 MB),  
    [`Logs.MaxArchiveDays`](../logging/configuration.mdx#logsmaxarchivedays) (default: 3 days),  
    [`Logs.MaxArchiveFiles`](../logging/configuration.mdx#logsmaxarchivefiles).  
