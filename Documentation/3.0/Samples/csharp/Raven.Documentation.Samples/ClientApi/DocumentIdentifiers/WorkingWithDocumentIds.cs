namespace Raven.Documentation.Samples.ClientApi.DocumentIdentifiers
{
	...
	public class WorkingWithDocumentIds
	{
		public WorkingWithDocumentIds()
		{
			...
			var key = result.Key; // "9ce12df5-1027-4704-b2cf-d312e9ea0e59"
			#endregion


			#region commands_identity

			result = store.DatabaseCommands.Put("products/", null, new RavenJObject()
			{
				{
					"Name", "RavenDB"
				}
			...
			var identityKey = result.Key; // "products/1"
			#endregion

			#region commands_identity_generate
			var identity = store.DatabaseCommands.NextIdentityFor("products");

			result = store.DatabaseCommands.Put("products/" + identity, null, new RavenJObject()
			{
				{
					"Name", "RavenDB"
				}
