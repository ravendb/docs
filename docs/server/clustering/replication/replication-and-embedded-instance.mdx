...
## Overview
[Replication](./replication-overview.mdx) works by using long-living TCP connections between cluster nodes, or in case of [External Replication](../../ongoing-tasks/external-replication.mdx) instances external to the cluster. 
Essentially, Embedded RavenDB is the same as non-embedded, their only difference is what process is a host. [Here](../../embedded.mdx) you can read more in-depth about Embedded RavenDB functionality.
...
One possibility would be to configure it through [the studio](../../../studio/cluster/setting-a-cluster.mdx#add-another-node-to-the-cluster), by adding the Embedded instance to the cluster, then adding the database replicated to relevant [database group](../distribution/distributed-database.mdx).  
...
Another possibility to configure the embedded instance as an [external replication](../../ongoing-tasks/external-replication.mdx) source or a destination.  
...
It is also possible to configure [external replication](../../ongoing-tasks/external-replication.mdx) programmatically, so the embedded instance can serve as a source, destination or both.
