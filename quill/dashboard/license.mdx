---
title: "Dashboard: License View"
sidebar_label: License View
sidebar_position: 4
...
* The Quill license includes the RavenDB capabilities required by the appliance; it does not include every RavenDB Server feature.  
  For example, it does not include [read-only certificates](certificates.mdx#read-only-access-is-not-available).  
  You do not need to obtain or install a separate RavenDB Server license.

* You cannot install or change a license from this page.  
  To view write usage associated with the license, open the [Usage](../dashboard/usage.mdx) view under **License & usage**.
...
For current plan details and pricing, see [Quill pricing](https://ravendb.net/quill/pricing).
...
* **Report and retrieve write usage.**  
  ...
  The [**Usage**](../dashboard/usage.mdx) view retrieves the aggregated write usage from the same API.
