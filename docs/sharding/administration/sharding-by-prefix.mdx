...
* The default technique of distributing data across shards uses a hashing algorithm based on document IDs  
  to [populate buckets](../overview.mdx#buckets-population) that are assigned to shards. While this method is effective in distributing data evenly,  
...
* With prefixed sharding, you only control the shard where a document is stored.  
  ...
  This can be partially addressed by [anchoring documents](./anchoring-documents.mdx) to a bucket.
...
* Unlike when defining prefixes for the first time,  
  the following rules must be observed when updating an existing prefix configuration:
    ...
    * **When adding a shard to an existing prefix configuration**,  
      ...
      Therefore, after assigning a new shard to a prefix, manual bucket [re-sharding](../resharding.mdx) is required.  
...
* You can designate which shard/s to query by combining both selecting a shard by prefix  
  and [selecting a shard by document ID](../querying.mdx#querying-selected-shards).
...
**Anchoring documents**:  
With [anchoring documents](./anchoring-documents.mdx), you can ensure that designated documents will be stored in the same bucket  
