...
* The login endpoint accepts 10 attempts per minute and returns `429 Too Many Requests`  
  when that limit is exceeded.

* Quill's nginx front does not preserve the caller's IP address, as described in [Appliance Surfaces](appliance-surfaces.mdx#how-a-connection-is-routed).  
  All login attempts therefore share a single instance-wide limit.

* Attempts made by another client can temporarily prevent you from signing in.  
  If this happens, wait up to one minute and try again.
...
See [the Quill hostnames](appliance-surfaces.mdx#the-appliance-hostnames).
...
* **The public chat surface**  
  ...
  See [what is public](appliance-surfaces.mdx#what-is-public).
...
* **Direct database access**  
  ...
  See [direct database access](appliance-surfaces.mdx#direct-database-access).
...
* The login rate limit is a single instance-wide bucket because nginx does not preserve the original caller IP.
