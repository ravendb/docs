...
* Compare-exchange items are [not replicated externally](#why-compare-exchange-items-are-not-replicated-to-external-databases) to other databases.

* In this page:  
* [What Compare Exchange Items Are](#what-compare-exchange-items-are)  
* [Creating and Managing Compare-Exchange Items](#creating-and-managing-compare-exchange-items)  
* [Why Compare-Exchange Items are Not Replicated to External Databases](#why-compare-exchange-items-are-not-replicated-to-external-databases)  
* [Example I - Email Address Reservation](#example-i---email-address-reservation)  
* [Example II - Reserve a Shared Resource](#example-ii---reserve-a-shared-resource)  
* [Example III - Ensuring Unique Values without Using Compare Exchange](#example-iii---ensuring-unique-values-without-using-compare-exchange)  
...
* Each compare-exchange item contains: 
  ...
  * **Metadata** - Data that is associated with the compare-exchange item.
     ...
     * For example, the metadata can be used to set expiration time for the compare-exchange item.  
       Learn more in [compare-exchange expiration](../compare-exchange-expiration.mdx).  
...
* **Document Store Operations**  
  You can manage a compare-exchange item as an [Operation on the document store](../put-compare-exchange-value.mdx).  
...
* **Cluster-Wide Sessions**  
  You can manage a compare-exchange item from inside a [Cluster-Wide session](../../../session/cluster-transaction/compare-exchange.mdx).  
...
* **Atomic Guards**  
  When creating documents using a cluster-wide session RavenDB automatically creates [Atomic Guards](../../../session/cluster-transaction/atomic-guards.mdx),  
  which are compare-exchange items that guarantee ACID transactions.  
  See [Cluster-wide vs. Single-node](../../../session/cluster-transaction/overview.mdx#cluster-wide-transaction-vs-single-node-transaction) for a session comparision overview.
...
* To ensure unique values between two databases without using compare-exchange items see [Example III](#example-iii---ensuring-unique-values-without-using-compare-exchange).
...
To establish uniqueness without using compare-exchange see [Example III](#example-iii---ensuring-unique-values-without-using-compare-exchange).
...
* This compare-exchange item was [created as an operation](../put-compare-exchange-value.mdx)
  rather than with a [cluster-wide session](../../../session/cluster-transaction/overview.mdx).  
...
<Admonition type="note" title="">
Sessions which process fields that must be unique should be set to [TransactionMode.CLUSTER_WIDE](../../../session/cluster-transaction/overview.mdx).  
</Admonition>
