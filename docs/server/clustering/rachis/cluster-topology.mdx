...
* In this page:
    * [Fetching the topology](#fetching-the-topology)
    * [Modifying the topology](#modifying-the-topology)
    * [Nodes states and types](#nodes-states-and-types)
...
| watcher          | `bool`       | Add the node as a [Watcher](#watcher).<br/>Default: `false` |
...
In Rachis every cluster node has a [state](#state) and a [type](#type).  
...
| **Watcher**    | A non-voting node that is still fully managed by the cluster. <br/> Can be assigned with databases and tasks. See more [below](#watcher). |
...
| **Leader**       | Leader of the cluster. See more [below](#leader).                                                                                                                                           |
