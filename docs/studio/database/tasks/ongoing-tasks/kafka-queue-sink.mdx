...
* RavenDB can collaborate with message brokers like Kafka both as a **producer**, 
  by running [ETL tasks](./kafka-etl-task.mdx)), 
...
* In this page:  
  ...
  * [Define a Kafka Sink Task](#define-a-kafka-sink-task)  
      * [Define and Test Task Scripts](#define-and-test-task-scripts)  
...
4. **Responsible Node** (Optional)  
  * Select a node from the [Database Group](../../settings/manage-database-group.mdx) 
    to be responsible for this task.  
  * If no node is selected, the cluster will assign a responsible node 
    (see [Members Duties](../../settings/manage-database-group.mdx#database-group-topology---members-duties)).  
...
enter Studio's [ongoing tasks stats](../../stats/ongoing-tasks-stats/overview.mdx) 
view.  
