...
3. **Register the schema with the server**:  
   ...
   <Admonition type="note" title="">
   Note that once a schema is activated, associated documents are validated not only when saved directly, but also when added or modified by operations such as patching or ETL tasks. (See a list of [operations that trigger validation](../../../documents/schema-validation/schema-validation_overview#when-is-validation-triggered).)
   </Admonition>
