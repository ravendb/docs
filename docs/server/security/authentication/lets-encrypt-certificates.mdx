...
`Renew` on the server certificate. This will trigger the same certificate replacement process which was described [above](../../../server/security/authentication/lets-encrypt-certificates.mdx##automatic-renewal-for-lets-encrypt-certificates-obtained-via-ravendb).
