...
* Queries defined with the `query` method are **translated by the RavenDB client to [RQL](../../querying/rql/what-is-rql.mdx)** when sent to the server.
...
* Queries defined with [query](../../querying/overview.mdx#session-query) are translated by the RavenDB client to [RQL](../../querying/rql/what-is-rql.mdx) when sent to the server.  
...
| Return Value | |
| - | - |
| `object` | Instance implementing `IDocumentQuery` exposing the additional [query methods](../../querying/overview.mdx#query-api). |
