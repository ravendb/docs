...
An agent can invoke other agents as tools. To do this, reference the target
...
* **Example**  
  ```csharp
    ...
    chat.Handle<ChangeUserNameRequest, ActionToolResult>(
        "pharma-input-validator-agent/ChangeUserName",
...
This is useful, for example, when you want to pre-load the conversation with contextual information that would normally be retrieved by a tool, such as user preferences, permissions, or other data that is already available to the client and does not require a real tool call to be obtained.  
...
| **Policy** | `AiAgentParameterPolicy` | Policy flags for this parameter. When an agent is used as a sub-agent, `ForbidModelGeneration` prevents the parent from generating a value for the parameter; the value must be inherited from the parent's parameters or provided when the conversation is started. Default: `Default` (no special behavior). |
| **Type** | `AiAgentParameterValueType` | Expected value type of the parameter. When set to a concrete value, the server validates the provided value against it.<br />`Default` disables type validation. |
...
| **ForbidModelGeneration** | When an agent is used as a sub-agent, prevents the parent from generating a value for the parameter. The value must be inherited from the parent's parameters or provided when the conversation is started; otherwise `MissingAiAgentParameterException` is thrown. |

</TabItem>

<TabItem value="AiAgentParameterValueType" label="AiAgentParameterValueType">

**The expected value type of an agent parameter, used for validation.**
