...
description: "View and filter real-time RavenDB server logs in Studio with configurable log levels and source filtering for live diagnostics."
sidebar_position: 2
---
...
* In this article:  
  ...
  * [Logs on this view](#logs-on-this-view)  
  * [Logs on disk](#logs-on-disk)  
  * [Log stream](#log-stream)  
  * [Syntax for filter conditions](#syntax-for-filter-conditions)  

</Admonition>

<Panel heading="Admin Logs view">
...
1. [Logs on this view](#logs-on-this-view)  
...
* Adjust [monitor controls](#monitor-controls).  
* Set the [minimum logging level](#set-displayed-logs-minimum-level) 
  from which log entries are displayed.  
* Create [additional filters](#set-displayed-logs-filters) 
  to select the precise data that you want to view.  
* [Export the displayed log data](#export-displayed-logs-data) 
  to files.  
...
2. <a id="set-displayed-logs-minimum-level"/>**Min level**  
   The minimum [logging level](../../../server/troubleshooting/logging.mdx#available-logging-levels) from which log entries are displayed on this view.  
...
* Set the [minimum logging level](#define-logs-on-disk-minimum-level) 
  from which log entries are stored in log files on the server disk.  
* Create [additional filters](#define-logs-on-disk-filters) 
  to select the precise data that you want to log.  
* [Download existing log files](#download-log-files) 
...
5. **Settings**  
   ...
   Only entries that match both the selected min level (see no. 2 above) and the filter's conditions will be written to the logs. Existing entries remain unaffected.  
      ...
      * <a id="define-logs-on-disk-minimum-level"/>**A.** **Current Minimum Level**  
        The minimum [logging level](../../../server/troubleshooting/logging.mdx#available-logging-levels) from which log entries will be stored in log files.  
         ...
         * **Save the minimum level in `settings.json`**  
           ...
           If you don't save your configuration in `settings.json`, it will be overridden by the settings defined in that file at the next server restart.
...
<Panel heading="Log stream">

* This area displays live log entries as they are generated by the server, filtered by the [Logs on this view](#logs-on-this-view) settings.  
  ...
  4. **Log entry**  
        * Each log entry follows this pipe-separated (|) pattern:  
          ...
          All fields are always present (empty if not applicable).   
            ...
            * **Level** - The entry's [logging level](../../../server/troubleshooting/logging.mdx#available-logging-levels).
...
<Panel heading="Syntax for filter conditions">
