---
title: "Studio: Manage Server"
sidebar_label: Manage Server
description: "Access RavenDB server management features in Studio including certificates, server settings, client configuration, and debug tools."
sidebar_position: 0
---

import Admonition from '@theme/Admonition';
import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';
import CodeBlock from '@theme/CodeBlock';
import LanguageSwitcher from "@site/src/components/LanguageSwitcher";
import LanguageContent from "@site/src/components/LanguageContent";

# Studio: Manage Server
<Admonition type="note" title="">

* Use the **Manage Server** menu to access your server's configuration and debugging options.  

* In this page:  
   * [Manage Server menu](../../studio/server/manage-server.mdx#manage-server-menu)  
       * [Configuration options](../../studio/server/manage-server.mdx#manage-server-configuration-options)  
       * [Debug options](../../studio/server/manage-server.mdx#debug-debugging-and-statistics-options)  

</Admonition>
## Manage Server menu

Open the **Manage Server** menu from Studio's main menu.  

![Main menu: Click to manage server](./assets/manage-server_click-to-manage-server.png)
The menu is divided into two main sections, one to configure various server 
options and the second to debug the server and view various statistics.  

![Manage Server menu](./assets/manage-server.png)

### 1. Manage Server: Configuration options

* [Cluster](../../studio/cluster/cluster-view.mdx)  
  View and modify your cluster topology.  
* [Client Configuration](../../studio/server/client-configuration.mdx)  
  Set a client configuration that applies to all the databases in the cluster.  
* [Studio Configuration](../../studio/database/settings/studio-configuration.mdx#studio-configuration---server-wide)  
  Set server-wide Studio-related options, applied to all the databases hosted by this server.  
* [Server Settings](../../studio/server/server-settings.mdx)  
  Display settings common to all the databases hosted by this server.  
* [Admin JS Console](../../studio/server/admin-js-console.mdx)  
  Use this console to run JavaScript code, to execute advanced operations on the server.  
* [Certificates](../../studio/server/certificates/server-management-certificates-view.mdx)  
  Export, Import and Customization options for server and client certificates.  
* **Server-Wide Tasks**  
  Define server-wide 
  [External Replication](../../studio/database/tasks/ongoing-tasks/external-replication-task.mdx) 
  and [Periodic Backup](../../backup/create/periodic-tasks/database-backup#creating-and-managing-periodic-backups-via-studio) 
  tasks that apply to all the databases hosted by this server.  
* [Server-Wide Analyzers](../../studio/database/settings/custom-analyzers.mdx#server-wide-custom-analyzer-view)  
  Add server-wide custom analyzers, to split index fields into tokens 
  that can be used by indexes on all the databases hosted by this server.  
* [Server-Wide Sorters](../../querying/sorting-query-results/custom-sorters/server-wide-custom-sorters.mdx)  
  Add server-wide custom sorters, to define how query results are ordered 
  by queries made on all the databases hosted by this server.  

### 2. Debug: Debugging and statistics options

* [Admin Logs](../../monitoring/logging/admin-logs.mdx)  
  View a live stream of RavenDB log data, and modify logging settings.  
* [Traffic Watch](../../monitoring/traffic-watch/overview.mdx)  
  View the HTTP requests made to the server.  
* [Gather Debug Info](../../monitoring/debug/debug-package/create-debug-package.mdx)  
  Collect diagnostics data from selected or all databases on this 
  server or the entire cluster, to help troubleshoot and resolve issues.
* [Storage Report](../../monitoring/performance-views/storage-reports.mdx)  
  View the disk space used by the server-wide and cluster-level data.  
* [IO Stats](../../monitoring/performance-views/io-stats.mdx)  
  View, Export, and Import I/O statistics.  
* **Stack Traces**  
  Capture debugging stack traces for this server or the entire cluster.  
* [Running Queries](../../monitoring/performance-views/running-queries.mdx)  
  View the queries the server is running right now, and abort a query that runs for too long.  
* [Advanced](../../monitoring/debug/advanced-debug-view.mdx)  
  Advanced debug options, including the
  [Cluster Observer Log](../../monitoring/debug/advanced-debug-view.mdx#cluster-observer-log).  


