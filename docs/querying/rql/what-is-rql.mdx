...
description: "RQL (Raven Query Language) is RavenDB's SQL-like query syntax that powers all queries with support for filtering, projections, and functions."
sidebar_position: 0
---
...
* **What is RQL**:  
  ...
  RQL exposes the RavenDB query pipeline in a straightforward, accessible manner that is easy to interact with.    
  The RQL query structure reflects the order of operations performed by the RavenDB server when serving a query.   

* **High-level queries are translated to RQL**:  
  Any query written using high-level Session methods (`Query`, `DocumentQuery`) will be translated by the client into RQL  
  before being sent to the server for execution.
  Learn more about querying from the session in this [Query Overview](../overview.mdx).    

* **Writing RQL directly:**  
  A query can be written directly in RQL by either:  
  * Using the session's [RawQuery](../overview.mdx#session-advanced-rawquery) method in the Client API.  
  * Writing a query in the [Query view](../../studio/database/queries/query-view.mdx) in Studio, where [RQL code assistance](./rql-code-assistance.mdx) is available.
...
* In this article:
  ...
  * [RQL keywords and methods](#rql-keywords-and-methods)  
     * [`declare`](#declare)  
     * [`from`](#from)
     * [`where`](#where)
     * [`group by`](#group-by)
...
1. **Detect query source** ([`from`](#from))
   ...
   * Note: all queries in RavenDB use an index to provide results, even when you don't specify one.  
     Learn more about these **query scenarios** in this [Query Overview](../overview.mdx#queries-always-provide-results-using-an-index).

2. **Filter the data** ([`where`](#where))  
   * The index is scanned for records that match the query predicate. 

3. **Include related documents**  ([`include`](#include))
...
4. **Sort results** ([`order by`](#order-by)) 
...
6. **Project results** ([`select`](#select)) 
   ...
   * Data can be loaded ([`load`](#load)) from related documents to be used in the projected fields.  
...
- [DECLARE](#declare)
- [FROM](#from)
    - [index](#query-an-index--from-index-index-name)
- [GROUP BY](#group-by)
    - [array()](../../client-api/session/querying/how-to-perform-group-by-query.mdx#by-array-values)
- [WHERE](#where)
    - id()
    ...
    - [lucene()](../filtering-query-results/filter-with-lucene-syntax.mdx)
    ...
    - [when()](../filtering-query-results/conditional-filtering.mdx)
- [ORDER BY](#order-by)
    - [ASC | ASCENDING](../sorting-query-results/sort-query-results.mdx)
    - [DESC | DESCENDING](../sorting-query-results/sort-query-results.mdx)
    - [AS](../sorting-query-results/sort-query-results.mdx)
        - [string](../sorting-query-results/sort-query-results.mdx)
        - [long](../sorting-query-results/sort-query-results.mdx)
        - [double](../sorting-query-results/sort-query-results.mdx)
        - [alphaNumeric](../sorting-query-results/sort-query-results.mdx)
    - [NULLS FIRST | NULLS LAST](../sorting-query-results/sort-query-results.mdx) // TODO...
    - [random()](../sorting-query-results/sort-query-results.mdx#order-by-random)
    - [score()](../sorting-query-results/sort-query-results.mdx#order-by-score)
...
- [SELECT](#select)
    ...
    - [timeseries()](../../document-extensions/timeseries/querying/overview-and-syntax.mdx#rql-syntax)
...
Learn more about combining filter conditions with these operators in [Use logical operators](../filtering-query-results/use-logical-operators.mdx).
...
Learn more in [Filter by date and time](../filtering-query-results/filter-by-date-and-time.mdx).    
...
Learn more in [Conditional filtering](../filtering-query-results/conditional-filtering.mdx).
...
See [Group conditions to control precedence](../filtering-query-results/use-logical-operators.mdx#group-conditions-to-control-precedence).
...
- [including compare-exchange items](../../compare-exchange/include-cmpxchg-items.mdx#include-cmpxchg-items-when-querying) in the query response
...
Learn more about sorting in the [Sort query results](../sorting-query-results/sort-query-results.mdx) article.
