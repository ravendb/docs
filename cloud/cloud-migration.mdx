...
description: "Migrate databases to and from RavenDB Cloud using live instance import, file import, or backup restore between any RavenDB versions."
sidebar_position: 5
---
...
You can easily migrate your database using the [import data from RavenDB](../7.2/studio/database/tasks/import-data/import-from-ravendb) operation.
...
![Manage Certificates](./assets/studio-server-dashboard-manage-certificates.png)
Export the **destination server**'s **Cluster certificate**.  

![Cluster Certificate](./assets/studio-certificates-export-server-certificates.png)
Import the certificate as a **Client Certificate** by the **source server**.  

![Client Certificate](./assets/studio-certificates-client-certificate-menu.png)
Configure the Client Certificate's **Database Permissions** to include the database whose data you want to migrate.  

![Database Permissions](./assets/studio-certificates-upload-client-certificate.png)
...
Another option is to [export a database](../7.2/studio/database/tasks/export-database) from the source server in the
`.ravenDBDump` format, and upload it to another database using the
[import data from file](../7.2/studio/database/tasks/import-data/import-data-file) operation.
...
This is because after deleting a document, a [tombstone](../7.2/monitoring/tombstones/overview) is left behind as a signal for backups and
various behind-the-scene processes.
* Once all of these processes have been completed,
the tombstones are cleaned. They are cleaned every 5 minutes by default. You can configure the [tombstone cleaner time intervals](../7.2/monitoring/tombstones/configuration).
