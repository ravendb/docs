...
* When set, the `ReadBalanceBehavior` configuration will be in effect according to the   
  conditional flow described in [Client logic for choosing a node](../overview.mdx#client-logic-for-choosing-a-node).
...
* In this page:
    * [ReadBalanceBehavior options](#readbalancebehavior-options)  
    * [Initialize ReadBalanceBehavior on the client](#initialize-readbalancebehavior-on-the-client)  
    * [Set ReadBalanceBehavior on the server:](#set-readbalancebehavior-on-the-server)  
        * [By operation](#set-readbalancebehavior-on-the-server---by-operation)  
        * [From Studio](#set-readbalancebehavior-on-the-server---from-studio)  
...
### `None` (default option)
  ...
  * **Read-balance**  
    ...
    The client will always send _Read_ requests to the [preferred node](../overview.mdx#the-preferred-node).
  * **Failover**  
...
* **Read-balance**
  ...
  * A _Read_ request that is made on the store (i.e. executing an [operation](../../../operations/what-are-operations.mdx))   
...
### `FastestNode`
  ...
  * **Read-balance**  
    ...
    The fastest node is determined by a [Speed Test](../../../cluster/speed-test.mdx).
  * **Failover**  
...
* This setting can be **overriden** by setting 'ReadBalanceBehavior' on the server, see [below](#set-readbalancebehavior-on-the-server).   
...
* The `ReadBalanceBehavior` configuration can be set **on the server** by sending an [operation](../../../operations/what-are-operations.mdx).  
