...
description: "Send batch document commands to RavenDB via the REST API endpoint to execute multiple PUT, PATCH, and DELETE operations atomically."
sidebar_position: 5
---
...
* The commands are sent as a JSON array in the [request body](#body).  
...
* [Options](#batch-options) can be set to make the server wait 
...
2. Execute a [patch](../../operations/patching/single-document.mdx) on that same document.  
...
(see the format for batching a Put Attachment Command [below](#put-attachment-command)):  
...
The query string takes [batch options](#batch-options), which 
...
* `multipart/mixed; boundary=<separator>` - if the batch [_does_](#put-attachment-command) 
...
* [Put Document Command](#put-document-command)  
* [Patch Document Command](#patch-document-command)  
* [Delete Document Command](#delete-document-command)  
* [Delete by Prefix Command](#delete-by-prefix-command)  
* [Put Attachment Command](#put-attachment-command)  
* [Delete Attachment Command](#delete-attachment-command)  
### Batch Options
...
| **waitForIndexThrow**     | `boolean`  | Set to `true` to throw an exception if the indexing doesn't complete before `waitForIndexesTimeout`. <br/> Set to `false` to receive the normal response body. |
...
| **throwOnTimeoutInWaitForReplicas** | `boolean`        | Set to `true` to throw an exception if the replication doesn't complete before `waitForReplicasTimeout`. <br/> Set to `false` to receive the normal response body.                                           |
...
Format within the `Commands` array in the [request body](#request-format):  
...
|-------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------|
| **Id**                        | ID of document to create or update                                                                                                                                                                                               | Yes to update, [no to create](../../document-identifiers/working-with-document-identifiers.mdx#autogenerated-ids) |
| **ChangeVector**              | When updating an existing document, this parameter that document's expected [change vector](../../../server/clustering/replication/change-vector.mdx). If it does not match the server-side change vector a concurrency exception is thrown. <br/> An exception is also thrown if the document does not exist. | No       |
...
Update a document. A [patch](../../operations/patching/single-document.mdx) is executed on the server side and 
...
| **Patch** | A script that modifies the specified document. [Details below](#patch-request). | Yes |
| **PatchIfMissing** | An alternative script to be executed if no document with the given ID is found. This will create a new document with the given ID. [Details below](#patch-request). | No |
...
Add an [attachment](../../../document-extensions/attachments/overview.mdx) to a document, or update an existing attachment.  
...
* Format within the `Results` array in the response body:  
  * [Put Document Command](#put-document-command-1)  
  * [Patch Document Command](#patch-document-command-1)  
  * [Delete Document Command](#delete-document-command-1)  
  * [Delete by Prefix Command](#delete-by-prefix-command-1)  
  * [Put Attachment Command](#put-attachment-command-1)  
  * [Delete Attachment Command](#delete-attachment-command-1)  
...
| **@collection** | Name of the [collection](../../faq/what-is-a-collection.mdx) that contains the document. If none was specified, the collection will be `@empty`. |
...
| **PatchStatus** | See [below](#patchstatus) |
...
* In this section:  
  * [Put Document Command](#put-document-command-2)  
  * [Patch Document Command](#patch-document-command-2)  
  * [Delete Document Command](#delete-document-command-2)  
  * [Delete by Prefix Command](#delete-by-prefix-command-2)  
  * [Put Attachment Command](#put-attachment-command-2)  
  * [Delete Attachment Command](#delete-attachment-command-2)  
### Put Document Command
