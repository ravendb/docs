...
* The restore operation will create a **new database**, load backup files from the backup folder you provide it, and populate the database with the [backed up content](./overview.mdx#backup-content).  
...
* In this article:  
  * [Restoring from backup using the client API](#restoring-from-backup-using-the-client-api)
     * [Restoring a database from backup](#restoring-a-database-from-backup)
     * [Key configuration settings](#key-configuration-settings)
        * [Setting restore points](#lastfilenametorestore---setting-restore-points)
        * [Encryption settings](#encryptionkey--backupencryptionsettings---encryption-settings)
        * [Backup location settings](#restorebackupconfiguration---backup-location-settings)
     * [Syntax](#syntax)
  * [Restoring from backup using Studio](#restoring-from-backup-using-studio)
      * [Set restore options](#set-restore-options)
  * [Restoring a database to multiple cluster nodes](#restoring-a-database-to-multiple-cluster-nodes)
     * [Restoring to a single node and replicating to additional nodes](#restoring-to-a-single-node-and-replicating-to-additional-nodes)
     * [Restoring to multiple nodes simultaneously](#restoring-to-multiple-nodes-simultaneously)
  * [Restoring from server-wide backups](#restoring-from-server-wide-backups)
...
* `BackupEncryptionSettings`  
  ...
  [See details in the syntax section.](#additional-restore-classes)
...
[See details in the syntax section.](#restore-configuration-classes)

<Admonition type="note" title="">

When restoring a [snapshot](./overview.mdx#snapshot-image) from a remote destination
...
- [`Backup.TempPath`](./configuration.mdx#backuptemppath)
...
[Logical backups](./overview.mdx#logical-backup) are streamed directly from the
...
<Admonition type="note" title="">
The restore operation will conclude the process by creating a **new database** on the server and populating it with the [backup content](./overview.mdx#backup-content).  
</Admonition>
...
The new database will be available in the Databases view with the restored [content](./overview.mdx#backup-content).
...
[Learn to add cluster nodes using the cluster API](../server/clustering/cluster-api.mdx)  
[Learn to add cluster nodes using Studio](../studio/cluster/cluster-view.mdx#cluster-view-operations)
...
* **Soft-delete** the databases with the temporary names.  
  This will retain their data files on disk.  
   ...
   - Using the client API:  
     [Learn to delete a database.](../client-api/operations/server-wide/delete-database.mdx)  
...
* On the node you didn't delete the database from, expand the database group (as [shown above](#using-studio)) to include all the other nodes.  
...
Restoring databases backed up by a server-wide backup task **is identical** to restoring databases backed up by regular [database backup tasks](./create/periodic-tasks/database-backup.mdx).  
