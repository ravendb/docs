---
title: "Connection String to OpenAI and  OpenAI-Compatible Providers"
sidebar_label: "OpenAI & Compatible Providers"
description: "Define a RavenDB connection string to OpenAI or OpenAI-compatible providers for embedding generation and AI integration."
sidebar_position: 2
supported_languages: ["csharp", "java", "nodejs", "python"]
see_also:
     - title: "Vector search overview"
       link: "ai-integration/vector-search/overview"
       source: "docs"
       path: "AI Integration > Vector Search"
     - title: "Vector search using a static index"
       link: "ai-integration/vector-search/vector-search-using-static-index"
       source: "docs"
       path: "AI Integration > Vector Search"
     - title: "Vector search using a dynamic query"
       link: "ai-integration/vector-search/vector-search-using-dynamic-query"
       source: "docs"
       path: "AI Integration > Vector Search"
     - title: "Generating embeddings - overview"
       link: "ai-integration/generating-embeddings/overview"
       source: "docs"
       path: "AI Integration > Generating Embeddings"
     - title: "Embeddings generation task"
       link: "ai-integration/generating-embeddings/embeddings-generation-task"
       source: "docs"
       path: "AI Integration > Generating Embeddings"
     - title: "Connection strings - overview"
       link: "ai-integration/connection-strings/overview"
       source: "docs"
       path: "AI Integration > Connection Strings"
---

import LanguageSwitcher from "@site/src/components/LanguageSwitcher";
import LanguageContent from "@site/src/components/LanguageContent";

import OpenAiCsharp from './content/_open-ai-csharp.mdx';
import OpenAiJava from './content/_open-ai-java.mdx';
import OpenAiNodejs from './content/_open-ai-nodejs.mdx';
import OpenAiPython from './content/_open-ai-python.mdx';

<LanguageSwitcher supportedLanguages={frontMatter.supported_languages} />

<LanguageContent language="csharp">
   <OpenAiCsharp />
</LanguageContent>

<LanguageContent language="java">
   <OpenAiJava />
</LanguageContent>

<LanguageContent language="nodejs">
   <OpenAiNodejs />
</LanguageContent>

<LanguageContent language="python">
   <OpenAiPython />
</LanguageContent>


