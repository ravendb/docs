...
You can use the `declare` keyword to create a JavaScript function, that can then be called 
from a `select` clause when projection is used. Javascript functions can be used to manipulate 
and format retrieved results, and add flexibility to your queries in various other ways.  

Here, for example, an "output" Javascript function is declared to format the values projected 
by the query.  
