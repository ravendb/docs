...
If no inherited value can be found, invoking the sub-agent fails with `MissingAiAgentParameterException`.  
...
* **Note** that this policy is only meaningful when the agent is used as a
  sub-agent. On a root agent, the value is always provided when the conversation is started.  
...
When a type is set, the server validates the value provided when the conversation 
is started against the declared type.  
Mismatches are rejected with a `InvalidCastException` and a descriptive message and the conversation start is aborted.  
When the type is left at its default, no type validation is performed.  
...
| **Policy** | `AiAgentParameterPolicy` | Policy flags for this parameter. When this agent is used as a sub-agent, `ForbidModelGeneration` prevents the parent from generating a value for the parameter; the value must be inherited from the parent's parameters or provided when the conversation is started. Default: `Default` (no special behavior). |
| **Type** | `AiAgentParameterValueType` | Expected JSON value type of the parameter. When set to a concrete value, the server validates the provided value against it.<br />`Default` disables type validation. |
...
|----------|------|-------------|
| **name** | `string` | The parameter name. Matches a parameter defined in the agent configuration, or provides a value for a sub-agent's parameter that the root agent does not declare. |
...
| **ForbidModelGeneration** | When this agent is used as a sub-agent, prevents the parent from generating a value for the parameter. The value must be inherited from the parent's parameters or provided when the conversation is started; otherwise `MissingAiAgentParameterException` is thrown. |
