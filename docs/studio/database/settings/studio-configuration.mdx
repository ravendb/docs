...
description: "Customize per-database Studio behavior in RavenDB including environment labels, disabled features, and default query settings."
sidebar_position: 4
---
...
* Settings changed in this view do not change configurations in other scopes such as Client or Server API.
  * To disable the creation of Auto-Indexes in other scopes, set [Indexing.DisableQueryOptimizerGeneratedIndexes](../../../server/configuration/indexing-configuration.mdx#indexingdisablequeryoptimizergeneratedindexes) 
    to `true`:
     * Server-wide - set it in the server's [settings.json](../../../server/configuration/configuration-options.mdx#settingsjson) file.
...
[Studio Configuration - Single Database](#studio-configuration---single-database)

* [Database Studio Configuration View](#database-studio-configuration-view)
* [Disabling Auto-Index Creation on Studio Queries or Patches](#disabling-auto-index-creation-on-studio-queries-or-patches)
  * [About Auto-Index Creation](#about-auto-indexes-and-the-studio)
  * [If Disabled, Temporarily Enable Auto-Index Creation](#if-disabled-temporarily-enable-auto-index-creation)

[Studio Configuration - Server-wide](#studio-configuration---server-wide)

* [Server Studio Configuration View](#server-studio-configuration-view)
...
* If set to `true`, **Disable auto-index-creation** will prevent future queries or patches from creating Auto-Indexes.  
  ...
  * You can temporarily allow auto-index creation as a result of a query or patch in the Studio [query settings](../queries/query-view.mdx#query-view) 
    and [patch settings](../documents/patch-view.mdx#patch-configuration)
...
* Auto-indexes are created when all of the following conditions are met:
  * A query is issued without specifying an index (a [dynamic query](../../../querying/overview.mdx#dynamicquery)).
...
* Note: dynamic queries can be issued either when [querying](../queries/query-view.mdx#query-view) or when [patching](../documents/patch-view.mdx#patch-configuration).

* Auto-indexes are dynamically maintained to change automatically in response to changing query demands.  
  After a certain amount of time that an auto-index is not used [(30 minutes by default)](../../../server/configuration/indexing-configuration.mdx#indexingtimetowaitbeforemarkingautoindexasidleinmin),  
  the index goes into an [idle state](../indexes/indexes-list-view.mdx#3-index-state)
  and deleted after a set time-period [(72 hours by default)](../../../server/configuration/indexing-configuration.mdx#indexingtimetowaitbeforedeletingautoindexmarkedasidleinhrs).
...
* [Query Settings](../queries/query-view.mdx#query-settings)
* [Patch Settings](../documents/patch-view.mdx#patch-settings)
...
To configure API-based Auto-Indexing, see [Indexing.DisableQueryOptimizerGeneratedIndexes](../../../server/configuration/indexing-configuration.mdx#indexingdisablequeryoptimizergeneratedindexes)
...
4. **Default replication factor**  
   Change the default [replication factor](../create-new-database/general-flow.mdx#3-configure-replication) from 
