...
* Always call `Statistics()` on the **final** query instance that you will execute - the one on which you ultimately call `ToList(), ToListAsync(), First(), FirstOrDefault()`, and so on.
