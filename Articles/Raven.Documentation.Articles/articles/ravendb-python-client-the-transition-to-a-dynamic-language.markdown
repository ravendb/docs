...
```python 
   1. def  load(self, key_or_keys, object_type=None, includes=None):
        if not key_or_keys:
  ...
  2.def save_entity(self, key, entity, original_metadata, metadata, document,force_concurrency_check=False):
  ...
  ```
First example using ```isinstance()```.<br >
Second example for duck taping. See ```document.copy()``` (document needs to be a dict).
...
```python
 ...
 ```
* see https://docs.python.org/2/library/inspect.html for more information.
...
https://github.com/IdanHaim/RavenDB-Python-Client<br >
https://ravendb.net<br >
