...
| `none`, `operations` or `information` | sets the log mode to desired [level](../../server/configuration/logs-configuration.mdx#logsmode) |
...
When the server is configured to use a Let's Encrypt certificate obtained via the Setup Wizard,
