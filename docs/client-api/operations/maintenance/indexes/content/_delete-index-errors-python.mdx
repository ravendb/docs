...
* The operation will be executed only on the server node that is defined by the current [client-configuration](../../../../configuration/load-balance/overview.mdx#client-logic-for-choosing-a-node).

* Deleting the errors will only **clear the index errors**.  
  An index with an 'Error state' will Not be set back to 'Normal state'.  

* To just get index errors see [get index errors](../get-index-errors.mdx). 

* In this page:
    * [Delete errors from all indexes](#delete-errors-from-all-indexes)
    * [Delete errors from specific indexes](#delete-errors-from-specific-indexes)
    * [Syntax](#syntax)
