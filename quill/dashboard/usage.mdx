...
* The **Usage** view shows write activity reported under the installed Quill license,  
  measured in **Write Request Units (WRU)**.  
...
**Write Request Unit (WRU)** is the billing unit Quill uses for metered write activity.  
Under Quill's pricing model, each document insert, update, or delete counts as one WRU, regardless of document size or ingestion speed.

Writes that count include:

* Quill's initial import and later [mirroring](../overview.mdx#mirroring) of changes from a source relational database to an app's RavenDB database.
* Quill storing conversation messages in an app's RavenDB database.
* Generating embeddings for app data, which Quill's pricing model counts as writes rather than internal index maintenance.
...
Reads, queries, and the indexes Quill maintains internally are not metered.  
For current WRU billing rules and published plan allowances, see [Quill pricing](https://ravendb.net/quill/pricing).
...
open the [**License**](../dashboard/license.mdx) view under **License & usage**.
...
It does not show the plan associated with the license, its included WRU volume, the remaining allowance, or resulting charges.  

For published plan allowances and pricing, see [Quill pricing](https://ravendb.net/quill/pricing).  
To confirm the terms of your installed license, contact RavenDB sales.
