...
description: "Complete the RavenDB setup wizard to apply configuration, generate setup packages for additional nodes, and start the server."
sidebar_position: 7
---
...
* After clicking the _Finish_ button in the [Review setup](./review-setup.mdx) step,
...
* In this article:
  * [Configuration in process](#configuration-in-process) 
  * [Configuration is done](#configuration-is-done) 
    * [The downloaded ZIP file](#the-downloaded-zip-file) 
    * [The "All Set!" screen](#the-all-set-screen) 
...
While the process depends on your [Setup method](./choose-setup-method.mdx)
and [Chosen security method](./choose-security-option.mdx), it generally follows these stages:
...
See the [Configuration is done](#configuration-is-done) section below.
...
* If you are setting up a multi-node cluster, copy this _.zip_ file to each of your remaining server machines.
  You will use it to set up the other nodes. See [Use setup package](./use-setup-package.mdx).
...
When [Creating a new cluster](./choose-setup-method.mdx#set-up-a-new-cluster) with more than one node,  
...
2. **Setup instructions**:  
   ...
   For detailed steps, see [Use setup package](./choose-setup-method.mdx#use-an-existing-setup-package).    

---
    
Similarly, this info is displayed when you follow the [Create package for external setup](./choose-setup-method.mdx#create-a-package-for-external-setup) flow.      
...
* **Use it on this machine**:  
  The UI offers a handy shortcut to take you back to the [Choose setup method](./choose-setup-method.mdx) step,  
  where you can immediately apply the configuration using the [Use setup package](./choose-setup-method.mdx#use-an-existing-setup-package) flow.
...
* The Studio will open on the current node (e.g., Node **A**).  
  ...
  This is expected at this stage because those nodes have not been started yet.
    ...
    To complete the cluster, continue with the [Use setup package](./use-setup-package.mdx) flow 
