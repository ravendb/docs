...
description: "Definition of RavenDB etags — sequential numeric identifiers that track the order of document and index changes within a database."
sidebar_label: Etag
sidebar_position: 3
---
...
* [Change Vector](../server/clustering/replication/change-vector.mdx) may be changed when a document is modified locally or replicated
...
For developing with the RavenDB client API, etags are not needed. Etags are used as a "building block" of a [Change Vector](../server/clustering/replication/change-vector.mdx).
