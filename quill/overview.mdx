...
* A [channel](channels/overview.mdx) carries the conversations between your users and an agent: user messages reach the agent
  through the channel, and replies return the same way.
...
3. The [Agents table](dashboard/manage-an-app/app-agents-view.mdx) lists the app's agents, each with its status, model, and activity.
