...
* **Default page size**:
    ...
    * Querying **Corax** indexes:  
      ...
      Note: when using [Corax](../../search-engine/corax.mdx) as the search engine, indexes with more than `2,147,483,647` entries can be created and used.
...
* **Performance**:  
  ...
  See [paging and performance](#paging-and-performance) here below.

* In this page:

    * [No-paging example](#no-paging-example)
    * [Paging examples](#paging-examples)
    * [Paging and performance](#paging-and-performance)
    * [Paging through tampered results](#paging-through-tampered-results)
...
* As suggested by the hint, you may consider using [Streaming query results](../../../querying/stream-query-results.mdx) instead of paging.
...
* The server will skip duplicate results internally in the following two scenarios:
    ...
    1. When making a [Projection query](../projections.mdx) with [distinct](../distinct.mdx).

    2. When querying a [Fanout index](../../indexing-nested-data.mdx#fanout-index---multiple-index-entries-per-document).
