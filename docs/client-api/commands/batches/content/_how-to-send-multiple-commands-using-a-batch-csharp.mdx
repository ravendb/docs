...
* To run multiple commands as a **cluster-wide** transaction, do not use `SingleNodeBatchCommand`,
  ...
  See [Open a cluster-wide session](../../../session/cluster-transaction/overview.mdx#open-a-cluster-transaction).
...
* Note that the transaction created for the HTTP request when executing `SingleNodeBatchCommand`
  is separate from the transaction initiated by the session's [SaveChanges](../../../session/saving-changes.mdx) method, even if both are called within the same code block.  
