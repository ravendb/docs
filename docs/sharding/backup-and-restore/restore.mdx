...
* Backup files can be restored from local shard node storage or from 
  a [remote location](./backup.mdx#backup-storage-local-and-remote).  
...
* `.ravendbdump` files (exported from RavenDB databases) and 
  ...
  [imported](../import-and-export.mdx#import) 
...
| Restore from a [remote location](./backup.mdx#backup-storage-local-and-remote) | **Yes** | Define a [restore configuration](#define-a-restore-configuration) with S3, Azure, or Google Cloud settings. |
