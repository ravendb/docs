...
A conversation is a communication session between the client, the agent, and the LLM that maintains the history of messages exchanged between these participants since the session began.  
If query or action tools were defined, the LLM can use them during the conversation to request the agent for client actions or for data from the database.  
...
* A **response object** - a JSON schema that defines the layout for the LLM response.  
  ...
  When handling an agent via the API, the response object is provided to the agent not as part of the agent configuration but as the client invokes the agent.  
  Allowing the client to define the response object each time it starts the agent gives it the ability to tailor each conversation to its current needs.  
  </Admonition>
...
* Optional **stored conversation messages**  
  Since the LLM keeps no record of previous messages, the agent is responsible for allowing a continuous conversation. It does that by automatically recording all messages of each conversation in a dedicated document in the `@conversations` collection.  
  ...
  The conversation will remain available in the `@conversations` collection even after it ends, so it can be resumed at any future date.  
...
* Narrow your queries:  
   ...
   * Consider setting a limit on the number of results returned by each query to prevent excessive data transfer and processing, e.g. -  
