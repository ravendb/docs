...
description: "Diagnose and resolve common RavenDB revision issues including unexpected revision counts, storage growth, and configuration conflicts."
...
* **What to be aware of:**  
   ...
   * **Do not** add IDs of databases that are currently in use.  
     The ID of a RavenDB database can be found in Studio's **Stats** view.  
   ...
   * **Do not** use the IDs indicated by the database record `DatabaseTopologyIdBase64` and 
     ...
     Find these IDs using Studio's Settings &gt; **Database Record** view.  
