...
* A subscription task can be created in two ways:
  * **From the client API**:  
    The client can create a subscription task on the server using this [creation API](../api-overview.mdx#subscription-creation-options). 
  * **From the Studio**:  
...
* For more complex subscription creation scenarios, see the these [examples](../examples.mdx).  
 
* A subscription also can be modified after it has been created, see [update existing subscription](../examples.mdx#update-existing-subscription).
