...
* Queries defined with `query` or `document_query` are **translated by the RavenDB client to [RQL](../rql/what-is-rql.mdx)** when sent to the server.
    
* All queries in RavenDB use an **index** to provide results, even when you don't specify one.  
  Learn more [below](#queries-always-provide-results-using-an-index).
...
* Query results are **cached** by default. To disable query caching see [NoCaching](../customize-query.mdx#nocaching).
...
* In this article:
  * [Queries always provide results using an index](#queries-always-provide-results-using-an-index)  
  * [session.query](#session-query)  
  * [session.advanced.document_query](#session-advanced-document_query)  
  * [session.advanced.raw_query](#session-advanced-raw_query)
  * [query API](#query-api)  
  * [Syntax](#syntax)  
...
We differentiate between the following **3 query scenarios**:
  ...
  1. [Query an existing index](#1-query-an-existing-index)  
  2. [Query a collection - with filtering](#2-query-a-collection---with-filtering)  
  3. [Query a collection - query full collection | query by ID](#3-query-a-collection---query-full-collection--query-by-id)
...
* Example RQL: &nbsp; `from Employees where FirstName == "Laura"`  
  See more examples [below](#session-query).
...
* The simplest way to issue a query is using session's `query` method.   
  Customize your query with these [API methods](#query-api).  
...
* Although you can explicitly call `document_query`, in the **Python** client API, the `query` and `document_query` methods provide **identical functionality**.
  Learn more in:  
    * [What is a document query](../document-query/what-is-document-query.mdx)
    * [query -vs- document_query](../document-query/query-vs-document-query.mdx)
...
* Queries defined with [query](#session-query) or [document_query](#session-advanced-document_query) are translated by the RavenDB client to [RQL](../rql/what-is-rql.mdx)  
...
Available methods for the session's [query](#session-query) method:
    
- add_order
- [after_query_executed](../customize-query.mdx#afterqueryexecuted)
- [after_stream_executed](../customize-query.mdx#afterstreamexecuted)
...
- and_also
- [before_query_executed](../customize-query.mdx#beforequeryexecuted)
...
- negate_next
- [not](../filtering-query-results/use-logical-operators.mdx#negate-a-condition-with-not)
- [no_caching](../customize-query.mdx#nocaching)
- [no_tracking](../customize-query.mdx#notracking)
- [of_type](../../client-api/session/querying/how-to-project-query-results.mdx#oftype-(as)---simple-projection)
- open_subclause
- [order_by](../sorting-query-results/sort-query-results.mdx)
- [order_by_descending](../sorting-query-results/sort-query-results.mdx)
...
- [order_by_score](../sorting-query-results/sort-query-results.mdx#order-by-score)  
- [order_by_score_descending](../sorting-query-results/sort-query-results.mdx#order-by-score)
- or_else
- [projection](../customize-query.mdx#projection)
- [project_into](../../client-api/session/querying/how-to-project-query-results.mdx)    
- proximity
- [random_ordering](../customize-query.mdx#randomordering)
...
- take
- [timings](../customize-query.mdx#timings)
- using_default_operator
- [wait_for_non_stale_results](../customize-query.mdx#waitfornonstaleresults)
- where
- WhereBetween
...
- [where_lucene](../filtering-query-results/filter-with-lucene-syntax.mdx)
- where_not_equals
...
| Return Value | |
| - | - |
| `DocumentQuery[_T]`<br/>`RawDocumentQuery[_T]` | Instances exposing additional query methods and [extensions](#custom-methods-and-extensions-for-linq) |
