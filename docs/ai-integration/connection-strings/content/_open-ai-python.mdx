...
3. **Reasoning effort** (optional)  
   ...
   * The Python Client API's `OpenAiSettings` does not expose this setting.
     Configure it from Studio or the C# Client API.
...
5. **Temperature** (optional)  
   ...
   * OpenAI GPT-5.1 and GPT-5.2 support `temperature` only when reasoning effort is `none`,
     which is the default for both models.
     Because the Python Client API cannot set reasoning effort, it relies on this default.
   ...
   * See [GPT-5.4 parameter compatibility](https://developers.openai.com/api/docs/guides/latest-model?model=gpt-5.4#gpt-54-parameter-compatibility),
     [GPT-5.2 parameter compatibility](https://developers.openai.com/api/docs/guides/latest-model?model=gpt-5.2#gpt-52-parameter-compatibility),
     and the [GPT-5.1 model page](https://developers.openai.com/api/docs/models/gpt-5.1).
...
class OpenAiSettings(OpenAiBaseSettings):
    def __init__(
        ...
        # If None, the temperature is not sent and the model's default will be used.
