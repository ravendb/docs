...
* Replace the existing license key with the new one from Studio, as described [here](./replace-license.mdx#replace-license-from-studio).

**Or**:  

* Set [DisableAutoUpdateFromApi](../../server/configuration/license-configuration.mdx#licensedisableautoupdatefromapi) to true.

* Activate the new license key using the [configuration keys](./activate-license.mdx#activate-license-with-configuration-keys).
