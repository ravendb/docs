...
Unlike [StopIndex](../stop-index.mdx) or [StopIndexing](../stop-indexing.mdx) disable index is a persistent operation, so the index remains disabled even after a server restart.
</Admonition>
