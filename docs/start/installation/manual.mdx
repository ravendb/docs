...
description: "Install RavenDB manually by downloading the distribution package, extracting files, and configuring the server with or without security."
sidebar_position: 1
---
...
By default, when a server is started using `run.ps1` (or `run.sh` in Linux) script. It will open a browser with a [Setup Wizard](../../start/installation/setup-wizard/overview.mdx) which will guide you through the server configuration process.  
