...
3. Try to resolve the conflict by using a script, which is set up by configuring a [Conflict Resolver](#conflict-resolution-script).  
4. If all else fails, record conflicting document variants as "Conflicted Documents" which will have to be resolved [Manually](#manually-resolving-conflicts). 
