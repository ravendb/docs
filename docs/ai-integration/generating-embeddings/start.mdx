...
sidebar_label: Start
description: "Create vector embeddings from your document data to enable AI-powered similarity search using RavenDB's embeddings generation task."
sidebar_position: 0
---
...
* [Embeddings](https://en.wikipedia.org/wiki/Embedding_(machine_learning)) are numeric vectors that represent the meaning, context, or relationships within the data (e.g., text or images).
  They allow you to search for similar content using [Vector search](../vector-search/start.mdx), rather than exact matches.
    
* RavenDB can generate embeddings automatically using [Embeddings-generation tasks](./embeddings-generation-task.mdx) that you define.
  ...
  (e.g., [OpenAI](../connection-strings/open-ai.mdx)) or [RavenDB’s built-in model](../connection-strings/embedded.mdx).
...
* Alternatively, you can skip pre-generating embeddings via _Embedding-generation tasks_ and let RavenDB generate them on-the-fly during dynamic vector search queries.
  Learn more in [Vector search using a dynamic query](../vector-search/vector-search-using-dynamic-query.mdx). 
...
* Embeddings also power other AI features in RavenDB, such as [GenAI](../gen-ai-integration/start.mdx) and [AI agents](../ai-agents/start.mdx),  
