...
By default, all four resolve to the IP address provided during sign-up:
...
Direct database access through `db.<domain>` uses RavenDB's HTTPS API on port `443`;  
...
Operators, monitoring tools, chat users, and external applications use these four published hostnames to reach your Quill.
...
The additional `a.<domain>` hostname serves a different purpose and is described in [The `a` hostname](#the-a-hostname).

</Panel>

<Panel heading="The `a` hostname">

In addition to the four published hostnames, each Quill instance has a fifth hostname:
...
Quill also uses this hostname for its own connection to RavenDB inside the container.  
For that internal connection, Quill changes the URL to RavenDB's internal HTTPS port and disables topology updates.

`a.<domain>` is therefore different from the four hostnames described in the previous panel:
...
Confirm the supported DNS behavior of a.<domain>.
...
The four published hostnames normally resolve to the same host address.  
...
After the connection reaches your Quill, nginx uses the requested hostname to route it:
  ...
  * Connections to `db.<domain>` are passed to RavenDB without terminating TLS at nginx.  
...
The four published DNS records continue to return the address configured during sign-up until you explicitly update them.
...
### Update the four published records
...
docker exec quill /app/ravendb/rvn dns update \
  ...
  -d <domain> \
  -n <new-ip>=dashboard,db,public,api
```
<br/>

The command updates the four published hostnames in one request:
...
To assign several addresses to each published hostname, list them before the `=` and separate them with commas:
...
{/* TODO before publication:
1. Confirm the supported update behavior for a.<domain>. The command above intentionally lists only
   dashboard, db, public, and api while that contract remains unresolved.
2. Do not replace the direct rvn command with `docker exec quill update-dns --ip ...` until the wrapper
...
1. Confirm that the published hostnames resolve to the intended address.
...
Resolve each of the four published hostnames:
...
Do not include `a.<domain>` in this check:  
the command in [Update the records](#update-the-records) updates only the four published hostnames.
...
| ------- | ------------ | ---------- |
| The DNS update command exits with an error | The license file or base domain is invalid, outbound HTTPS failed, or the DNS service rejected the request | Read the returned error, verify the command arguments and outbound access to `api.ravendb.net`, and resolve the four records before retrying |
| A hostname does not resolve | The name or base domain is incorrect, or the record was not provisioned | Compare the hostname with the `PublicServerUrl` in the activated setup package |
| A hostname still resolves to the old address | The answer is cached, the update has not completed, or that hostname was omitted from the command | Allow for the TTL, then resolve the hostname again |
...
* During sign-up, Quill provisions four published hostnames under `<quill-name>.myquill.ai`:  
  `dashboard`, `api`, `public`, and `db`. RavenDB manages these DNS records.

* The additional `a.<domain>` hostname is RavenDB's advertised node URL.  
  It is separate from the four published Quill hostnames.
...
* If the host's externally reachable IP address changes, update the four published records with `rvn dns update`.  
  The command uses the activated license file to authorize the request.
