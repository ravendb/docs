...
namespace Raven.Documentation.Samples.ClientApi.HowTo
{
    public class HandleDocumentRelationships
    {
        ...
        #region order_2
        public class Order2 // not used in markdown
        {
        ...
        #region customer_2 
        public class Customer2 // not used in markdown
        {
        ...
        #region referral_2 
        public class Referral2  // not used in markdown
        {
            public int CustomerId { get; set; }
        ...
        public void Includes()
        {
            ...
            using (var store = new DocumentStore())
            {
                ...
                using (var session = store.OpenSession())
                {
                    #region includes_3_1_builder
                    IList<Order> orders = session  // not used in markdown
                        .Advanced
                        .DocumentQuery<Order>()
                        .Include(x => x.CustomerId)
                        .WhereGreaterThan(x => x.TotalPrice, 100)
