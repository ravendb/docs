...
* The Session [tracks](../client-api/session/what-is-a-session-and-how-does-it-work.mdx) the included compare-exchange items,
  which means their values can be accessed later in the same session without making additional requests to the server.

* In this page:
    * [Sample data](../compare-exchange/include-cmpxchg-items.mdx#sample-data)
    ...
    * [Syntax](../compare-exchange/include-cmpxchg-items.mdx#syntax)
...
To learn about ALL the available methods for creating a compare-exchange item, see [Create compare-exchange item](../compare-exchange/create-cmpxchg-items.mdx).
...
* Note:  
  Similar to the above dynamic query example, you can query the index with a [raw query](../indexes/querying/query-index.mdx#query-an-index-by-rawquery) using the provided RQL.
...
* Use the [include](../querying/rql/what-is-rql.mdx#include) keyword followed by `cmpxchg()` to include a compare-exchange item.  
...
* Use `includes.cmpxchg()` In [JavaScript functions](../querying/rql/what-is-rql.mdx#declare) within RQL queries. 
