...
clearance or higher.  
On an unsecured server, no certificate is needed.
...
`/admin/monitoring/v1/server` returns a single JSON object: a few top-level fields, and
a nested group of fields per area.
...
| Field | Description |
| - | - |
| `ServerCertificateExpirationLeftInSec` | Time left until the server certificate expires, in seconds; `0` when the certificate has expired, `-1` on an unsecured server |
...
Each entry holds a few top-level fields, and a nested group of fields per area.
