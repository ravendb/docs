...
* An Azure Service Bus queue sink task uses an **Azure Service Bus connection string** to connect to its source Azure Service Bus namespace.
  The connection string stores the namespace endpoint and the authentication settings used to access queues or topic subscriptions.
...
* An Azure Service Bus connection string can be created in any of these ways:
  * **From the Client API**:
  ...
  * **From Studio**:
    ...
    or while creating an [Azure Service Bus queue sink task](../../../studio/database/tasks/ongoing-tasks/azure-service-bus-queue-sink.mdx#define-an-azure-service-bus-sink-task).

* The example below adds a **per-database** Azure Service Bus connection string from the **Client API**.  
  The connection string is stored in the current database record and can be used by tasks in that database.

#### Authentication methods

An Azure Service Bus connection string authenticates in one of three modes.  
Set exactly one of these properties in `AzureServiceBusConnectionSettings`:

* **ConnectionString**:  
  A SAS connection string for the namespace, copied from the Azure portal.
* **EntraId**:   
  Microsoft Entra ID application credentials: `Namespace`, `TenantId`, `ClientId`, and `ClientSecret`.
* **Passwordless**:  
  Passwordless machine authentication using `Namespace`.
...
```csharp
// Define an Azure Service Bus connection string
// =============================================
var azureServiceBusConnectionString = new QueueConnectionString
{
    // The name used by the Azure Service Bus queue sink task configuration
    Name = "azure-service-bus-connection-string-name",

    // Set the queue broker type to Azure Service Bus
    BrokerType = QueueBrokerType.AzureServiceBus,

    // Configure the Azure Service Bus namespace connection
...
var putConnectionStringOp =
    new PutConnectionStringOperation<QueueConnectionString>(azureServiceBusConnectionString);
