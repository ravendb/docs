...
* **Document Revisions** are snapshots of documents and their extensions:
    ...
    * A document's live version can be [reverted](../revert-documents-to-revisions/revert-documents-to-specific-revisions.mdx) to any of its recorded revisions.
...
* Managed via the Client API or from the Studio, revisions can be created automatically or manually:
    ...
    * **Manual revisions creation**:  
      ...
      See [Force revision creation](#force-revision-creation) below.
* In this page:
    * [Revisions configuration](#revisions-configuration)
        * [Defining a revisions configuration](#defining-a-revisions-configuration)
        * [Default settings and collection-specific configurations](#default-settings-and-collection-specific-configurations)
        * [Revisions configuration options](#revisions-configuration-options)
        * [Revisions configuration execution](#revisions-configuration-execution)
        * [Enabling and disabling revisions for existing documents](#enabling-and-disabling-revisions-for-existing-documents)
    * [How it works](#how-it-works)
    * [Revisions storage](#revisions-storage)
    * [Force revision creation](#force-revision-creation)
...
* Read about the conflict revisions API here:
  [Conflict Revisions Configuration](../client-api/operations/conflict-revisions-configuration.mdx)
...
* Via Studio:
    * Manage the revisions configuration in the [Document Revisions](../../../studio/database/settings/document-revisions.mdx) settings view.
    * Inspect existing revisions and manually create a new revision in the [Revisions tab](../../../studio/database/document-extensions/revisions.mdx#revisions-tab) in the Studio's Document View.
* Via Client API:
    * Use the [ConfigureRevisionsOperation](../client-api/operations/configure-revisions.mdx) _Store_ operation to define and apply a revisions configuration.
...
* Learn more about the available configuration options in [Configure revisions operations](../client-api/operations/configure-revisions.mdx).
...
1. **Enable Revisions** so we can experiment with the feature.  
   ...
   or using the [ConfigureRevisionsOperation](../client-api/operations/configure-revisions.mdx) _Store_ operation.
...
3. **Inspect the new document's [Revisions tab](../../../studio/database/document-extensions/revisions.mdx#revisions-tab)** in the Studio.  
...
5. **Delete the document**.  
   Though you deleted the document, its **audit trail** is **not lost**: all its revisions are moved to the [Revisions Bin](../../../studio/database/document-extensions/revisions.mdx#revisions-bin),  
...
Read [here](../revisions-and-other-features.mdx#revisions-and-time-series) about revisions and **time series**.  
Read [here](../revisions-and-other-features.mdx#revisions-and-counters) about revisions and **counters**.  
Read [here](../revisions-and-other-features.mdx#revisions-and-attachments) about revisions and **attachments**.
...
* You can force the creation of a revision via the [Studio](#force-revision-creation-via-the-studio)
  or use the [Client API](#force-revision-creation-via-the-client-api).
...
* Similar to revisions created automatically due to the revisions configuration,  
  deleting a document with a manually created revision will move the revision to the [Revisions Bin](../../../studio/database/document-extensions/revisions.mdx#revisions-bin),  
...
<TabItem value="force_revision_creation_for" label="force_revision_creation_for">
