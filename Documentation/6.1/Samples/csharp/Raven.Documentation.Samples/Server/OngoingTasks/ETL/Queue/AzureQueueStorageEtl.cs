...
namespace Raven.Documentation.Samples.Server.OngoingTasks.ETL.Queue
{
    public class AzureQueueStorageEtl
    {
        ...
        public void AddAzureQueueStorageEtlTask()
        {
            using (var store = new DocumentStore())
            {
                ...
                var etlTask = new QueueEtlConfiguration()
                {
                    ...
                    // Set to false to allow failover to another node
                    PinToMentorNode = false
                }; 
                
                // Deploy (send) the task to the server vial the AddEtlOperation:
                // ==============================================================
        ...
        public void DeleteProcessedDocuments()
        {
            using (var store = new DocumentStore())
            {
                ...
                var etlTask = new QueueEtlConfiguration()
                {
                    ...
                    // Define whether to delete documents from RavenDB after they sent to the target queue
                    Queues = new List<EtlQueue>()
                    {
                        new()
                        {
                            ...
                            // from RavenDB after the message is loaded to the "Orders" queue in Azure.
        ...
        private class Definition
        {
            ...
            public sealed class QueueEtlConfiguration
            {
            ...
            public class EtlQueue
            {
                ...
                public string Name { get; set; }
                // Delete processed documents whe set to 'true'
