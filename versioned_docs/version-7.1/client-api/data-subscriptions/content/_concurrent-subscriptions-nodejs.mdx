...
* Documents that were assigned to workers whose connection has ended unexpectedly,  
  ...
  See [connection failure](#connection-failure) below.

* In this page:  
   * [Defining concurrent workers](#defining-concurrent-workers)  
   * [Dropping a connection](#dropping-a-connection)  
...
[strategy](../consumption/how-to-consume-data-subscription.mdx#worker-strategies) 
is set to [Concurrent](../consumption/how-to-consume-data-subscription.mdx#multiple-workers-per-subscription-strategy).  

* To define a concurrent worker:  
   * Create the worker using [getSubscriptionWorker](../consumption/api-overview.mdx#create-the-subscription-worker).  
   * Pass it a [subscription worker options](../consumption/api-overview.mdx#subscription-worker-options) object.  
