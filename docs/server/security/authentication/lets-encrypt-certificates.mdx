...
Once the renewed certificate is obtained, [it will be replaced](./certificate-renewal-and-rotation.mdx) in all the nodes of the cluster without needing to shut down any server.
...
If a certificate is not renewed in time and expires, see [recover from an expired certificate](./recover-from-expired-certificate.mdx).
...
the standard output. [See this example](./certificate-configuration.mdx) of how to write a file to standard output in 
Powershell.  
...
This is accomplished using a [public key pinning hash](./certificate-renewal-and-rotation.mdx#implicit-trust-by-public-key-pinning-hash).  
</Admonition>
...
* Run the following command via [`rvn admin-channel`](../../administration/cli.mdx#rvn):

```
ravendb> triggerCertificateRefresh [-replaceImmediately]
```

Both methods trigger the same certificate replacement process described [above](#automatic-renewal-for-lets-encrypt-certificates-obtained-via-ravendb).
