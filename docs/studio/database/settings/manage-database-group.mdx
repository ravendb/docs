...
description: "Add or remove nodes in a RavenDB database group through Studio, controlling replication topology, node priority, and data distribution."
sidebar_position: 9
---
...
* A **Database instance** can reside on a single node, on a number of nodes, or on all cluster nodes,  
  depending on the replication factor defined when [creating the database](../create-new-database/general-flow.mdx).  
...
* The **Database Group** refers to all the database instances in the cluster.  
  ...
  including documents, indexes, [tombstones](../../../monitoring/tombstones/overview.mdx) (which is how we replicate deletes), 
...
* At the **Database Group Level**, all the nodes are working cooperatively together.  
  ...
  Instead of selecting a leader, as in the [Cluster Level](../../cluster/cluster-view.mdx), 
...
* In this page:  
  * [Database Group Topology - View](#database-group-topology---view)  
  * [Database Group Topology - Actions](#database-group-topology---actions)  
  * [Database Group Topology - Add New Node](#database-group-topology---add-new-node)  
  * [Database Group Topology - Members Duties](#database-group-topology---members-duties)  
</Admonition>
...
2. **Tasks** for which a node is responsible for.  
   ...
   See more about the database tasks in [Ongoing Tasks](../tasks/ongoing-tasks/general-info.mdx)  
...
2. **Add a Node to the database group** - See [below](#database-group-topology---add-new-node)  
...
3. **Settings**  
   ...
   See [States Flow](../../server/debug/advanced/cluster-observer.mdx#3-states-flow)  
...
1. **Node:**  
   ...
   Note: Node should be already part of the Cluster Topology. See [Adding a node to the Cluster](../../cluster/setting-a-cluster.mdx#add-another-node-to-the-cluster)  
...
2. **Preferred Mentor:**  
   ...
   See [Members Duties](#database-group-topology---members-duties)  
...
as well as tasks that are defined on the database, are written to the [Database Record](./database-record.mdx). 
