...
* In this article:
  * [Creating and deploying a JavaScript index](#creating-and-deploying-a-javascript-index)
  * [Map index](#map-index)
     * [Map index - basic](#example-i---map-index---basic)
     * [Map index - with additional sources](#example-ii---map-index---with-additional-sources)
     * [Map index - with inline string compilation](#example-iii---map-index---with-inline-string-compilation)
  * [Multi-Map index](#multi-map-index)
...
* This base class itself inherits from [AbstractIndexCreationTask](../creating-and-deploying.mdx#define-a-static-index-using-a-custom-class),
...
* Deploy a JavaScript index using the standard creation flow as described in [Deploy a static-index](../creating-and-deploying.mdx#deploy-a-static-index).
...
* All other capabilities and features of JavaScript indexes are identical to those of [C# indexes](../indexing-basics.mdx).
...
* A map index contains a single `map` function.  
  To define an index that uses multiple map functions, see the section on [Multi-Map indexes](#multi-map-index) below.
...
Learn more about [Map indexes here](../map-indexes.mdx).
...
Learn more about [Multi-Map indexes here](../multi-map-indexes.mdx).
...
Learn more about [Map-Reduce indexes here](../map-reduce-indexes.mdx).
