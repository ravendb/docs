...
* Learn more about querying from the session in this [Query Overview](./how-to-query.mdx). 
...
1. __Detect query source__  ([`from`](#from))
   ...
   * Learn more about these __query scenarios__ in this [Query Overview](./how-to-query.mdx#queries-always-provide-results-using-an-index).

2. __Filter the data__ ([`where`](#where))  
   * The index is scanned for records that match the query predicate. 

3. __Include related documents__  ([`include`](#include))
    * [Related documents](../../how-to/handle-document-relationships.mdx#includes) that are included in the query will be retrieved and returned to the client  
...
4. __Sort results__ ([`order by`](#order-by)) 
...
- [FROM](#from)
    - index
- [GROUP BY](#group-by)
    - [array()](./how-to-perform-group-by-query.mdx#by-array-content)
- [WHERE](#where)
    - id()
    - [search()](./text-search/full-text-search.mdx)
    - cmpxchg()
    - [boost()](./text-search/boost-search-results.mdx)
    - [regex()](./text-search/using-regex.mdx)
    - [startsWith()](./text-search/starts-with-query.mdx)
    - [endsWith()](./text-search/ends-with-query.mdx)
    - [lucene()](./document-query/how-to-use-lucene.mdx)
    - [exists()](./how-to-filter-by-field.mdx)
    - [exact()](./text-search/exact-match-query.mdx)
...
- [ORDER BY](#order-by)
    ...
    - [spatial.distance()](./how-to-make-a-spatial-query.mdx#spatial-sorting)
- [LOAD](#load)
- [SELECT](#select)
    - DISTINCT
    - key()
...
For example, you can return every document from the [Companies collection](../../faq/what-is-a-collection.mdx) 
...
Learn more in [dynamic group by queries](./how-to-perform-group-by-query.mdx).
...
- [including related documents](../../how-to/handle-document-relationships.mdx#includes) in the query response
...
- [including compare-exchange items](../../operations/compare-exchange/include-compare-exchange.mdx#include-cmpxchg-items-when-querying) in the query response
- [highlighting](./text-search/highlight-query-results.mdx) results
- [get query timings](./debugging/query-timings.mdx)
- [get explanations](./debugging/include-explanations.mdx)
...
For more information, please refer to this [patching](../../operations/patching/set-based.mdx) article.  
