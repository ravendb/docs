...
* **RavenDB ETL Task** creates an [ETL](./basics.mdx) process
...
Refer to [The loadTo method](./basics.mdx#theloadtomethod) for full syntax details.
...
* Note that `load()` increases the maximum number of allowed script execution steps.  
  Read about "Execution limitations" in [How RavenDB uses Jint](../../kb/javascript-engine.mdx#how-ravendb-uses-jint).
...
* Alternatively, you can use the predefined function [`getMetadata(document)`](../../kb/javascript-engine.mdx#document-operations), 
 ...
 * You can add `@expires` tag to the metadata when storing the marker document, so it would be automatically cleaned up after a certain time
   by [the expiration extension](../../extensions/expiration.mdx#setting-the-document-expiration-time).
