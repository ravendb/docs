...
the server will wrap these operations in a [transaction](../../faq/transaction-support.mdx) upon execution in the database.  
...
when you customize [the read balance behavior](../../configuration/load-balance/read-balance-behavior.mdx) and need to ensure the next request from the user 
...
With this feature enabled the [Session](../what-is-a-session-and-how-does-it-work.mdx) will support the following _write_ commands:
