...
The only way a delete can fail is if [the etag doesn't match](http://ravendb.net/docs/2.0/http-api/http-api-comcurrency). If the document doesn't exist, a delete will still respond with a successful status code. 
