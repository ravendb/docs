...
* The patching operation is executed on the server-side within a [Single write transaction](../../../faq/transaction-support.mdx).

* This article covers patch operations on single documents from the Client API.  
  To patch multiple documents that match certain criteria see [Set based patching](../set-based.mdx).  
...
* In this page:  
  ...
  * [Examples](#examples)  
      ...
      * [Add or patch an existing array](#add-or-patch-an-existing-array)  
      * [Insert item into specific position in array](#insert-item-into-specific-position-in-array)  
      * [Modify item in specific position in array](#modify-item-in-specific-position-in-array)  
      * [Remove items from array](#remove-items-from-array)  
      * [Load documents in a script](#load-documents-in-a-script)  
      * [Remove property](#remove-property)  
      * [Rename property](#rename-property)  
      * [Add document](#add-document)  
      * [Clone document](#clone-document)  
      * [Create/Increment counter](#createincrement-counter)  
...
Patching can be performed using either of the following interfaces (detailed syntax is provided [below](#syntax)):
...
* Multiple patch methods can be defined on the [session](../../../session/what-is-a-session-and-how-does-it-work.mdx) 
  and are sent to the server for execution in a single batch (along with any other modified documents) only when calling [SaveChanges](../../../session/saving-changes.mdx).

* This API includes the following patching methods (see examples [below](#examples)):
  * `patch`
  * `addOrPatch`
  * `increment`
  * `addOrIncrement`
...
* [Operations](../../what-are-operations.mdx) allow performing ad-hoc requests directly on the document store **without** creating a session.
...
Use `crypto.randomUUID()` to generate a UUID value and store it in a document field.   
...
// Define the 'PatchCommandData':
const patchCommand = new PatchCommandData("companies/1-A", null, patchRequest);
session.advanced.defer(patchCommand);
...
// Define and send the 'PatchOperation':
const patchOp = new PatchOperation("companies/1-A", null, patchRequest);
await documentStore.operations.send(patchOp);
```
</TabItem>
</Tabs>
...
Use `crypto.digest()` to create a deterministic hash from a field value and store it in a separate field.  
For the full list of supported cryptographic methods, see [JavaScript Engine - Cryptographic methods](../../../../server/kb/javascript-engine.mdx#cryptographic-methods).
...
const patchRequest = new PatchRequest();
patchRequest.script = `
    // Create a hash from the email value
    if (this.Email) {
        var normalizedEmail = this.Email.trim().toLowerCase();
...
Use `crypto.encryptAesGcm()` to encrypt a field value, and store the generated _IV_ alongside the encrypted result.  
For the full list of supported cryptographic methods, see [JavaScript Engine - Cryptographic methods](../../../../server/kb/javascript-engine.mdx#cryptographic-methods).
...
const patchRequest = new PatchRequest();
patchRequest.script = `
    // this.SensitiveData is the document field you want to encrypt
    if (this.SensitiveData) {
       ...
       // You may wish to remove the original unencrypted field
...
* Learn more about using operations in this [Operations overview](../../what-are-operations.mdx).
