...
* Counters interact with and can trigger other RavenDB features.  
 To find out how to use counters with other features, read [Counters and Other Features](../counters-and-other-features.mdx).  
...
A Counter's value is [distributed between cluster nodes](../counters-in-clusters.mdx).  
...
* When a Counter is deleted, the sequence of Counter actions becomes non-cumulative and may require 
  [special attention](../counters-in-clusters.mdx#concurrent-delete-and-increment).  
#### Counters and Conflicts

Counter actions (for either name or value) almost never cause conflicts.  
The only exception to this is [concurrent `delete` and `increment`](../counters-in-clusters.mdx#concurrent-delete-and-increment) 
...
* **Usage flow**:  
  ...
  * If you execute [increment](../create-or-modify.mdx) or [delete](../delete.mdx), call `session.save_changes` for the action to take effect on the server.  
