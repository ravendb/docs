...
* RavenDB 5.4 and on can function as a _Producer_ in this architecture.  
  ...
  **only** RavenDB's role as a _Consumer_ through the implementation of 
  ...
  [Queue ETL section](../etl/queue-etl/overview.mdx).  
  </Admonition>
...
3. Create a new **database task**.  
   ...
   * **A**. **Kafka Sink**  
     Click to define a [Kafka Queue Sink task](./kafka-queue-sink.mdx).  
   * **B**. **RabbitMQ Sink**  
     Click to define a [RabbitMQ Queue Sink task](./rabbit-mq-queue-sink.mdx).  
   * **C**. **Azure Service Bus Sink**  
     Click to define an [Azure Service Bus Queue Sink task](./azure-service-bus-queue-sink.mdx).  
