...
Define a response object that the LLM will use to respond to the user.  
...
* **Example**  
  ...
  var newAgentsList = new List<AiAgentConfiguration>();

  foreach (var newAgent in existingAgentsList.AiAgents)
  {
      var newAgentConfig = new AiAgentConfiguration
      {
          Identifier = newAgent.Identifier,
          SystemPrompt = newAgent.SystemPrompt,
          Parameters = newAgent.Parameters,
          Queries = newAgent.Queries,
          Actions = newAgent.Actions,
          ChatTrimming = newAgent.ChatTrimming,
          SampleObject = newAgent.SampleObject,
          OutputSchema = newAgent.OutputSchema,
          ConnectionStringName = newAgent.ConnectionStringName
      };

      newAgentsList.Add(newAgentConfig);
  }
  ```
...
* `Conversation` Definition  
  ...
  ```
      ...
      | conversationId | `string` | The conversation unique ID |
...
* `StreamAsync` overloads:  
      ...
      | Property | Type | Description |
      |----------|------|-------------|
      | streamPropertyPath | `Expression<Func<TAnswer, string>>` | A lambda expression that selects the property to stream from the response object.<br /><ul><li>**The selected property must be a simple string** (and not a JSON object or an array, for example).</li><li>Strongly recommended that this would be the <strong>first property defined in the response schema</strong>.</li></ul> |
      | streamPropertyPath | `string` | The name of the property in the response object to stream.<br /><ul><li>**The selected property must be a simple string** (and not a JSON object or an array, for example).</li><li>Strongly recommended that this would be the <strong>first property defined in the response schema</strong>.</li></ul> |
