...
It can access every app database and Quill's internal configuration database (`quill-config` by default).    
...
* The downloaded archive contains a `.pfx`, a `.crt`, and a `.key` file.  
  The PHP client uses the `.crt` and `.key` together in one PEM file, as described below.  
...
Client certificates are registered separately by thumbprint, so renewing the Quill server certificate does not invalidate them.
...
| Method             | Set it to                         | Identifies                                        |
| ------------------ | --------------------------------- | ------------------------------------------------- |
| `setUrls(...)`     | `https://db.<domain>`             | The RavenDB server for your Quill instance.       |
| `setDatabase(...)` | The app's RavenDB database name   | The database containing that app's mirrored data. |
...
Pass the value of `database` to `DocumentStore::setDatabase(...)`; do not derive it from the app's display name.

For more information about authenticating this Quill API request, see [Operator Authentication](../../security-and-architecture/operator-authentication.mdx).
...
The RavenDB PHP client's latest release is `5.2.6`, and it has no 7.x release line.  
Its README states that the 5.4.x client versions support RavenDB server releases 5.4 and 6.0.  
It connects to a Quill app database and queries the mirrored documents,  
but it does not expose RavenDB 7.2 features such as the CDC Sink ongoing task type.

</Admonition>

The following example loads the client certificate in PEM form, connects to the app's RavenDB database,  
...
$certificatePassword = getenv('QUILL_CLIENT_CERTIFICATE_PASSWORD') ?: null;

$authOptions = new AuthOptions();
$authOptions->setType(CertificateType::pem());
$authOptions->setCertificatePath('C:\secrets\quill-client.pem');
$authOptions->setPassword($certificatePassword);

$store = new DocumentStore();
$store->setUrls(['https://db.<domain>']);
$store->setDatabase('northwind-app');
$store->setAuthOptions($authOptions);
...
try {
    ...
    try {
        $products = $session
            ->query(Product::class, Query::collection('Products'))
            ->take(10)
            ->toList();
...
Replace the example values with those for your Quill app:  
  ...
  * `https://db.<domain>` - replace `<domain>` with the base domain assigned to your Quill instance.
  * `northwind-app` - replace this with the app's `database` value returned by `GET https://api.<domain>/api/apps/`.
  * `C:\secrets\quill-client.pem` - the path to the PEM file holding the certificate and its private key.
...
If the PEM file's private key is not encrypted, the environment variable can be absent.

`setCertificatePath` expects a single `.pem` file containing both the certificate and its private key.  
Concatenate the `.crt` and `.key` from the downloaded archive, or convert the `.pfx` with `openssl pkcs12 -in quill-client.pfx -out quill-client.pem`.  
Set `setCaFile` or `setCaPath` as well only when the server certificate is not publicly trusted.

The explicit `Query::collection` makes the query target the mapped collection exactly.
Without it, the RavenDB Client API derives the collection name from the entity class—for example, `query(Customer::class)` targets `Customers` by default.

The `Query::collection` argument applies only to this query.
To override the collection convention throughout the application, configure `setFindCollectionName` before calling `initialize()`, which freezes the store conventions.
...
For example, a row with primary key `1` mapped to the `Products` collection becomes `Products/1`.
...
An app database can contain:

* **Mapped collections**, which contain data mirrored from the source relational database.
* **Internal collections**, whose names begin with `@` and contain operational data used by Quill or RavenDB.  
  Examples include `@channels`, `@conversations`, `@cdc-states`, and `@widget-theme-defaults`.
* **Application-owned collections**, if an application stores its own documents directly in the app database.

Do not use the `@`-prefixed internal collections as application data.

The following Quill API endpoint lists the collections currently present in the app database whose names do not begin with `@`:
...
The endpoint excludes internal collections, but it does not distinguish CDC-mapped collections from application-owned collections.
It may also omit a mapped collection that currently has no documents.

Use the response as a starting point.
Then confirm which collections are mirrored from the source relational database by checking the mapping configured for the app's CDC task
or by inspecting representative documents in RavenDB Studio.

</Panel>

<Panel heading="Inspect the CDC task">

Quill creates a [CDC Sink task](https://docs.ravendb.net/server/ongoing-tasks/cdc-sink/overview) in each app database to keep its mapped documents synchronized with the source relational database.
...
The RavenDB PHP client (`5.2.6`) does not define a CDC Sink member in `OngoingTaskType`,  
so `GetOngoingTaskInfoOperation` cannot request this task type from PHP.  
Inspect the task in RavenDB Studio, or from an application written in a client that exposes the CDC Sink task type.

</Admonition>

A **User** certificate with **Read/Write** access can inspect the task.  
...
For an application that only queries the mirrored data, use a **User** certificate with the minimum available **Read/Write** access and keep the application logically read-only.  
Quill does not grant new **Read** permissions, so this access level does not technically prevent writes.
...
In the connection example, add the following convention before the existing initialization call:
...
For more information, see [The `a` hostname](../../networking-and-dns.mdx#the-a-hostname).
...
See [Moving your Quill to a new IP](../../networking-and-dns.mdx#moving-your-quill-to-a-new-ip).
...
* Generate a **User** client certificate on the dashboard's Certificates page and grant it access only to the intended app.
  Grant the minimum available **Read/Write** access and keep a query-only application logically read-only.
    
* Configure the `DocumentStore` with `https://db.<domain>`, the app database name, and the client certificate in PEM form.
