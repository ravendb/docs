...
Use this audit approach to examine the [compliance of documents in a collection](../overview.mdx#running-an-audit-operation) with a validation schema. You can limit the number of documents to be checked and the number of error messages returned.  
...
* If no schema is defined in the index configuration, the index will use the [database-level schema](../write-validation/write-validation_api.mdx#creating-a-validation-schema).  
...
<Admonition type="note" title="">
**Note**: When the index uses a database-level schema, [disabling validation at the database level](../write-validation/write-validation_api.mdx#2-create-the-schema-configuration) will also disable auditing by the index.  
