...
sidebar_position: 2
description: "Set up, run, and use the RavenDB Kubernetes Operator."
---
...
### Deploy and operate RavenDB clusters on Kubernetes with the RavenDB Operator.

<Admonition type="note" title="">

* This article serves as an overview page for the operator workflow: it explains what the RavenDB Kubernetes Operator is, how the operator-managed deployment works, and how to use the running cluster afterward.  
...
The RavenDB Kubernetes Operator applies this pattern to a secure RavenDB cluster.  
The configuration you declare is a `RavenDBCluster` resource, that defines the RavenDB cluster you want Kubernetes to run.  
...
![RavenDB Kubernetes operator](./assets/ravendb-operator-diagram.png)
*RavenDBCluster is stored in the Kubernetes API; The RavenDB Operator reads this resource and manages the Kubernetes objects needed for the deployment; RavenDB itself runs in Pods on worker nodes.*
...
The operator uses admission webhooks, and those webhooks need TLS certificates.  
...
### Compute, licensing, networking, security, and storage

Review these requirements before you begin:

- [Compute](../../start/containers/requirements/compute)  
  Make sure the cluster can provide enough CPU and memory for RavenDB nodes and the operator.

- [Licensing](../../start/containers/requirements/licensing)  
  RavenDB must be able to read a valid license from the cluster.

- [Networking](../../start/containers/requirements/networking)  
  RavenDB uses both HTTPS and TCP.  
  Each node needs public URLs that clients and other nodes can actually reach, and RavenDB must also be able to reach itself through those addresses.

- [Security](../../start/containers/requirements/security)  
  Decide whether the cluster will use Let's Encrypt or self-signed certificates.  
  Both modes require an admin client certificate for bootstrap.

- [Storage](../../start/containers/requirements/storage)  
  RavenDB is stateful.  
  Each node needs persistent storage that survives Pod restarts and rescheduling.

</ContentFrame>

For the full environment setup, including the lab layout, `cert-manager`, and operator installation, see [Part 1: Setup & Operator Installation](../../../guides/the-ravendb-kubernetes-operator-way#part-1-setup--operator-installation).
...
If you later need to add or remove nodes, handle that change through RavenDB cluster management rather than by editing the custom resource.
