...
* To store an attachment in a remote storage destination, you must:  
    ...
    * Mark the attachment for remote storage, either when [storing](../store-attachments/store-attachments-remote.mdx)
      it or later using a [patch](../store-attachments/store-attachments-remote.mdx#schedule-existing-attachments-for-remote-upload) operation.
