...
{/* TODO: Screenshot needed — open a Production Tier Azure product on a region that supports Premium SSD v2, go to Manage > Change Storage, and capture the storage edit form showing the editable IOPS and Throughput fields for a Premium SSD v2 disk. */}
...
2. **Upgrade an existing Premium disk to Premium SSD v2** — if your product already runs on a
   **Premium** data disk and is eligible, the **Upgrade to Premium V2** button is shown in the
   *Storage* section of the product page. See
...
{/* TODO: Screenshot needed — capture the "Upgrade to Premium V2" modal that opens after clicking the Upgrade to Premium V2 button on the product page. The modal should show the IOPS and Throughput fields used to configure the new Premium SSD v2 disk. */}

The upgrade is performed **node by node**. Each node being migrated is briefly unavailable, and the
full operation may take **multiple hours** to complete. The action creates a **support request** —
the Cloud team schedules and runs the migration; you will be notified when it starts and when it
completes.
...
{/* TODO: Screenshot needed — on an Azure Production Tier product in a region that supports Premium SSD v2, open Manage > Change Storage > Migrate disk and capture the migration modal. The modal should clearly show the note indicating that the new data disk will be created as Azure Premium SSD v2. */}
