...
description: "Balance RavenDB client requests across cluster nodes using read balance behavior and load balance behavior configuration options."
sidebar_position: 0
---
...
* The [database-group-topology](../../../studio/database/settings/manage-database-group.mdx#database-group-topology---view) is the list of nodes that contain those database replicas.  
  The first node in this list is called the [preferred node](#the-preferred-node).
...
* By default, the client will access the preferred node for all _Read & Write_ requests it makes.  
  This default behavior can be changed by configuring:
    * [ReadBalanceBehavior](./read-balance-behavior.mdx) - load balancing `Read` requests only
    * [LoadBalanceBehavior](./load-balance-behavior.mdx) - load balancing `Read & Write` requests  
* In this page:  
    * [Keeping the client topology up-to-date](#keeping-the-client-topology-up-to-date)
    * [Client logic for choosing a node](#client-logic-for-choosing-a-node)
    * [The preferred node](#the-preferred-node)
    * [Single-node session usage](#single-node-session-usage)
...
* For more information see [Topology in the client](../../cluster/how-client-integrates-with-replication-and-cluster.mdx#cluster-topology-in-the-client).  
...
* Use the __specified node__:   
  A client can explicitly specify the target node when executing a [server-maintenance operation](../../operations/what-are-operations.mdx#server-maintenance-operations).  
  Learn more in [switch operation to a different node](../../operations/how-to/switch-operations-to-a-different-node.mdx).  
* Else, if using-session-context is defined, use __LoadBalanceBehavior__:  
  Per session, the client will select a node based on the [session context](./load-balance-behavior.mdx#loadbalancebehavior-options).  
  All `Read & Write` requests made on the session will be directed to that node.
* Else, if defined, use __ReadBalanceBehavior__:  
  `Read` requests: The client will select a node based on the [read balance options](./read-balance-behavior.mdx#readbalancebehavior-options).  
  `Write` requests: All _Write_ requests will be directed to the preferred node. 
* Else, use the __preferred node__:  
  Use the [preferred node](#the-preferred-node) for both `Read & Write` requests.
...
* The preferred node can be explicitly set by:  
  ...
  * Sending [ReorderDatabaseMembersOperation](../../operations/server-wide/reorder-database-members.mdx) from the client code.  
...
* When using a [single-node session](../../session/cluster-transaction/overview.mdx#single-node),  
...
* If `ReadBalanceBehavior` or `LoadBalanceBehavior` are defined,  
  ...
  then use [Write Assurance](../../session/saving-changes.mdx#waiting-for-replication---write-assurance).  
