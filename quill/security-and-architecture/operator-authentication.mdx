...
By default, it matches your Quill name. `<recorded-volume-name>` is the volume name specified before `:/var/lib/quill` in that command.
...
outside the persistent volume mounted at `/var/lib/quill`. Recreating the container discards those keys and invalidates previously issued sessions.

</Admonition>

Recreating the container preserves the Quill data stored in the persistent volume mounted at `/var/lib/quill`,  
