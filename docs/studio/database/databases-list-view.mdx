...
* In this page:  
  ...
  * [Creating New Database Options](#creating-new-database-options)  
...
1. **Database state**    
   ...
   * Note:  
     If the database is contained in a cluster's node that is different than the node that is currently viewed in this Studio instance, then 'Remote' will be indicated - see more [below](#remote-database).
...
1. **Create new database**  
   A new database can be created from scratch, from a backup copy, or from existing 3.x data - see more [below](#creating-new-database-options).

2. **Manage group**  
   Click to manage the nodes that contain this database - see more about [The Database Group](./settings/manage-database-group.mdx).

3. **Disable/Enable the database**  
   A disabled database cannot be accessed. 

4. **More actions**  
   Pause indexing, Disable indexing, Compact database - see more [below](#more-actions). 
...
1. **Create new database from scratch**   
   See detailed flow [here](./create-new-database/general-flow.mdx)  

2. **Create new database from an existing backup copy**   
   See detailed flow [here](../../backup/restore.mdx#restoring-from-backup-using-studio)  

3. **Create new database from legacy data files**      
   Create a new database from a 3.x RavenDB version database    
   See detailed flow [here](./create-new-database/from-legacy-files.mdx)
