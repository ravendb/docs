...
* In this page:  
   * [Revisions and Counters](#revisions-and-counters)  
   * [Revisions and Time Series](#revisions-and-time-series)  
   * [Revisions and Attachments](#revisions-and-attachments)  
   * [Revisions and Replication](#revisions-and-replication)
   * [Revisions and ETL](#revisions-and-etl)
   * [Revisions and Backup](#revisions-and-backup)  
   * [Revisions and Data Subscriptions](#revisions-and-data-subscriptions)
   * [Revisions Import and Export](#revisions-import-and-export)
...
* A revision will Not be created upon modifying the value of an existing [counter](../../counters/overview.mdx).
### Stored data:
...
When a document is [reverted](../revert-documents-to-revisions/revert-documents-to-specific-revisions.mdx) to a revision that has counters,  
the counters from the revision are restored to functionality along with their values.  
### Extract counters data from revisions:

Use [GetMetadataFor](../client-api/session/loading.mdx#get-revisions-metadata) to get the revisions metadata for a specified document,  
...
* A revision will Not be created upon modifying the values of an existing [time series](../../timeseries/overview.mdx).
### Stored Data:
...
* Read more about Revisions and Time Series [here](../../timeseries/time-series-and-other-features.mdx#revisions).
...
When a document is [reverted](../revert-documents-to-revisions/revert-documents-to-specific-revisions.mdx) to a revision that has a time series:  
...
A document revision will be created when:  
  ...
  * A new [attachment](../../attachments/overview.mdx) is **added** to the document.  
...
When a document is [reverted](../revert-documents-to-revisions/revert-documents-to-specific-revisions.mdx) to a revision that has attachments,  
...
Revisions are [backed up](../../../backup/overview.mdx#backup-content) both by a logical-backup and by a snapshot.
...
* Revisions can be imported and exported with a `.ravendbdump` file:
    ...
    * From the [import](../../../studio/database/tasks/import-data/import-data-file.mdx#import-options) and [export](../../../studio/database/tasks/export-database.mdx#advanced-export-options) views in the Studio  
