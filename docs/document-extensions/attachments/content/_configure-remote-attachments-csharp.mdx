...
* To store an attachment in a remote storage destination, you must first:  
    * Enable the feature.
    * Configure the remote destinations. (Currently, only Azure and Amazon S3 are supported).
...
5. **Set max number of concurrent uploads**  
    Toggle ON to define the maximum number of attachments that can be uploaded to remote storage simultaneously.  
...
1. **Destination Identifier**  
    The unique identifier string that you have assigned to this remote storage destination.  
...
5. **Destination identifier**  
   Enter a unique custom identifier for this Amazon S3 destination.  
...
2. **Destination identifier**  
   Enter a unique custom identifier for this Azure destination.  
   The background task will use this identifier to reference this destination configuration. 
3. **Storage container**  
