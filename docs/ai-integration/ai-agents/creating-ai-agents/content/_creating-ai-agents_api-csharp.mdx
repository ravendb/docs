...
* In this article:
   ...
   * [Managing conversations](../../../../ai-integration/ai-agents/creating-ai-agents/creating-ai-agents_api.mdx#managing-conversations)
      ...
      * [Adding attachments](../../../../ai-integration/ai-agents/creating-ai-agents/creating-ai-agents_api.mdx#adding-attachments)
...
#### Forbidding model-generated parameter values

When this agent is used as a **sub-agent** and a parent agent invokes it without
supplying a value for one of its parameters, the default behavior is for the
parent's LLM to generate a value. For parameters that represent trusted values - a
user identifier, a session token, an account number - this is not safe: the model
can invent a value.  

Use `AiAgentParameterPolicy.ForbidModelGeneration` on the parameter to block this.
A parameter marked with this policy accepts a value only if it is **inherited**
from the parent (a parent-side parameter with the same name, or a parameter passed
at conversation start). If no inherited value can be found, invoking the sub-agent
fails with `MissingAiAgentParameterException`.  

The flag is declared on the sub-agent's own parameter, so each agent controls its
own trust requirements regardless of which parent invokes it.  

* **Example**  
  ```csharp
  // Declare a user identifier that must come from a trusted source
  // and is not exposed to the LLM.
  agent.Parameters.Add(new AiAgentParameter(
      name: "userId",
...
* **Note** that this policy is only meaningful when the agent is used as a
  sub-agent. On a root agent, the value is always supplied at conversation start.  

See [Multi-agents](../../../../ai-integration/ai-agents/multi-agents.mdx#forbidding-the-parent-from-generating-a-parameter-value)
for the full picture, including an end-to-end example.  
...
When a type is set, the server validates the value supplied at conversation start
against the declared type and rejects mismatches; when the type is left at its
default, no type validation is performed.  

Declaring a type is most useful in multi-agent setups, where a parameter may flow
from one agent to another by name and the agent's author wants to guarantee that
inherited values are of the right shape.  

* **Example**  
  ```csharp
  // Declare an age parameter that must be a number.
  agent.Parameters.Add(new AiAgentParameter(
      name: "age",
...
See [AiAgentParameterValueType](../../../../ai-integration/ai-agents/creating-ai-agents/creating-ai-agents_api.mdx#enums)
in the [Syntax](../../../../ai-integration/ai-agents/creating-ai-agents/creating-ai-agents_api.mdx#syntax) section.  
...
agents from the parent agent's `SubAgents` list. Each entry contains the
**identifier** of an existing agent and a short **description** the LLM uses when
deciding whether to delegate to that sub-agent.  

For the full conceptual picture - parameter propagation, sub-conversation
isolation, action-tool path routing, and security considerations - see
[Multi-agents](../../../../ai-integration/ai-agents/multi-agents.mdx).  

**Example**  
```csharp
// Reference existing agents as sub-agents of this agent.
// Each sub-agent must have been created first (store.AI.CreateAgentAsync)
// so its identifier is available here.
agent.SubAgents =
[
    new AiAgentToolSubAgent
    {
        Identifier = "pharma-input-validator-agent",
        Description = "Validates whether the user prompt is allowed and age-appropriate."
    },
    new AiAgentToolSubAgent
    {
        Identifier = "pharma-assistant-agent",
        Description = "Provides a cautious general pharma answer."
    }
];
```

<Admonition type="note" title="">
Sub-agents live in the same database as their parent and must be created before
they can be referenced. An agent cannot spawn a new agent at runtime.
</Admonition>

<br />
See [AiAgentToolSubAgent](../../../../ai-integration/ai-agents/creating-ai-agents/creating-ai-agents_api.mdx#agent-tool-classes) in the [Syntax](../../../../ai-integration/ai-agents/creating-ai-agents/creating-ai-agents_api.mdx#syntax) section.  
See [Multi-agents](../../../../ai-integration/ai-agents/multi-agents.mdx) for the end-to-end picture.  
...
When an action tool is declared on a **sub-agent**, the client still handles the
action - but the handler is registered under a path that reflects where the
sub-agent sits in the hierarchy. Use `<sub-agent-id>/<action-name>` for a
depth-two sub-agent, and add additional identifiers to the prefix for deeper
levels.  
...
* **Example**  
  ...
  // Action declared on a depth-three sub-agent:
  chat.Handle<ChangeUserNameRequest, ActionToolResult>(
      "user-info-agent-1/user-info-agent-3/ChangeUserName",
      request => HandleChangeUserName(store, request));
  ```

See [Multi-agents: Action tool routing back to the client](../../../../ai-integration/ai-agents/multi-agents.mdx#action-tool-routing-back-to-the-client)
...
You can attach files to a conversation turn to allow the LLM to analyze file content as part of its response. Attachments are scoped to the current turn and are cleared after each `RunAsync` or `StreamAsync` call.

Two methods are available:

* **`AddAttachment`**  
  attaches a file from a `Stream`. Use this when the file content is available in memory or read from disk.
* **`CopyAttachmentFrom`**  
  copies an existing attachment from a RavenDB document into the conversation turn.  
  Use this when the file is already stored as a document attachment in RavenDB.

<Admonition type="note" title="">
Note that the LLM must support multimodal input (e.g., vision-capable models such as GPT-4o) for image attachments to be processed. Confirm attachment support with your LLM provider before use.
</Admonition>

---

#### Example: Attaching a file from disk

```csharp
// Open a file and attach it to the conversation turn
using var fileStream = File.OpenRead("monthly-report.pdf");
chat.AddAttachment("monthly-report.pdf", fileStream, "application/pdf");

// Set the user prompt
chat.SetUserPrompt("Summarize the key findings in this report.");

var response = await chat.RunAsync<ReportSummary>(CancellationToken.None);
```

---

#### Example: Copying an attachment from a RavenDB document

```csharp
// Copy an attachment that is already stored in a RavenDB document
chat.CopyAttachmentFrom("products/42-A", "spec-sheet.pdf");

// Set the user prompt
chat.SetUserPrompt("List the technical specifications from this document.");

var response = await chat.RunAsync<SpecList>(CancellationToken.None);
```

---

#### Example: Combining multiple attachments in one turn

```csharp
// Attach a file from disk
using var contract = File.OpenRead("contract.pdf");
chat.AddAttachment("contract.pdf", contract, "application/pdf");

// Also copy an existing RavenDB document attachment
chat.CopyAttachmentFrom("products/42-A", "spec-sheet.pdf");

// Set the user prompt
chat.SetUserPrompt("Does the contract cover all items listed in the spec sheet?");

var response = await chat.RunAsync<ComplianceResult>(CancellationToken.None);
```
...
Sets the user prompt for the next conversation turn, replacing any previously added prompt parts.
...
| Parameter | Type | Description |
|-----------|------|-------------|
| **userPrompt** | `string` | The text of the user's message. Replaces any previously added prompt parts. |

</TabItem>
<TabItem value="AddUserPrompt" label="AddUserPrompt">

Appends one or more text parts to the user prompt for the next conversation turn, without replacing parts that were previously added.
...
```csharp
chat.AddUserPrompt("Summarize the findings for France.", "Focus on Q4 data.");
```
<br />

| Parameter | Type | Description |
|-----------|------|-------------|
| **userPrompt** | `IEnumerable<string>` | One or more text strings to append as prompt parts. |

</TabItem>
<TabItem value="AddAttachment" label="AddAttachment">

Attaches a file to the current conversation turn. The attachment is sent to the LLM together with the user prompt when `RunAsync` or `StreamAsync` is called, and is cleared afterwards.
...
| Parameter | Type | Description |
|-----------|------|-------------|
| **name** | `string` | A descriptive name for the attachment (e.g., `"monthly-report.pdf"`). A descriptive name is recommended as it helps the LLM understand the file's context. |
| **stream** | `Stream` | The data stream of the file to attach. |
| **contentType** | `string` | The MIME type of the attachment content (e.g., `"application/pdf"`, `"image/png"`). |

</TabItem>
<TabItem value="CopyAttachmentFrom" label="CopyAttachmentFrom">

Copies an attachment from an existing RavenDB document into the current conversation turn. The attachment is sent to the LLM together with the user prompt when `RunAsync` or `StreamAsync` is called, and is cleared afterwards.
...
| Parameter | Type | Description |
|-----------|------|-------------|
| **sourceDocumentId** | `string` | The ID of the document in RavenDB that contains the attachment. |
| **fileName** | `string` | The name of the file attached to the source document. This name is also used to identify the file in the conversation context. |
...
class AiAgentConfiguration
{
    ...
    List<AiAgentToolSubAgent> SubAgents
...
| **SubAgents** | `List<AiAgentToolSubAgent>` | Other agents that this agent can invoke as sub-agents. See [Sub-agents as tools](../../../../ai-integration/ai-agents/creating-ai-agents/creating-ai-agents_api.mdx#sub-agents-as-tools). |
...
// Initialize with name, description, and a policy
public AiAgentParameter(string name, string description, AiAgentParameterPolicy policy)

// Initialize with name, description, visibility flag, and a policy
public AiAgentParameter(string name, string description, bool sendToModel, AiAgentParameterPolicy policy)

// Initialize with name, description, visibility flag, policy, and value type
public AiAgentParameter(string name, string description, bool sendToModel, AiAgentParameterPolicy policy, AiAgentParameterValueType type)
```
<br />
...
| **Policy** | `AiAgentParameterPolicy` | Policy flags for this parameter. When this agent is used as a sub-agent, `ForbidModelGeneration` prevents the parent from generating a value for the parameter; the value must be inherited from the parent's parameters or supplied at conversation start. Default: `Default` (no special behavior). |
| **Type** | `AiAgentParameterValueType` | Expected JSON value type of the parameter. When set to a concrete value, the server validates the supplied value against it. `Default` disables type validation. |
...
<TabItem value="AiAgentToolSubAgent" label="AiAgentToolSubAgent">

**A reference to another agent that this agent can invoke as a sub-agent.**
...
| Property | Type | Description |
|----------|------|-------------|
| **Identifier** | `string` | The identifier of an existing agent that this agent may invoke as a sub-agent.<br />The referenced agent must be created in the same database. |
| **Description** | `string` | A short description the LLM reads when deciding whether to delegate to this sub-agent. |

See [Multi-agents](../../../../ai-integration/ai-agents/multi-agents.mdx) for the full picture, including parameter propagation and sub-conversation isolation.
...
{
    Dictionary<string, AiConversationParameter> Parameters
    int? ExpirationInSec
    int? MaxModelIterationsPerCall
}
```
...
// Create with a set of parameter values (all defaulting to SendToModel = true)
public AiConversationCreationOptions(Dictionary<string, object> parameters)

// Create with a set of fully specified conversation parameters
...
public AiConversationCreationOptions AddParameter(
    string name, object value, AiConversationParameterOptions options = null)
```
<br />

| Property | Type | Description |
|----------|------|-------------|
| **Parameters** | `Dictionary<string, AiConversationParameter>` | Values for agent parameters defined in the agent configuration, keyed by parameter name. |
...
| **MaxModelIterationsPerCall** | `int?` | Optional per-conversation override of the agent configuration's `MaxModelIterationsPerCall`. |

| `AddParameter` parameter | Type | Description |
|----------|------|-------------|
| **name** | `string` | The parameter name. Matches a parameter defined in the agent configuration, or supplies a value for a sub-agent's parameter that the root does not declare. |
| **value** | `object` | The value to assign to the parameter. |
| **options** | `AiConversationParameterOptions` | Optional. Controls conversation-level behavior of this parameter, currently limited to `SendToModel` (whether the value is exposed to the LLM for this conversation). When `null`, defaults apply. |

</TabItem>

<TabItem value="AiConversationParameter" label="AiConversationParameter">

**A single conversation-level parameter, including its value and conversation-level visibility.**
...
| **SendToModel** | `bool` | Conversation-level visibility to the LLM. When `false`, the value is hidden from every LLM in the hierarchy for this conversation, regardless of the agent configuration. Combined with `AiAgentParameter.SendToModel` as an AND gate: the LLM sees the value only if both permit it. Default: `true`. |
...
| Property | Type | Description |
|----------|------|-------------|
| **SendToModel** | `bool` | Controls conversation-level visibility of this parameter to the LLM. When `false`, the value is hidden from every LLM in the hierarchy for this conversation. The final visibility is the AND of this flag and the matching `AiAgentParameter.SendToModel` on the agent configuration. Default: `true`. |
...
<TabItem value="AiAgentParameterPolicy" label="AiAgentParameterPolicy">

**Policy flags that control how an agent parameter behaves across parent and sub-agent boundaries.**

```csharp
[Flags]
enum AiAgentParameterPolicy
{
    Default = 0,
...
| Value | Description |
|-------|-------------|
| **Default** | No special behavior. The parent's LLM may generate a value for this parameter if no inherited value is available. |
| **ForbidModelGeneration** | When this agent is used as a sub-agent, prevents the parent from generating a value for the parameter. The value must be inherited from the parent's parameters or supplied at conversation start; otherwise `MissingAiAgentParameterException` is thrown. |

</TabItem>

<TabItem value="AiAgentParameterValueType" label="AiAgentParameterValueType">

**The expected JSON value type of an agent parameter, used for validation.**

```csharp
enum AiAgentParameterValueType
{
    Default,
    String,
    Number,
...
| **Number** | The value must be a JSON number. |
| **Boolean** | The value must be a JSON boolean. |
| **ArrayOfString** | The value must be a JSON array whose items are all strings. |
| **ArrayOfNumber** | The value must be a JSON array whose items are all numbers. |
| **ArrayOfBoolean** | The value must be a JSON array whose items are all booleans. |
