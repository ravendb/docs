...
in the database. The LLM will provide the employee's ID andother details whenever it requests the agent 
to apply the tool. 
...
public async Task createAndRunAiAgent_full()
{
    ...
    agent.Queries =
    [
        // Set a query tool that to retrieve all orders sent everywhere
        new AiAgentToolQuery
            {
                // Query tool name
                Name = "retrieve-orders-sent-to-all-countries",
