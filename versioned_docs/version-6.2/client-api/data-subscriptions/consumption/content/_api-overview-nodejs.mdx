...
* In this page:  
  * [Create the subscription worker](#create-the-subscription-worker)  
  * [Subscription worker options](#subscription-worker-options)  
  * [Run the subscription worker](#run-the-subscription-worker)  
  * [Subscription batch](#subscription-batch)  
  * [Subscription batch item](#subscription-batch-item)  
  * [Subscription worker](#subscription-worker)  
...
after creating the worker, you need to [run the subscription worker](#run-the-subscription-worker) to initiate document processing.
...
| **options**          | `object` | [Subscription worker options](#subscription-worker-options) object that affect how the worker interacts with the subscription. These options do not alter the definition of the subscription itself. |

| Return value         |                                                                                                                                                                                                                                                                       |
|----------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `SubscriptionWorker` | The [subscription worker](#subscription-worker) that has been created.<br/>The worker will start processing documents once you define the worker's `on` method,<br/> which listens to the `batch` event. |
...
| **strategy**                        | `string`  | The strategy configures how the server handles connection attempts from workers to a specific subscription task.<br/><br/>Available options:<br/>`OpenIfFree` (default), `TakeOver`, `WaitForFree`, or `Concurrent`.<br/><br/>Learn more in [worker strategies](../how-to-consume-data-subscription.mdx#worker-strategies).                                         |



## Run the subscription worker

After [creating](#create-the-subscription-worker) a subscription worker, the subscription worker is still not processing any documents.  
...
|-------------------------------|------------|--------------------------------------------------------------------------------------------------------------------------------------------------------| 
| **items**                     | `Item[]`   | List of items in the batch.<br/>See [subscription batch item](#subscription-batch-item). |
...
| **openSession(options)**      | `object`    | Open a new document session - can pass [session options](../../../session/opening-a-session.mdx#session-options). |
...
If the worker fails to reach that node, it will try to [failover](../../../configuration/load-balance/overview.mdx) to another node from the session's topology list.  
