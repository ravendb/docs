...
description: "Override client-side configuration per RavenDB database in Studio, including identity parts separator and read balance behavior."
sidebar_position: 3
---
...
* You can **override** the [server-wide client configuration](../../server/client-configuration.mdx) per database:  
...
* In this article:
  * [Set the client configuration (for database)](#set-the-client-configuration-for-database)
...
2. Select whether to override the server-wide client configuration:  
   Toggle **"Use server config"** to make this database use the settings defined [server-wide](../../server/client-configuration.mdx).  
...
3. **Identity parts separator**
   Set the **separator character** used in generated document IDs of the following types:
   * [Identity IDs](../../../server/kb/document-identifier-generation.mdx#identity-id)
     The server will use this character when generating identity IDs.
   * [HiLo IDs](../../../server/kb/document-identifier-generation.mdx#hilo-algorithm-id)
     The client will include this character when generating HiLo IDs.
   * [Server-side IDs](../../../server/kb/document-identifier-generation.mdx#server-side-id)
     To trigger server-side ID generation, the ID you provide must end with this character.
     The server will include this character in the generated server-side ID.

    You can use any single character except `|` (pipe).
    Default value: `/`.
    To set this character from the Client API, see [Customizing the separator character](../../../server/kb/document-identifier-generation.mdx#customizing-the-separator-character).
