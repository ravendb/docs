...
description: "Horizontally scale RavenDB by distributing documents across multiple shards, each hosted on different cluster nodes for parallel throughput."
...
* In this article:  
  * [Sharding](#sharding)  
     ...
     * [When should sharding be used?](#when-should-sharding-be-used)  
  * [Shards](#shards)  
     * [Shard replication](#shard-replication)  
  * [How documents are distributed among shards](#how-documents-are-distributed-among-shards)  
     * [Buckets allocation](#buckets-allocation)  
     * [Buckets population](#buckets-population)  
     * [Document extensions storage](#document-extensions-storage)  
  * [Resharding](#resharding)  
  * [Paging](#paging)  
  * [Using local IP addresses](#using-local-ip-addresses)  
  * [Creating a sharded database](#creating-a-sharded-database)  
...
With sharding, as the volume of stored data grows, the database can be scaled out by splitting it into [shards](#shards).  
...
(see [Sharding.ShardExecutor.UseHttpCompression](./configuration.mdx#shardingshardexecutorusehttpcompression)
and [Sharding.ShardExecutor.UseHttpDecompression](./configuration.mdx#shardingshardexecutorusehttpdecompression)),  
and a timeout can be set on the orchestrator's requests to the shards (see [Sharding.OrchestratorTimeoutInMin](./configuration.mdx#shardingorchestratortimeoutinmin)).
...
automatically-appointed [orchestrator node](#client-server-communication)
...
Studio can be used to relocate ([reshard](./resharding.mdx)) documents from one shard to another.  
...
(Note: This default reservation method differs when using prefixed sharding. Learn more in [Bucket management](./administration/sharding-by-prefix.mdx#bucket-management)).
...
(Note: This default hashing method differs when using prefixed sharding. Learn more in [Bucket management](./administration/sharding-by-prefix.mdx#bucket-management).)  
...
* **Anchoring documents to a bucket**:  
  ...
  Learn more in [Anchoring documents to a bucket](./administration/anchoring-documents.mdx).  
...
* **Anchoring documents to a shard**:   
  ...
  Learn more in [Sharding by prefix](./administration/sharding-by-prefix.mdx).
...
[Resharding](./resharding.mdx) is the relocation of data from one shard to another to maintain a balanced database,  
...
Read more about paging [here](./querying.mdx#paging-query-results).  
...
* A sharded database can be created via the [Studio](./administration/studio-admin.mdx#creating-a-sharded-database) or the [Client API](./administration/api-admin.mdx).
