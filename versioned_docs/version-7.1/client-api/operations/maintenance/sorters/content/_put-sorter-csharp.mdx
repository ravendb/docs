...
* Use `PutSortersOperation` to deploy a custom sorter to the RavenDB server.  
  ...
  that is scoped to your [Document Store](../../../../setting-up-default-database.mdx).  

* To deploy a custom sorter that will apply cluster-wide, to all databases, see [put server-wide sorter](../../../server-wide/sorters/put-sorter-server-wide.mdx).

* A custom sorter can also be uploaded to the server from the [Studio](../../../../../studio/database/settings/custom-sorters.mdx).

* In this page:
    * [Put custom sorter](#put-custom-sorter)
    * [Syntax](#syntax)
...
A query example is available [here](../../../../session/querying/sort-query-results.mdx#custom-sorters).
