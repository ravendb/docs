---
title: "Transactional Outbox Pattern with RavenDB Queue ETL"
tags: [ongoing-tasks, architecture, csharp, integration]
icon: "etl"
description: "Learn how to implement the transactional outbox pattern in C# using RavenDB data subscriptions or Queue ETL. Covers RabbitMQ and Apache Kafka integration with full code examples."
...
<script type="application/ld+json">{`
{
  ...
  "@type": "TechArticle",
  "headline": "Transactional Outbox Pattern with RavenDB Queue ETL",
  "description": "Learn how to implement the transactional outbox pattern in C# using RavenDB data subscriptions or Queue ETL with RabbitMQ and Apache Kafka.",
  "url": "https://docs.ravendb.net/guides/transactional-outbox/",
  "datePublished": "2026-02-13",
  "inLanguage": "en",
  "keywords": ["RavenDB", "transactional outbox", "Queue ETL", "RabbitMQ", "Kafka", "C#"],
  ...
  "mainEntityOfPage": { "@type": "WebPage", "@id": "https://docs.ravendb.net/guides/transactional-outbox/" },
...
<script type="application/ld+json">{`
{
  ...
  "itemListElement": [
    ...
    { "@type": "ListItem", "position": 2, "name": "Transactional Outbox Pattern with RavenDB Queue ETL", "item": "https://docs.ravendb.net/guides/transactional-outbox/" }
  ]
}
`}</script>
...
![Kafka terminal output showing invoice-created messages after the RavenDB subscription worker ran](./assets/outbox1.png)

And RabbitMQ can be checked in the GUI.

![RabbitMQ management GUI showing the invoice-created queue with queued messages](./assets/outbox2.png)
...
![RavenDB Studio Ongoing Tasks panel showing available ETL task types including RabbitMQ and Kafka](./assets/outbox3.png)
...
![RavenDB Studio RabbitMQ ETL connection string configuration form](./assets/outbox4.png)
...
![RavenDB Studio Kafka ETL connection string form with bootstrap server address field](./assets/outbox5.png)
...
![RavenDB Studio ETL transformation script editor showing the loadToInvoiceCreated function with Invoices collection selected](./assets/outbox6.png)

Let’s check if the data is stored properly. Kafka:

![Kafka terminal output confirming invoice-created events published via RavenDB Queue ETL](./assets/outbox7.png)

And for RabbitMQ we can just look into the GUI.

![RabbitMQ management GUI showing messages published via RavenDB Queue ETL](./assets/outbox8.png)
