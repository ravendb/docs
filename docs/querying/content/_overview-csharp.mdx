...
* Queries defined with `Query` or `DocumentQuery` are **translated by the RavenDB client to [RQL](../rql/what-is-rql.mdx)** when sent to the server.
...
* Queries defined with [Query](#session-query) or [DocumentQuery](#session-advanced-documentquery) are translated by the RavenDB client to [RQL](../rql/what-is-rql.mdx)  
...
* The available methods for the session's [DocumentQuery](#session-advanced-documentquery) method
  are listed in [Custom methods and extensions](../document-query/what-is-document-query.mdx#custom-methods-and-extensions).    
