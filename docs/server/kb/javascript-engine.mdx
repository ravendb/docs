...
description: "RavenDB uses the Jint JavaScript engine for patching, ETL transformations, and index definitions with supported ECMAScript features."
sidebar_position: 1
---
...
* RavenDB integrates **JavaScript scripting** across various features, including:  
  ...
  [Subscriptions](../../data-subscriptions/creating-subscription/creating-subscription_api.mdx#example-filtering-and-projecting-fields),
  ...
  [Single](../../documents/patching-documents/patch-a-single-document/api-overview.mdx) or [Set based](../../documents/patching-documents/patch-multiple-documents/client-api.mdx) document patches,  
  ...
  and [Embeddings generation tasks](../../ai-integration/generating-embeddings/embeddings-generation-task.mdx#chunking-methods-and-tokens).
...
* In this article:  
   ...
   * [Predefined JavaScript functions](../../server/kb/javascript-engine.mdx#predefined-javascript-functions)  
      * [Document operations](../../server/kb/javascript-engine#document-operations)
      * [Counter operations](../../server/kb/javascript-engine#counter-operations)
      * [Time series operations](../../server/kb/javascript-engine#time-series-operations)
      * [Attachment operations](../../server/kb/javascript-engine#attachment-operations)
      * [Compare-exchange](../../server/kb/javascript-engine#compare-exchange)
      * [String manipulation](../../server/kb/javascript-engine#string-manipulation)
      * [Arrays & objects](../../server/kb/javascript-engine#arrays--objects)
      * [Mathematical operations](../../server/kb/javascript-engine#mathematical-operations)
      * [Conversion operations](../../server/kb/javascript-engine#conversion-operations)
      * [Generating embeddings](../../server/kb/javascript-engine#generating-embeddings)
      * [Cryptographic methods](../../server/kb/javascript-engine#cryptographic-methods)
      * [Debugging](../../server/kb/javascript-engine#debugging)

</Admonition>

<Panel heading="How RavenDB uses Jint">
...
<Panel heading="Predefined JavaScript functions">
...
|-------------------------------------------------------------|-------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **id(document)**                                            | `string`    | Returns the ID of the specified document<sup>[[ex]](../../documents/patching-documents/patch-multiple-documents/client-api.mdx#updating-by-document-id)</sup>.                                                                                                                                                                                                                                                                                                                                                                                                                   |
| **load(documentId)**                                        | `object`    | Returns the document with the given ID. <br/> Used in [patching](../../documents/patching-documents/patch-a-single-document/examples/patch-examples-documents-and-metadata.mdx#loading-documents-in-a-script) or [ETL scripts](../../server/ongoing-tasks/etl/basics.mdx#transform).                                                                                                                                                                                                                                                                                                                         |
...
| **include(documentId)**                                     | `Task`      | Used in RQL [queries](../../querying/rql/what-is-rql.mdx) to include the document with the specified ID with the results.                                                                                                                                                                                                                                                                                                                                                                          |
| **put(documentId, document, [optional]changeVectorString)** | `Task`      | Creates or updates a document with the specified ID.<br/>Learn about the different document identifiers in<br/>[Document identifier generation](../../server/kb/document-identifier-generation.mdx)<sup>[[ex]](../../documents/patching-documents/patch-a-single-document/examples/patch-examples-documents-and-metadata.mdx#add-document)</sup>. <br/> This function can also clone an existing document. <br/> Note: attachments & counters will not be included in the clone<sup>[[ex]](../../documents/patching-documents/patch-a-single-document/examples/patch-examples-documents-and-metadata.mdx#clone-document)</sup>. Used in patching. |
| **del(documentId)**                                         | `void`      | Deletes the document with the specified ID. <br/> Used in [patching](../../documents/patching-documents/patch-multiple-documents/client-api.mdx#updating-a-collection-name).                                                                                                                                                                                                                                                                                                                                                             |
...
| Method Signature                                     | Return type  | Description                                                                                                                                                                                                                                                                                                                  |
|------------------------------------------------------|--------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------| 
| **counter(documentId, counterName)**                 | `number`     | Returns the value of the specified counter for the given document ID&lt;sup&gt;[[ex]](../../documents/patching-documents/patch-a-single-document/examples/patch-examples-counters-time-series-attachments.mdx#get-counter)&lt;/sup&gt;.                                                                                                                                                                     |
| **counter(document, counterName)**                   | `number`     | Returns the value of the specified counter for the given document&lt;sup&gt;[[ex]](../../documents/patching-documents/patch-a-single-document/examples/patch-examples-counters-time-series-attachments.mdx#get-counter)&lt;/sup&gt;.                                                                                                                                                                        |
| **incrementCounter(documentId, counterName, value)** | `void`       | Increments the specified counter for the given document ID.<br/>If the counter does not exist, it is implicitly created with the provided `value`. Counter values can be negative, allowing both increment and decrement operations&lt;sup&gt;[[ex]](../../documents/patching-documents/patch-a-single-document/examples/patch-examples-counters-time-series-attachments.mdx#increment-counter)&lt;/sup&gt;. |
...
| **deleteCounter(documentId, counterName)**           | `void`       | Delete the specified counter from the given document ID&lt;sup&gt;[[ex]](../../documents/patching-documents/patch-a-single-document/examples/patch-examples-counters-time-series-attachments.mdx#delete-counter)&lt;/sup&gt;.                                                                                                                                                                               |
| **deleteCounter(document, counterName)**             | `void`       | Delete the specified counter from the given document&lt;sup&gt;[[ex]](../../documents/patching-documents/patch-a-single-document/examples/patch-examples-counters-time-series-attachments.mdx#delete-counter)&lt;/sup&gt;.                                                                                                                                                                                  |
...
| **attachments(targetDocument, targetAttachmentName)<br/>.copyFrom(sourceDocId, sourceName)** | `boolean` | Copies an attachment from a source document to the target document with the specified name.<br/>Returns `true` on success,<br/>`false` if the source document, the destination document, or the source attachment was not found.<br/>See [Copy attachment via patching](../../document-extensions/attachments/copy-move-rename.mdx#copy-attachments---via-patching). |    
...
| Method Signature                | Return type  | Description                                                                                                                         |
|---------------------------------|--------------|-------------------------------------------------------------------------------------------------------------------------------------| 
| **cmpxchg(compareExchangeKey)** | `object`     | Returns the value stored in a [Compare Exchange](../../compare-exchange/overview.mdx) item for the specified key. |
...
* RavenDB provides a partial set of synchronous cryptographic methods inspired by the [Web Crypto API](https://developer.mozilla.org/en-US/docs/Web/API/Web_Crypto_API).     
* These predefined methods execute **synchronously**, returning their results immediately.
  RavenDB's JavaScript engine does not support async operations in [patches](../../documents/patching-documents/patch-a-single-document/api-overview.mdx), so the standard `crypto.subtle` async API is **not** available.  

| Method Signature                                            | Return type | Description |    
|-------------------------------------------------------------|-------------|-------------|     
| **crypto.randomUUID()**                                     | `string`    | Generates a random UUID (v4). |
| **crypto.getRandomValues(<br/>typedArray)**                 | `ArrayBufferView` | Fills the provided typed array with cryptographically secure random values. |
| **crypto.getRandomValuesBase64(<br/>lenInBytes)**           | `string`    | Generates `lenInBytes` cryptographically secure random bytes and returns them encoded as a **Base64** string. |  
| **crypto.digest(<br/>algorithm, data)**                     | `string`    | Computes a hash of `data` using `algorithm` (`'SHA-256'`, `'SHA-384'`, or `'SHA-512'`).<br/>Returns the hash as a **base64** string.<br/>`data` can be a string (automatically encoded as UTF-8) or a `BufferSource` (`TypedArray` / `ArrayBuffer`). |
| **crypto.sign(<br/>hash, key, data)**                       | `string`    | Computes an HMAC signature over `data` using the specified `hash` algorithm (`'SHA-256'`, `'SHA-384'`, or `'SHA-512'`) and `key`.<br/>Returns the signature as a **Base64** string.<br/>`key` must be a `BufferSource`; `data` can be a string or `BufferSource`. |
| **crypto.verify(<br/>hash, key, signature, data)**          | `boolean`   | Verifies an HMAC `signature` over `data` using the specified `hash` algorithm and `key`.<br/>`signature` can be a **Base64** string or `BufferSource`.<br/>Returns `true` if the signature is valid, `false` otherwise. |
| **crypto.encryptAesGcm(<br/>iv, key, data)**                | `string`    | Encrypts `data` using **AES-GCM** with the provided `iv` (initialization vector) and `key`.<br/>Returns the encrypted value encoded as a **Base64** string.<br/>`iv` and `key` can be a `BufferSource` or a Base64 string; `data` can be a string (UTF-8 encoded) or `BufferSource`. |
| **crypto.decryptAesGcm(<br/>iv, key, data [, outputType])** | `string` \| `ArrayBuffer` | Decrypts AES-GCM `data` using the provided `iv` and `key`.<br/>data can be a Base64 string or BufferSource.<br/>`outputType` controls the return format: `'string'` (default) decodes the plaintext as UTF-8; `'raw'` or `'buffer'` returns an `ArrayBuffer`. |    
...
|-------------------------------------------------|----------|--------------------------------------------------------------------------------------------------------------------------------------------| 
| **output(message)** or **console.log(message)** | `void`   | Prints message to the debug output.<br/>Used for debugging [single document patches](../../documents/patching-documents/patch-a-single-document/api-overview.mdx). |
