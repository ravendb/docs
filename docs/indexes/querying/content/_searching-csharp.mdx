...
* However, as opposed to making a dynamic search query where an auto-index is created for you,  
  when using a **static-index**:  
    ...
    * You can configure which analyzer will be used to tokenize this field.  
      Learn more in [Setting analyzer for index-field](../../using-analyzers.mdx#setting-analyzer-for-index-field).    
...
* [Dynamic index fields](../../using-dynamic-fields.mdx) are created at indexing time, with names and values derived from document content,  
...
* Dynamic index fields can be configured for full-text search just like regular index fields.  
  Learn more in [Indexing dynamic fields for full-text search](../../using-dynamic-fields.mdx#indexing-dynamic-fields-for-full-text-search).
...
* In order to prioritize results, you can provide a boost value to the searched terms.  
  ...
  * Add a boost value to the relevant index-field **inside the index definition**.  
    Refer to article [indexes - boosting](../../boosting.mdx).
...
* When making a full-text search with wildcards in the search terms, 
  ...
  [analyzer](../../using-analyzers.mdx) used in the index.
...
* When using [Corax](../../search-engine/corax.mdx) as the search engine,  
...
* or when querying a static index that uses the default [RavenStandardAnalyzer](../../using-analyzers.mdx#using-the-default-search-analyzer)
* or when querying a static index that uses the [StandardAnalyzer](../../using-analyzers.mdx#analyzers-that-remove-common-stop-words)
* or when querying a static index that uses the [NGramAnalyzer](../../using-analyzers.mdx#analyzers-that-tokenize-according-to-the-defined-number-of-characters)

the queried terms in the _Search_ method are processed with the [LowerCaseKeywordAnalyzer](../../using-analyzers.mdx#using-the-default-analyzer)  
...
* When setting a [custom analyzer](../../using-analyzers.mdx#creating-custom-analyzers) in your index to tokenize field content,
...
When using the default Exact analyzer in your index (which is [KeywordAnalyzer](../../using-analyzers.mdx#using-the-default-exact-analyzer)),  
