...
This function can also be used to clone an existing document (Note: Attachments will not be attached to the clone)&lt;sup&gt;[[ex]](../../client-api/operations/patching/single-document.mdx#clone-document)&lt;/sup&gt;.  |
...
| **output(message)** | `void` | Used for [single document patches](../../client-api/operations/patching/single-document.mdx) debug. |
