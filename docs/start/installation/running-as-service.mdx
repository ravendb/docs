...
* After completing the Server configuration process either via the [Setup Wizard](./setup-wizard/overview.mdx) 
  or [Manually](./manual.mdx), you can register the Server as a Service using the `rvn` tool that can be found inside the RavenDB Server 
...
The prerequisites for running RavenDB as a Service are defined [here](../getting-started.mdx#prerequisites).  
