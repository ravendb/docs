...
* Expand **Advanced options** and enter your query, written in 
  ...
  **SQL statement** box.  
  ...
  e.g., this RQL query retrieves the destination city and freight charge of orders 
  shipped to France:  
...
* The preview shows only the fields the query selects, plus two columns added by RavenDB:  
  `id`, holding each document's ID, and `json`, holding results that do not fit the 
  regular columns.  
