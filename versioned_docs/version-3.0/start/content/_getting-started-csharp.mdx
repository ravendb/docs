...
Now, we will focus on the distribution package (if you want to read about the installer go [here](../../server/installation/using-installer.mdx)). The package contains the following directories:

- `Backup` - contains [Raven.Backup](../../server/administration/backup-and-restore.mdx) utility for doing backups,
...
- `Smuggler` - contains utility for [importing and exporting](../../server/administration/exporting-and-importing-data.mdx) data between servers,
- `Web` - contains all files needed for [IIS deployment](../../server/installation/iis.mdx)

In the package there is a file `Start.cmd` which will start a server as a console application (aka [debug mode](../../server/troubleshooting/running-in-debug-mode.mdx)), which is great for development purposes or simply to try out various functionalities, but not for production release. When server has started, [the Studio](../../studio/accessing-studio.mdx) will be available at `http://localhost:<port>/`. RavenDB is configured by default to use port 8080, or the next available, if this is one is occupied.
