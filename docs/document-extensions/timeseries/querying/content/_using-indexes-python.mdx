...
* **Time series index**:
    ...
    * STATIC-time-series-indexes can be defined from the [Client API](../../indexing.mdx) 
...
* In this page:
    ...
    * [Querying the index](#querying-the-index)
        * [Query all time series entries](#query-all-time-series-entries)
        * [Filter query results](#filter-query-results)
        * [Order query results](#order-query-results)
...
* Each **index-entry** consists of:
  ...
  * One index-field obtained from the time series [segment](../../indexing.mdx#timeseriessegment-object) header: `EmployeeID`.
...
* When querying this time series index:  
  ...
  * Each item in the results will be of type `TsIndex.IndexEntry`, which is the index-entry.  
    Different result types may be returned when the query [projects the results](#project-results).
...
* [query](../../../../querying/overview.mdx#query-overview)  
...
* [document_query](../../../../querying/document-query/query-vs-document-query.mdx)  
<TabItem value="json" label="json">
<CodeBlock language="json">
{`def document_query(
self,
