...
* A [tombstone](../../monitoring/tombstones/overview.mdx) is the marker RavenDB keeps when a
...
* RavenDB periodically [cleans up](../../monitoring/tombstones/overview.mdx#tombstone-cleanup) all the tombstones that are no longer needed by any process.
...
* In this article:
   * [Studio view](../../monitoring/tombstones/tombstones-view.mdx#studio-view)
   * [Reading the Tombstones tables](../../monitoring/tombstones/tombstones-view.mdx#reading-the-tombstones-tables)
      * [Per Collection table](../../monitoring/tombstones/tombstones-view.mdx#per-collection-table)
      * [Per Task table](../../monitoring/tombstones/tombstones-view.mdx#per-task-table)
      * [Example: a disabled index blocking cleanup](../../monitoring/tombstones/tombstones-view.mdx#example-a-disabled-index-blocking-cleanup)
...
4. **Force cleanup**  
   ...
   set by [Tombstones.CleanupIntervalInMin](../../monitoring/tombstones/configuration.mdx#tombstonescleanupintervalinmin).  
...
6. **Per Collection - Max Etags that can be deleted**  
   ...
   See [Per Collection table](../../monitoring/tombstones/tombstones-view.mdx#per-collection-table).  
...
7. **Per Task - Max Etag that can be deleted**  
   ...
   See [Per Task table](../../monitoring/tombstones/tombstones-view.mdx#per-task-table).  
