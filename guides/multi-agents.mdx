...
<Image img={require("./assets/Multi-Agents1.webp")} alt="HR Assistant chatbot showing Mike Thompson's conversation: the user uploaded a TechConf 2026 receipt for $199 and asked to report it. The agent replied with a full expense summary including vendor, receipt ID, date, line items, payment method, and pending-review status." />
...
<Image img={require("./assets/Multi-Agents1A.webp")} alt="RavenDB Studio showing the HRAssistant collection with two saved conversation documents for Mike Thompson: one belonging to the hr-assistant agent and one to the expense-manager agent, each with its own CreatedAt and LastMessageAt timestamps." />
...
<Image img={require("./assets/Multi-Agents2A.webp")} alt="RavenDB Studio showing the HR Manager's conversation log with two sub-agent call sequences: the first calls expense-manager to extract receipt data and returns a structured answer with follow-up options; the second confirms the user's intent and returns a success response indicating the expense record was created." />

We can go into the business trip bills collection and check if it is saved.

<Image img={require("./assets/Multi-Agents2.webp")} alt="RavenDB Studio showing the BusinessTripBills/1-A document created by the expense-manager agent, containing the extracted fields: EmployeeId, Vendor (TechConf 2026), Category (Conference), Amount (199 USD), ExpenseDate, Description, ReportedAt, and collection metadata." />
...
<Image img={require("./assets/Multi-Agents3.webp")} alt="RavenDB AI Hub showing the AI Agents section with two agent cards: Business Trip Expense Manager (identifier: expense-manager) and HR Assistant (identifier: hr-assistant), each displaying a short system prompt preview and a Start new chat button." />
...
<Image img={require("./assets/Multi-Agents4.webp")} alt="RavenDB Studio showing the Define sub-agents panel with a single configured sub-agent: Agent identifier set to expense-manager and a description reading 'Manages business trip expenses: analyzing receipts/bills, reporting expenses, and retrieving monthly expense summaries.'" />
...
<Image img={require("./assets/Multi-Agents5.webp")} alt="RavenDB Studio showing the HR Manager's raw conversation document, with a tool_calls entry invoking the expense-manager function and a sub-agent-action-call entry recording the delegated call to expense-manager/ReportBusinessTripExpense along with its sub-conversation ID." />
...
<Image img={require("./assets/Multi-Agents6.webp")} alt="C# code showing AiAgentConfiguration for the HR Assistant, with two highlighted sections: the top box covers Name, Identifier, ConnectionStringName, and SystemPrompt; the bottom box covers the SubAgents array containing an AiAgentToolSubAgent with Identifier and Description fields." />

The two most important elements for us are the Indetifier and the sub-agents section. As we mentioned before, identifiers are important. It is a basic way RavenDB uses to identify which agent is which.
