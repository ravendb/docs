    ...
    class HiloAlgorithm
    {
        ...
        public async Task Generate_HiLo_Ids()
        {
            ...
            using (var session = store.OpenSession())
            {
                ...
                // The following uses the overload GenerateNextIdFor(string database, object entity);
                nextId = await store.HiLoIdGenerator.GenerateNextIdForAsync(null, new Product());

                // The following uses the overload GenerateNextIdFor(string database, Type type);
