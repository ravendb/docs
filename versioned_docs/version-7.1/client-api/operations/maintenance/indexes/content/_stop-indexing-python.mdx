...
* To pause only a specific index use the [StopIndexOperation](../stop-index.mdx).  
 
* In this page:
  * [Overview](#overview)
  * [Pause indexing example](#pause-indexing-example)
  * [Syntax](#syntax)
...
* When pausing indexing from the **client**:  
  Indexing will be paused on the [preferred node](../../../../configuration/load-balance/overview.mdx#the-preferred-node) only, and Not on all the database-group nodes.  
...
* When [resetting](../reset-index.mdx) indexes 
...
* Learn to resume indexing for all indexes by a client, here: [resume indexing](../start-indexing.mdx)  
