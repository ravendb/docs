...
* A Quill **Discord bot** channel lets your users converse with an [agent](../overview.mdx#ai-agent)
  of your app in a direct message with a bot on Discord.  
  Users can message the bot from any Discord client, and the agent will answer from the app's internal database.
...
* **A Discord account.**  
  The bot is created in the Discord Developer Portal, while you are signed in with your
...
the bot can ask questions, and the agent will answer from the app's internal database.  
...
* **Messages a user sends while the bot is still answering wait their turn.**  
  The bot answers them one by one, as long as the queue Quill keeps for this user has room.
* **The bot recognizes no commands.**  
  Every message is handled as a question, including messages that start with `/`.

The behavior common to all channels, like the conversation's context, is described in the channels overview, in
[Behavior and limitations common to all channels](../channels/overview.mdx#behavior-and-limitations-common-to-all-channels).
...
The bot answers text-only messages. A message that carries an attachment, like a photo or a file, or that has no text at all, is
answered with "I can only read text messages right now."  
...
Symptoms met by the users of every bot type, not just of a Discord bot, are listed in the channels overview, in
