...
description: "Replicate and transform documents between RavenDB databases using RavenDB ETL tasks with filtered collections and custom scripts."
sidebar_position: 1
---
...
* In this article:  
  * [Add a RavenDB connection string](../../../server/ongoing-tasks/etl/raven.mdx#add-a-ravendb-connection-string)  
...
<Panel heading="Add a RavenDB connection string">

* A RavenDB ETL task uses a **RavenDB connection string** to connect to its destination database.
  The connection string stores the destination database name and one or more target cluster URLs used for topology discovery.    
...
* A RavenDB connection string can be created in any of these ways:
  * **From the Client API**:  
  ...
  * **From Studio**:  
    ...
    or while creating a [RavenDB ETL task](../../../studio/database/tasks/ongoing-tasks/ravendb-etl-task.mdx).    
    
* The example below adds a **per-database** RavenDB connection string from the **Client API**.  
  The connection string is stored in the current database record and can be used by tasks in that database.    

<Tabs>
<TabItem value="sync" label="Sync" default>
```csharp
// Define a RavenDB connection string
// ==================================
var ravenConStr = new RavenConnectionString
{
    // The name used by the RavenDB ETL task configuration
    Name = "raven-connection-string-name",

    // The destination database the ETL task will write to
    Database = "target-database-name",

    // One or more target cluster node URLs used to discover the topology
...
var putConnectionStringOp =
    new PutConnectionStringOperation<RavenConnectionString>(ravenConStr);
...
| **Database** | `string` | The destination database the ETL task writes to. |
| **TopologyDiscoveryUrls** | `string[]` | One or more target cluster node URLs used to discover the cluster topology. |

<Admonition type="note" title="Secure servers">

When the destination is a secure server, the destination cluster must trust the source cluster.  
See [Passing certificate between secure clusters](../../../studio/database/tasks/ongoing-tasks/ravendb-etl-task.mdx#passing-certificate-between-secure-clusters).
