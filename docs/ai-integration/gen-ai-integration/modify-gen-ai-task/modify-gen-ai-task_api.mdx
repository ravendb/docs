...
* To modify an existing GenAI task, register a modified task configuration object with the server using the existing TaskID, via the `UpdateGenAiOperation` store maintenance operation.  
* Note that this TaskID is **not** the [user-defined task identifier](../../../ai-integration/gen-ai-integration/create-gen-ai-task/create-gen-ai-task_studio#configure-basic-settings) that we define as part of the task configuration.  
...
* You can -  
   * Either **create a new configuration object** and populate it from scratch with new settings for your task  
     (this approach may be preferable if you want to redefine the whole configuration),  
   * Or **modify the existing task configuration** and change only selected sections of it  
 ...
 * The **create-configuration-from-scratch** example demonstrates how to create a new configuration object and populate it with all necessary configuration details.  
 * The **modify-selected-configuration-details** example demonstrates how to retrieve the existing configuration and modify selected sections of it.  
