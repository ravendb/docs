...
description: "Stream RavenDB document changes to Amazon SQS queues using ETL tasks with JavaScript transformations and message routing."
sidebar_position: 4
---
...
* An Amazon SQS ETL task uses an **Amazon SQS connection string** to connect to its destination Amazon SQS service.  
  The connection string stores the authentication settings and AWS region used to access SQS.
...
* An Amazon SQS connection string can be created in any of these ways:
  * **From the Client API**:
  ...
  * **From Studio**:
    ...
    or while creating an [Amazon SQS ETL task](../../../../studio/database/tasks/ongoing-tasks/amazon-sqs-etl.mdx#define-amazon-sqs-etl-task).

* The example below adds a **per-database** Amazon SQS connection string from the **Client API**.
  The connection string is stored in the current database record and can be used by tasks in that database.

#### Authentication methods

An Amazon SQS connection string authenticates in one of two modes.  
Set exactly one of these properties in `AmazonSqsConnectionSettings`:

* **Basic**:  
  AWS access credentials: `AccessKey`, `SecretKey`, and `RegionName`.
* **Passwordless**:  
  passwordless machine authentication for self-hosted mode.  
  Set this property to `true` only when the host machine is already authorized to access SQS.
...
```csharp
// Define an Amazon SQS connection string
// ======================================
var amazonSqsConnectionString = new QueueConnectionString
{
    // The name used by the Amazon SQS ETL task configuration
    Name = "amazon-sqs-connection-string-name",

    // Set the queue broker type to Amazon SQS
    ...
    {
        Basic = new AmazonSqsCredentials
        {
...
var putConnectionStringOp =
    new PutConnectionStringOperation<QueueConnectionString>(amazonSqsConnectionString);
...
<Tabs groupId="amazon-sqs-connection-string-syntax">
...
{
    // Set the broker type to QueueBrokerType.AmazonSqs for an Amazon SQS connection string
...
<TabItem value="AmazonSqsConnectionSettings" label="AmazonSqsConnectionSettings">
```csharp
public sealed class AmazonSqsConnectionSettings
{
    // Set this to authenticate with AWS access credentials
    public AmazonSqsCredentials Basic { get; set; }

    // Set this to true for passwordless machine authentication
