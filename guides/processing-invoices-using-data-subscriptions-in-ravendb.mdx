...
The subscription worker is opened using default [SubscriptionWorkerOptions](https://docs.ravendb.net/client-api/data-subscriptions/consumption/api-overview), which means the subscription worker [strategy](https://docs.ravendb.net/client-api/data-subscriptions/consumption/how-to-consume-data-subscription) is `OpenIfFree`.

The code here is simply creation of a subscription worker. Additional examples can be found in the [Subscription Consumption Examples](https://docs.ravendb.net/client-api/data-subscriptions/consumption/examples) documentation article.

In the batch processing code, we check `order.InvoiceCreated` and if it is `true` we skip the item. The reason for that is that in case of [subscription connection failover](https://docs.ravendb.net/client-api/data-subscriptions/what-are-data-subscriptions) we might receive an item a second time, meaning we could get an `Order` that we already created an invoice for.  
