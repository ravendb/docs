...
| **Includes and loads are not supported in streaming queries**     | Learn more in [Limitations when streaming query results in a sharded database](../sharding/querying.mdx#todo..). |
| **Ordering streamed Map-Reduce results by non-reduce-key fields** | Learn more in [Limitations when streaming query results in a sharded database](../sharding/querying.mdx#todo..). |
