...
* The downloaded archive contains a `.pfx`, a `.crt`, and a `.key` file.  
  The Python client uses the `.crt` and `.key` together in one unencrypted PEM file, as described below.  
...
with DocumentStore(["https://db.<domain>"], "northwind-app") as store:
    store.certificate_pem_path = r"C:\secrets\quill-client.pem"

    store.initialize()

    with store.open_session() as session:
        products = list(
            session.query_collection("Products", Product).take(10))
...
The RavenDB Python client reads the certificate from an unencrypted PEM file, so no password is supplied in code.  
Protect the file with filesystem permissions and keep it out of source control.

`certificate_pem_path` expects a single `.pem` file containing both the certificate and its private key.  
Concatenate the `.crt` and `.key` from the downloaded archive, or convert the `.pfx` with `openssl pkcs12 -in quill-client.pfx -out quill-client.pem -nodes`.  
Set `store.trust_store_path` as well only when the server certificate is not publicly trusted.

The RavenDB Python client converts document property names to snake_case when it matches them to the entity's `__init__` parameters,
so the mapped `Name` property is read as `product.name`, and a mapped `UnitPrice` as `product.unit_price`.

The explicit `query_collection` makes the query target the mapped collection exactly.
Without it, the RavenDB Client API derives the collection name from the entity class—for example, `query(object_type=Customer)` targets `Customers` by default.

The `query_collection` collection name applies only to this query.
To override the collection convention throughout the application, configure `find_collection_name` before calling `initialize()`, which freezes the store conventions.
...
```python
from ravendb import GetOngoingTaskInfoOperation, OngoingTaskType
...
`OngoingTaskType.CDC_SINK` and the `OngoingTaskCdcSink` result type require the RavenDB Python client `7.2.6` or later.
