...
* Queries in RavenDB can be written using `Query`, `DocumentQuery`, `RawQuery`, or directly with [RQL](../../../querying/rql/what-is-rql.mdx).    
...
* In this article:
  * [API support](../../../querying/document-query/query-vs-document-query.mdx#api-support)
  * [Immutability](../../../querying/document-query/query-vs-document-query.mdx#immutability)
  * [Default query operator](../../../querying/document-query/query-vs-document-query.mdx#default-query-operator)

</Admonition>

<Panel heading="API support">

### `Query`:

* The _Query_ API supports LINQ, the essential data access solution in .NET.
...
* The available _Query_ methods and extensions are listed in [here](../../../querying/overview.mdx#custom-methods-and-extensions-for-linq).
    
### `DocumentQuery`:

* _DocumentQuery_ does Not support LINQ.
...
* The available _DocumentQuery_ methods and extensions are listed [here](../../../querying/document-query/what-is-document-query.mdx#custom-methods-and-extensions).
...
See [Convert between DocumentQuery and Query](../../../querying/document-query/what-is-document-query.mdx#convert-between-documentquery-and-query).

</Admonition>

</Panel>

<Panel heading="Immutability">
...
* A similar usage with `DocumentQuery`:
    ...
    `eyeDocumentQuery: from Users where startsWith(Name, 'A') and Age > 21 and EyeColor = 'blue'`

    In this case, all created Lucene queries are the same query (actually the same instance).  
    This is an important hint to be aware of if you are going to reuse `DocumentQuery`.

</Panel>

<Panel heading="Default query operator">
