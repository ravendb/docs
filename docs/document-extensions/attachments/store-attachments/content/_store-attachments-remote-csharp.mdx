...
* In this article
  ...
  * [Update existing attachments to become remote attachments](../../../document-extensions/attachments/store-attachments/store-attachments-remote#update-existing-attachments-to-become-remote-attachments)  
...
### Remote attachment storage flow  
...
<Panel heading="Update existing attachments to become remote attachments">

* You can update existing **local** attachments and mark them to be uploaded to a remote storage destination using a **patch operation**.
...
Attachments are stored in the remote destination using their SHA-256 hash as the file name.
