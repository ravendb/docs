...
description: "Enable Zstd document compression per collection in RavenDB Studio to reduce database storage size with minimal performance impact."
sidebar_position: 12
---
...
* The **Documents Compression** feature employs the `Zstd` compression algorithm  
  ...
  Learn more in this [overview](../../../server/storage/documents-compression.mdx#overview).
...
* From Studio, go to **Settings** &gt; **Documents Compression** to set compression for a particular database.  
  Compression can also be set from the [Client API](../../../server/storage/documents-compression.mdx).

* To see the contents and size details of your database storage go to the [Storage Report](../../../monitoring/performance-views/storage-reports.mdx) view in Studio.  

* In this page:  
  * [Set documents compression from Studio](../../../studio/database/settings/documents-compression.mdx#set-documents-compression-from-studio)  
