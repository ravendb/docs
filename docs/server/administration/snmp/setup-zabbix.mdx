...
* Zabbix reads RavenDB's metrics over SNMP, so SNMP must be enabled on RavenDB before Zabbix can reach it.  
  See [SNMP Support](../../../monitoring/integrations/snmp/overview.mdx) for the RavenDB-side configuration.
...
* **RavenDB with SNMP enabled.**  
  ...
  See [SNMP Support](../../../monitoring/integrations/snmp/overview.mdx) for the SNMPv2c / SNMPv3 credentials and other configuration.
...
Pick the SNMP version that matches what RavenDB accepts; see [SNMP Support](../../../monitoring/integrations/snmp/overview.mdx#snmp-configuration-options) 
