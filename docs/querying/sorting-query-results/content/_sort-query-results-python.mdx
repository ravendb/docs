...
* Not every condition uses relevance-based scoring.       
  In **RavenDB 7.2.6 and later**, when using the Corax search engine, an `exists()` condition   
  (`where_exists()` in the Python client) contributes a constant score to each matching document.  
