...
* An **Elasticsearch** [ETL](./basics.mdx) task creates an ETL process 
...
* In this page:  
  * [Elasticsearch ETL](#elasticsearch-etl)  
  ...
  * [Data Delivery](#data-delivery)  
     ...
     * [Insert Only Mode](#insert-only-mode)  
  * [Elasticsearch Index Definition](#elasticsearch-index-definition)  
  * [Client API](#client-api)  
     * [Add an Elasticsearch ETL Task](#add-an-elasticsearch-etl-task)  
...
* The following steps are required when creating an Elasticsearch ETL task:  
   * Define a [connection string](#add-an-elasticsearch-connection-string) which includes:  
   ...
   * **Define the Elasticsearch Indexes**  
      ...
      * A [document identifier](#document-identifiers) 
...
* For a thorough step-by-step explanation:  
   * Learn [here](#add-an-elasticsearch-etl-task) 
...
* The structure and syntax of an Elasticsearch ETL transformation script are similar to 
  those of all other ETL types ([RavenDB ETL](./raven.mdx), 
  [SQL ETL](./sql.mdx), and 
  [OLAP ETL](./olap.mdx)) scripts.  
  ...
  Learn about ETL transformation scripts [here](./basics.mdx#transform).  

* The script **Loads** data to the Elasticsearch destination using the 
  [loadTo\\&lt;Target\\&gt;(obj)](./basics.mdx#transform) command.  
...
* In addition to specifying this document property in the script, it must be defined for the ETL task:  
   * Either set `DocumentIdProperty` through code (see [code sample](#add-an-elasticsearch-etl-task)),  
...
You can enable the task's **Insert Only** mode using [code](#add-an-elasticsearch-etl-task) 
...
* To define an Elasticsearch ETL task through the client, use the 
   ...
   * The name of a defined **Connection String**.  
     ...
     [using code](#add-an-elasticsearch-connection-string) 
...
* `ElasticSearchIndex`  
    ...
    | **DocumentIdProperty** | `string` | The [document ID property](#document-identifiers) defined on the transferred document object inside the transformation script. |
