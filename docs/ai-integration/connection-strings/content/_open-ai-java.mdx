...
3. **Reasoning effort** (optional)  
   ...
   * The Java Client API's `OpenAiReasoningEffort` enum exposes only `MINIMAL`, `LOW`, `MEDIUM`, and `HIGH`,  
     which the client sends as `Minimal`, `Low`, `Medium`, and `High`.  
...
try (IDocumentStore store = new DocumentStore(
        new String[] { "http://localhost:8080" }, "YourDatabase")) {
    store.initialize();

    // Define the connection string to OpenAI
    AiConnectionString connectionString = new AiConnectionString();

    // Connection string Name & Identifier
    connectionString.setName("ConnectionStringToOpenAI");
    connectionString.setIdentifier("identifier-to-the-connection-string"); // optional

    // Model type
    connectionString.setModelType(AiModelType.TextEmbeddings);

    // OpenAI connection settings
    OpenAiSettings openAiSettings = new OpenAiSettings();
    openAiSettings.setApiKey("your-api-key");
    openAiSettings.setEndpoint("https://api.openai.com/v1");

    // Name of text embedding model to use
    openAiSettings.setModel("text-embedding-3-small");

    // Optionally, override the default maximum number of query embedding batches
    // that can be processed concurrently
    openAiSettings.setEmbeddingsMaxConcurrentBatches(10);

    connectionString.setOpenAiSettings(openAiSettings);

    // Deploy the connection string to the server
    PutConnectionStringOperation<AiConnectionString> putConnectionStringOp =
        new PutConnectionStringOperation<>(connectionString);
    PutConnectionStringResult putConnectionStringResult =
...
try (IDocumentStore store = new DocumentStore(
    ...
    // Model type
    connectionString.setModelType(AiModelType.Chat);
    ...
    openAiSettings.setModel("gpt-5.1");
    ...
    openAiSettings.setReasoningEffort(OpenAiReasoningEffort.LOW);

    // Optionally, enable or disable prompt prefix caching
    openAiSettings.setEnablePromptCache(true);
...
public final class AiConnectionString extends ConnectionString {
    ...
    // setName(), setIdentifier(), setModelType(), setOpenAiSettings()
}
...
public final class OpenAiSettings extends OpenAiBaseSettings {
    ...
    // Constructors:
    // OpenAiSettings()
    // OpenAiSettings(String apiKey, String endpoint, String model)
    // OpenAiSettings(String apiKey, String endpoint, String model,
    ...
    //                String organizationId, String projectId,
    //                Integer dimensions, Double temperature,
    //                OpenAiReasoningEffort reasoningEffort, Integer seed)

    private String organizationId; // default organization is used if not set
    private String projectId;      // default project is used if not set

    // Relevant only for CHAT models:
    // Controls the amount of internal reasoning performed by supported models.
    // The Java client exposes only MINIMAL, LOW, MEDIUM, and HIGH,
    // sent as 'Minimal', 'Low', 'Medium', and 'High';
    ...
    // For other model names, null omits the setting.
    private OpenAiReasoningEffort reasoningEffort; // optional
...
public abstract class OpenAiBaseSettings extends AbstractAiSettings {
    ...
    private Integer dimensions; // optional

    // Relevant only for CHAT models:
    // Controls the randomness and creativity of the model's output.
    // Higher values (e.g., 1.0 or above) produce more diverse and creative responses.
    // Lower values (e.g., 0.2) result in more focused and deterministic output.
    // If null, the temperature is not sent and the model's default will be used.
    ...
    private Boolean enablePromptCache; // optional
...
public abstract class AbstractAiSettings {
    ...
    private Integer embeddingsMaxConcurrentBatches; // optional
