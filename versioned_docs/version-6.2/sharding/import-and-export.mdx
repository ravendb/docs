...
* In this page:  
  * [Export](#export)  
  * [Import](#import)  
  * [Export and Import Options Summary](#export-and-import-options-summary)  
...
data into a database from either a [.ravendbdump](#export) 
...
[Sharding.Import.CompressionLevel](./configuration.mdx#shardingimportcompressionlevel) configuration key.
...
* If the database is sharded, the imported data will be distributed among the shards.  
  In case the shard has [several nodes](./overview.mdx#shard-replication), 
...
| -------------------- | --------------- | --------------------- |
| Export and Import [.ravendbdump](#export) files using [Smuggler](../client-api/smuggler/what-is-smuggler.mdx) | **Yes** | Smuggler behaves just like it does on non-sharded databases. |
...
| Import from **Backup files** | **Yes** | **Importing** data from a backup file does **not** create a new database like running the [restore process](./backup-and-restore/restore.mdx) over the backup file would, but **adds the data** to the existing database by distributing it among the shards. |
