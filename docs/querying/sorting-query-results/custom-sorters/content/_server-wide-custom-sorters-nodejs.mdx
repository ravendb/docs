...
* A server-wide custom sorter can be used to sort query results in all databases across your cluster.  
  To learn how to define and deploy a database-level custom sorter, see [Database-level custom sorters](../database-level-custom-sorters.mdx).    
...
* In this article:
  * [Add server-wide custom sorter - via the Client API](#add-server-wide-custom-sorter-via-the-client-api)
  * [Add server-wide custom sorter - via Studio](#add-server-wide-custom-sorter-via-studio)
  * [Delete server-wide custom sorter](#delete-server-wide-custom-sorter)
...
2. Click to add a new server-wide custom sorter.
   See [Add a server-wide custom sorter](#add-a-server-wide-custom-sorter) below.
...
In addition to deleting a server-wide custom sorter via [The server-wide custom sorters view](#the-server-wide-custom-sorters-view) in Studio,  
...
See [Test custom sorter](../database-level-custom-sorters.mdx#test-custom-sorter).    
