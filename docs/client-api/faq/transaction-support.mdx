...
In this mode, a user can perform all requested operations (read and/or write) in a single request.  
  ...
  * __Multiple reads & writes__:  
  Performing interleaving reads and writes or conditional execution can be achieved by [running a patching script](../../documents/patching-documents/patch-a-single-document/api-overview.mdx).  
  ...
  If you only need to modify a document in a transaction, [JSON Patch syntax](../../documents/patching-documents/patch-a-single-document/json-patch.mdx) allows you to do that.
