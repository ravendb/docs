...
* **A. Enable**  
  ...
  Configuration key: [Debug.EventListener.Mode](configuration.mdx#debugeventlistenermode)
* **B. Event Types**  
  Select the runtime events to record.  
  At least one type is required. `Select all` takes every type on the list.  
  Configuration key: [Debug.EventListener.EventTypes](configuration.mdx#debugeventlistenereventtypes)
* **C. Minimum Duration**  
  Record only the events that last for at least the duration you set.  
  Configuration key: [Debug.EventListener.MinimumDurationInMs](configuration.mdx#debugeventlistenerminimumdurationinms)
