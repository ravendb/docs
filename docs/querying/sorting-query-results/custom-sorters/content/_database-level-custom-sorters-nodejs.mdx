...
* Server-wide custom sorters are available for use in queries across all databases.  
  To learn how to define and deploy a server-wide custom sorter, see [Server-wide custom sorters](../server-wide-custom-sorters.mdx).  
...
* Custom sorters are available only when using the **Lucene** indexing engine; they are not available with [Corax](../../../../indexes/search-engine/corax.mdx).  
  To learn how to write a custom sorter, see: [How to write a custom sorter](../overview.mdx#how-to-write-a-custom-sorter).      
    
* In this article:
  * [Add database-level custom sorter - via the Client API](#add-database-level-custom-sorter-via-the-client-api)
  * [Add database-level custom sorter - via Studio](#add-database-level-custom-sorter-via-studio)
  * [Delete database-level custom sorter](#delete-database-level-custom-sorter)
  * [Test custom sorter](#test-custom-sorter)
  * [Syntax](#syntax)
...
[Client API](#add-database-level-custom-sorter-via-the-client-api).
...
2. Click to add a new database-level custom sorter.
   See [Add a custom sorter](#add-a-custom-sorter) below.
3. The custom sorters deployed for this **database** are listed here.
4. Click to test this custom sorter.
   Learn more in [Test custom sorter](#test-custom-sorter) below.
...
3. Or, you can enter the code manually in this editor.    
   ...
   Learn more in [How to write a custom sorter](../overview.mdx#how-to-write-a-custom-sorter).   
...
In addition to deleting a custom sorter via [The custom sorters view](#the-custom-sorters-view) in the Studio,  
...
Click the test button from the [Custom sorters view](#the-custom-sorters-view) to open the test panel:
...
4. **Diagnostics tab**  
   ...
   You can also add your own messages to this list - see [Adding your own diagnostic messages](#adding-your-own-diagnostic-messages).
