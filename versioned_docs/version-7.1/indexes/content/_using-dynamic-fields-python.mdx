...
Dynamic fields can be indexed for [Full-text search](../querying/searching.mdx) by setting `FieldIndexing.Search` in `CreateFieldOptions`.
...
* **At indexing time**:      
  ...
  See [Configuring analyzers for dynamic fields](#configuring-analyzers-for-dynamic-fields) below.    
    ...
    * If no analyzer is configured for the field, RavenDB uses the [Default search analyzer](../using-analyzers.mdx#using-the-default-search-analyzer).
...
* **At querying time**:  
  For full-text search to return the expected results, the analyzer used at indexing time and the analyzer used for the query term should produce compatible terms.  
    ...
    RavenDB uses the [Default analyzer](../using-analyzers.mdx#using-the-default-analyzer) to analyze the query term by default.
    ...
    configuration key to `true` to make RavenDB use the [Default search analyzer](../using-analyzers.mdx#using-the-default-search-analyzer) as the fallback analyzer for the query term instead.
...
* **Full-text search query**:
    ...
    the searched term `"north wind"` is analyzed with the [Default search analyzer](../using-analyzers.mdx#using-the-default-search-analyzer).
...
| ------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------- |
| No analyzer is explicitly configured for the generated field name | `false` | The [Default analyzer](../using-analyzers.mdx#using-the-default-analyzer)                |
| No analyzer is explicitly configured for the generated field name | `true`  | The [Default search analyzer](../using-analyzers.mdx#using-the-default-search-analyzer)  |
...
| `ALL_FIELDS` is configured with a specific analyzer,<br/>e.g. `WhitespaceAnalyzer` | `true`  | The [Default search analyzer](../using-analyzers.mdx#using-the-default-search-analyzer) |
...
| **stored**     | `bool`               | Sets [FieldStorage](../storing-data-in-index.mdx) <br/><br/> `false` - will set `FieldStorage.No` (default value) <br/> `true` - will set `FieldStorage.Yes` |
| **analyzed**   | `bool`               | Sets [FieldIndexing](../using-analyzers.mdx) <br/><br/> `null` - `FieldIndexing.Default` (default value) <br/> `false` - `FieldIndexing.Exact` <br/> `true` - `FieldIndexing.Search` |
| **options**    | `CreateFieldOptions` | Dynamic-index-field options                                                                                                                                         |

| CreateFieldOptions |                    |                                                                                   |
|--------------------|--------------------|-----------------------------------------------------------------------------------|
| **Storage**        | `FieldStorage?`    | Learn about [storing data](../storing-data-in-index.mdx) in the index. |
| **Indexing**       | `FieldIndexing?`   | Sets the field indexing behavior.<br/>Learn about [using analyzers](../using-analyzers.mdx) in the index.    |
| **TermVector**     | `FieldTermVector?` | Sets the field term-vector behavior.<br/>Learn about [term vectors](../using-term-vectors.mdx) in the index. |
