...
see_also:
  ...
  - title: "Gen AI Integration Overview"
    ...
    source: "docs"
    path: "AI Integration > Gen AI Integration"
...
Quill puts a conversational AI agent in front of a database you already have. You point it at your data, describe what the agent should be able to answer, and you get a chat interface that answers questions about that data in plain language. It's self-hosted: it runs on your own server, with no ML engineers or cloud dependency to operate it.

Quill doesn't give you a database, neither asks you to build one. You bring the app and its database, and Quill attaches to it, adding the AI chat layer on top. If you already run something on Postgres, SQL Server, or MySQL/MariaDB, Quill is the thing you put in front of it.
...
Quill requires a license. It's a 90-day trial, no card is required. In order to get it, you need to sign up. Sign up and get your license - go to `ravendb.net/quill-beta`, enter your email, and accept the terms-of-service and privacy terms. 

<Image img={require("./assets/quill-guide1.webp")} alt="Quill beta signup form on ravendb.net/quill-beta" />
...
The API key itself is the QUILLDASH-… value, and that's what you'll type when you log into the dashboard. When you pass it to your Quill app, prefix it with *primary/* (primary/QUILLDASH-…), exactly as the email shows. That prefix belongs to the command; it isn't part of the key you log in with. 

| Credential | Looks like | What it's for |
| :---- | :---- | :---- |
| API key | QUILLDASH-… | Logs you into the Quill **dash**board |
| License key | QUILL-… | Activates the appliance against the licensing API |
...
Your signup email ends with a short "What's next", three steps: run the Docker command, connect your database, and turn on the channels where people will chat. That's the rest of this guide. You'll run the appliance, connect your database and build the agent, and put the chat in front of your user.
...
Your domain is the one you chose at signup. Connect to it, and you should be welcomed with the Quill dashboard login page. Log in with your API key, the QUILLDASH-… one.
...
Throughout this guide, we’ll use Postgres database loaded with Northwind, a small sample store dataset, so you'll see names like Categories, Products, and Orders. Read those as stand-ins for whatever your own database holds; you don't need Northwind to follow along.   
</Admonition>
...
Quill needs to read changes from your database as they happen, and your database has to have that turned on. For Postgres, set wal_level = logical and use a login with the REPLICATION attribute. [SQL Server](https://learn.microsoft.com/en-us/sql/relational-databases/track-changes/enable-and-disable-change-data-capture-sql-server?view=sql-server-ver17) and [MySQL](https://dev.mysql.com/doc/refman/8.0/en/binary-log-formats.html) have their own equivalent settings. If the next step can't read your tables, this is usually what's missing.
...
It's your quick "is it up, and is anyone using it" check, conversations and messages tell you whether people are actually talking to the agent, and it's also where Add application starts a new app. 
