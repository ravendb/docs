...
1. A request sent from the client via [SaveChanges()](../../client-api/session/saving-changes.mdx#transaction-mode---cluster-wide) using `TransactionMode.ClusterWide` will generate a [Raft Command](./rachis/consensus-operations.mdx#implementation-details) 
