...
* **Type** - The [index Type](./indexes-overview.mdx#index-types), which can be one of the following:  
    * Map  
    * Map-Reduce  
    * Auto Map  
    * Auto Map-Reduce  
...
* Learn more in [Create new index](./create-map-index.mdx).  
...
* The 'old index' definition can still be referenced as RavenDB keeps a [history of index revisions](./index-history.mdx),  
...
* When an index that is [defined](./create-map-index.mdx#edit-index-view) with [Rolling deployment](../../../indexes/rolling-index-deployment.mdx) is created or modified,  
