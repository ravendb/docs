...
You can use [RQL](../../../client-api/session/querying/what-is-rql.mdx) keywords and 
...
* Operators like `and` and `or` can complement `where` within data-node and edge clauses.  
  ...
  [Multi-Section Search Patterns](../../../indexes/querying/graph/graph-queries-multi-section.mdx#graph-queries-multi-section-search-patterns))  
...
* In this page:  
   * [Using `where` To Filter Results](../../../indexes/querying/graph/graph-queries-filtering.mdx#using--to-filter-results)  
   * [Using `select` To Choose Filtered Results](../../../indexes/querying/graph/graph-queries-filtering.mdx#using--to-choose-filtered-results)  
   * [Using `where` With `or`](../../../indexes/querying/graph/graph-queries-filtering.mdx#using--with-)  
   * [Using `where` With `and`](../../../indexes/querying/graph/graph-queries-filtering.mdx#using--with--1)  
   * [Additional Filtering Examples](../../../indexes/querying/graph/graph-queries-filtering.mdx#additional-filtering-examples)  
</Admonition>
## Filtering

You can filter data within data-node and edge clauses, using [RQL](../../../client-api/session/querying/what-is-rql.mdx).  
...
* **Query Flow**:  
   * The edge clause uses `where` to search each order's 
     [Lines construct](../../../indexes/querying/graph/graph-queries-the-search-pattern.mdx#complex-edges).  
...
<Admonition type="info" title="You **cannot** use `select` within a data-node clause.  " id="you-cannot-use-select-within-a-data-node-clause" href="#you-cannot-use-select-within-a-data-node-clause">
![SELECT cannot be used in Node Clauses](./assets/Filtering_NoSelectInDataNode.png)
</Admonition>

<Admonition type="note" title="Note">
Note: Do not confuse the current usage of `select` with the implementation 
of the same keyword to [project](../../../indexes/querying/graph/graph-queries-overview.mdx#projecting-graph-results) data.  
</Admonition>
#### Using `where` With `or`
...
![Retrieved using OR](./assets/Filtering_Operators_1_GraphicalView.png)
...
![Retrieved using AND: Graphical View](./assets/Filtering_Operators_2_GraphicalView.png)

![Retrieved using AND: Textual View](./assets/Filtering_Operators_2_TextualView.png)
...
`}
    ...
    ![Locate Negotiable Products: Graphical View](./assets/Filtering_Operators_3_GraphicalView.png)

    ![Locate Negotiable Products: Textual View](./assets/Filtering_Operators_3_TextualView.png)
...
`}
     ...
     ![Filtering Nodes](./assets/Filtering_NarrowingResults1.png)
...
![Filtering Nodes](./assets/Filtering_NarrowingResults2.png)
