...
6. **Allow model queries**  
   ...
   <Admonition type="note" title="">
   When disabled, the LLM will not be able to trigger this tool - but if the tool is marked as an [initial context query](../../../ai-integration/ai-agents/ai-agents_overview#initial-context-queries) the agent will still be able to execute it when it is started.  
   </Admonition>

7. **Add to initial context**  
   Enable to mark the query tool as an [initial context query](../../../ai-integration/ai-agents/ai-agents_overview#initial-context-queries).  
...
9. **Sample parameters object** and **Parameters JSON schema**  
   Set a sample object or a JSON schema for [LLM parameters](../../../ai-integration/ai-agents/ai-agents_overview#llm-parameters).  
   Parameters defined here can be used in the tool's RQL query, and the LLM can fill them with values when it invokes the query tool.  
...
when a chat is started. To minimize the size of these messages, you can set the agent to summarize them before sending the history to the LLM.  
