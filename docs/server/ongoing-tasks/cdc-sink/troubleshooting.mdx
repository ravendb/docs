...
* **Persistent connection failure**  
  The source has been unreachable through multiple retry cycles.
  The first fallback delay is 5 seconds; subsequent retry delays roughly double up to the `CdcSink.MaxFallbackTimeInSec` base cap (default 15 minutes), with small jitter.
