...
* Some querying features are not yet implemented.
  Others, such as [filter](#filter), behave a little differently in a sharded database.
  These cases are described below.  

* In this article:  
  * [Querying a sharded database](#querying-a-sharded-database)
  * [Querying selected shards](#querying-selected-shards)
  * [Including items in a query](#including-items-in-a-query)
  * [Paging query results](#paging-query-results)
  * [Streaming query results](#streaming-query-results)    
  * [Filtering query results](#filtering-query-results)
      * [`where`](#where)
      * [`filter`](#filter)
      * [`where` vs `filter` recommendations](#wherevsfilterrecommendations)
  * [Loading a document within a projection](#loading-a-document-within-a-projection)  
  * [`order by` and `limit` in a Map-Reduce query](#order-by-and-limit-in-a-map-reduce-query)  
  * [Timing queries](#timing-queries)
  * [Unsupported querying features](#unsupported-querying-features)  
...
* To allow this, the database performs the following steps when a client sends a query to a sharded database:  
    * The query is received by a RavenDB server that was appointed as an [Orchestrator](./overview.mdx#client-server-communication).  
    ...
    * The orchestrator combines the data it receives from all shards into a single dataset and may perform additional operations on it.
      For example, when querying a [Map-Reduce index](./indexing.mdx#map-reduce-indexes-in-a-sharded-database), each shard returns results that were already reduced locally.
...
* The client remains unaware that it communicated with a sharded database.    
  ...
  Learn more in [When should sharding be used](./overview.mdx#when-should-sharding-be-used).   
...
* A query is normally executed over all shards. However, you can also query only selected shards.  
  ...
  This can be useful, for example, when documents are intentionally stored on the same shard using [Anchoring documents](./administration/anchoring-documents.mdx).

* **You can query specific shards in either of the following ways**:
  * Using a pre-defined sharding prefix, as explained in: [Querying selected shards by prefix](./administration/sharding-by-prefix.mdx#querying-selected-shards-by-prefix).
...
* To query specific shards using a document ID, use method `ShardContext` together with `ByDocumentId` or `ByDocumentIds`.
  ...
  Learn about the hashing method and bucket population in [How documents are distributed among shards](./overview.mdx#how-documents-are-distributed-among-shards).  
...
* Although includes are supported in regular sharded queries,  
  ...
  Learn more in [Streaming query results](#streaming-query-results).
...
### Limitations when streaming query results in a sharded database:
  ...
  * When streaming **Map-Reduce** results in a sharded database, `order by` is **supported only on the _reduce-key_ fields**.  
    ...
    <TabItem value="Index_OrdersByCompany" label="Index_OrdersByCompany">
    ```csharp
...
Data can be filtered using the [where](#where) and [filter](#filter) keywords on both non-sharded and sharded databases.  
...
[where](../querying/filtering-query-results/filter-query-results.mdx) is RavenDB's basic filtering command.  
...
* **SHARDED database**:  
  ...
  the filtering is applied **per-shard**, on each shard's local data.
      ...
      * To address this, use the [filter](#filter) keyword instead,  
      ...
      * Note: using `where` does **not** cause this problem when filtering on a `GroupBy` field (the reduce-key),  
        ...
        Learn more in [`where` vs `filter` recommendations](#wherevsfilterrecommendations) below.
...
* **Set a [limit](../indexes/querying/exploration-queries.mdx#usage-examples) on `filter` when possible** to bound how much data the orchestrator scans.    
...
Similar to its behavior under a non-sharded database, [OrderBy](../querying/sorting-query-results/sort-query-results.mdx) is used in an index query or a dynamic query to sort the retrieved dataset by a given order.  
        Reduce = results => from result in results
...
* **Loading a document within a Map-Reduce projection**  
  Read more about this topic in [Loading a document within a projection](#loading-a-document-within-a-projection) above.      
    
* **Ordering streamed Map-Reduce results by _non-reduce-key_ fields**  
  Read more about this topic in [Streaming query results](#streaming-query-results) above.
...
* **Order by score**   
  [OrderByScore](../querying/sorting-query-results/sort-query-results.mdx#order-by-score) is not supported in a sharded database.     
