...
* **Mitigation: Agent parameters**  
  You can use [agent parameters](./overview.mdx#query-parameters) to limit the scope of the defined query and the dataset subsequently transferred to the AI model.  
...
When an agent is composed with [sub-agents](./multi-agents.mdx), two specific risks arise in the data flow:
...
See [Multi-agents](./multi-agents.mdx) for the full picture, including parameter propagation and conversation-document isolation.  
