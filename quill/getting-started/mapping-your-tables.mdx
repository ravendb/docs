...
This page starts where [Connecting your database](../getting-started/connecting-your-database.mdx#verifying-your-schema)
left off: you are at the **How would you like to map your schema?** stage of the **Add new app** wizard.

</ContentFrame>

After verifying your schema, the **Add new app** wizard lets you choose the
...
3. **Tables**  
   ...
   * **D. A mapped table**  
     ...
     * Click a row to show its mapping in the configuration pane.  
       A row that carries attached tables has an arrow at its left, to fold or reveal these tables.
     * In the mapping above, `categories` is mapped to `Categories`.  
       `orders` is mapped to `Orders`, and carries two attached tables.
...
The load runs in the background, so you do not have to wait up; Quill reports its progress while it runs.  
Each document Quill writes is reported as a **message**. Quill writes the messages in batches, and reports each
