...
You can define either or both of the following:  
* A schema associated with the collection whose documents you want to audit.  
  Learn [here](../../../documents/schema-validation/write-validation/write-validation_api#creating-a-validation-schema) how to define a collection schema.  
* A schema associated with the index itself.  
...
* If you define both a collection schema and an index schema, the index schema takes precedence over the collection schema.  
</Admonition>

Creating a schema as part of the index definition:  
```csharp
// Define a validation Schema as a string
// `Customer` must be a string
// `Total` must be a number >= 0
