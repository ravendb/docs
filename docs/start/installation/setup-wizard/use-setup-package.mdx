...
* When you have already configured the first node (e.g., Node **A**) of a multi-node cluster.   
  You will use the generated ZIP package to set up and start the remaining machines (Node **B**, Node **C**, etc.).
    
* When a configuration package was created through the 
  [Create a package for external setup](../../../start/installation/setup-wizard/choose-setup-method.mdx#create-a-package-for-external-setup) flow  
  and you are now ready to apply that configuration to your target server.
