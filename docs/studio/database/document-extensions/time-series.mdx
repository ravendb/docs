...
description: "View, query, and graph RavenDB time series data attached to documents using Studio with interactive charting and rollup display."
sidebar_position: 1
---
...
* An **Incremental Time Series** is a special type of time series that allows you to 
  ...
  and learn how to define them through Studio [below](#incremental-time-series).  

* In this page:
  * [Document View](#document-view)
  * [Time Series View](#time-series-view)
     * [Create new Time Series (by Creating the First Entry)](#create-new-time-series-by-creating-its-first-entry)
     * [Editing an Entry](#editing-an-entry)
     * [Deleting a Range of Entries](#deleting-a-range-of-entries)
  * [Querying Time Series](#querying-time-series)
     * [Results in Graph View](#results-in-graph-view)
  * [Incremental Time Series](#incremental-time-series)
     * [Creating a new Incremental Time Series (by Creating its First Entry)](#creating-a-new-incremental-time-series-by-creating-its-first-entry)
     * [Editing an Incremental Time Series Entry](#editing-an-incremental-time-series-entry)
...
1. To view a document's time series, open its [document view](../documents/document-view.mdx) 
   and click the time series tab on the right.  
2. Click 'Add Time Series' to create a new time series [see more below](#create-new-time-series-by-creating-its-first-entry).  
...
2. Click to add a new entry to this time series (StockPrices), or click the dropdown to create a [new time series](#create-new-time-series-by-creating-the-first-entry).  
...
1. **Time Series Type**  
    * **Diasble** to create a **non-incremental time series**.  
      (Learn now to create an incremental time series [here](#incremental-time-series))  
2. **Time Series Name**  
...
5. **Values**  
    ...
    * Time series entry values can be given meaningful names rather than labels like Value #0 and Value #1.  
      To set entry values' names, use Studio's [Time Series Settings View](../settings/time-series-settings.mdx#add-or-edit-time-series-configuration).  
