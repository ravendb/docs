...
* In most cases, sharding is implemented to allow the efficient usage and management of exceptionally  
  large databases (e.g., a 10-terabyte DB).  

* Sharding is managed by the RavenDB server. 
  No special adaptation is required from clients when accessing a sharding-capable server or a sharded database.  
    * The client API remains **unchanged** when using a sharded database.  
      Clients of RavenDB versions older than 6.0 (that provided no sharding support) can seamlessly connect to a sharded database
      without any adaptations or even knowing that the database they connect is sharded.  
...
* Each [Shard](../sharding/overview#shards) hosts and manages a **unique subset** of the database content.  
  Documents are sorted among shards based on their document ID.  
...
As the volume of stored data grows, the database can be scaled out by splitting it into [shards](../sharding/overview#shards), 
allowing it to be handled by multiple nodes and presenting practically no limit to its growth.  
The size of the overall database, comprised of all shards, can reach in this fashion dozens of terabytes and more,  
while keeping the resources of each shard in check and maintaining its high performance and throughput.  
...
However, the additional communication between the client and the orchestrator, and between the orchestrator and the shards,
...
We can postpone the transit to a sharded database when, for example, the database size is 100 GB,  
...
* RavenDB 6.0 and above can **migrate** its database to a sharded database via [external replication](../server/ongoing-tasks/external-replication)  
...
* You cannot, however, upgrade a non-sharded database into a sharded one.  
  To upgrade RavenDB to 6.0 and migrate the database data you will need to upgrade the server,  
  create a new, sharded database, and replicate or export the data into it.  
...
Take a 3-shards database, in which shard **1** is populated with documents `Users/1`..`Users/2000`,  
...
However, shard-specific operations are available:  
...
Similarly to non-sharded databases, shards can be **replicated** across cluster nodes to ensure the continuous availability
...
The number of buckets allocated for the entire database is fixed, always remaining **1,048,576** (1024 times 1024).  
...
A hash algorithm is applied to each document ID, producing a hash code that is a number between **0** and **1,048,576**.  
This hash code determines the bucket number in which the document is stored.  

As buckets are distributed among different shards, the bucket number allocated for a document also determines which shard the document will reside on.
...
* **Anchoring documents to a bucket**:  
  You can make documents share a bucket (and therefore a shard) by adding a suffix to their ID.  
...
* **Anchoring documents to a shard**:   
  You can specify on which shard a document will reside based on the document ID prefix.  
