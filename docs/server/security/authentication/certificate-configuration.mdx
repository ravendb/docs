...
description: "Configure X.509 certificates for RavenDB server authentication including certificate paths, trust chains, and renewal settings."
sidebar_position: 0
---
...
Please also take a look at the automated [Setup Wizard](../../../start/installation/setup-wizard/overview.mdx) which lets you set up 
 ...
 * Or the Wizard can give you a free, highly secure **certificate via [Let's Encrypt](./lets-encrypt-certificates.mdx)**.  
...
* In this page:
   ...
   * [Standard Manual Setup With Certificate Stored Locally](#standard-manual-setup-with-certificate-stored-locally)  
      * [Certificate Requirements](#certificate-requirements)  
      * [Certificate Location](#certificate-location)  
   * [With Logic Foreign to RavenDB or External Certificate Storage](#with-logic-foreign-to-ravendb-or-external-certificate-storage)  
   * [Step-by-step Guide to Installing Certificate](#step-by-step-guide-to-installing-certificate)  
...
To enable authentication, either `Security.Certificate.Path` or `Security.Certificate.Load.Exec` must be set in [settings.json](../../configuration/configuration-options.mdx#json). 
 Please note that `Security.Certificate.Load.Exec` has replaced the old `Security.Certificate.Exec` as of 4.2 - [see FAQ](../common-errors-and-faq.mdx#automatic-cluster-certificate-renewal-following-migration-to-42).  
...
The first action an administrator will do is to [generate/register a new client certificate](./client-certificate-usage.mdx).  
Find detailed instructions of the process [below](#step-by-step-guide-to-installing-certificate).  

You can set up various client certificates with different security clearance levels and database permissions. 
See [Certificate Management](./certificate-management.mdx) for more about permissions.  
...
* Certificates created during setup using [Let's Encrypt](./lets-encrypt-certificates.mdx) 
...
* **Setup.Mode**  
 Set to "None" if you want a manual setup. If you want to use the [Setup Wizard](../../../start/installation/setup-wizard/overview.mdx), set to "Initial" 
...
2. In each node `Server` folder, create the `settings.json` file which you will configure like the [examples](#standard-manual-setup-with-certificate-stored-locally) above.  
...
3. Configure the `settings.json` file in each node `Server` folder.  
   ...
   - Set the `Security.Certificate.Path` to the **.pfx** that you placed in each server folder if certificate is stored with [RavenDB logic on local machines](#standard-manual-setup-with-certificate-stored-locally) 
     or `Security.Certificate.Load.Exec` if using [external location or logic](#with-logic-foreign-to-ravendb-or-external-certificate-storage).  
   - Make sure that the certificate .path or .load script lead to the correct certificate location. `.Path` should look something like this:  
         ...
         See [.json example for a .Path situation](#standard-manual-setup-with-certificate-stored-locally) above.  
   - Setting a **password** on the certificate is optional. See [settings.json example](#standard-manual-setup-with-certificate-stored-locally) above.  
   Run
...
5. The PowerShell CLI window will be running the server terminal. The last line should read `ravendb>`. 
  In the CLI, run the [generateClientCert](../../administration/cli.mdx#generateclientcert) command to generate a client certificate.  
...
6. Extract the contents of the .zip file generated into the folders where your nodes live.  
   - Install the client certificate into the OS by double-clicking the `admin.client.certificate...pfx` file and complete the OS Certificate Import Wizard.  
     ...
     * **To set a password on the client certificate**, do so in the Import Wizard.  You'll need to use that password every time you [work with the certificate](./certificate-management.mdx).  
...
9. Now the service should run whenever the machine starts and the Studio should be accessible by the user with the client certificate.  
   - See [Certificate Management](./certificate-management.mdx) for an easy way to generate various client certificates with customizable permissions.  
