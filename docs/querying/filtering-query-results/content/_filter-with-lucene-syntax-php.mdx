...
* In the client API, use `whereLucene` from a document query.  
...
$companiesWithSingleCharacterWildcard = $session->advanced()
    ->documentQuery(Company::class)
...
In PHP, wrap the clause in single quotes (e.g. `'"sales representative"'`),  
...
$employeesProx = $session->advanced()
    ->documentQuery(Employee::class)
...
```php
class Companies_ByNameAndPhone extends AbstractIndexCreationTask
{
    public function __construct()
    {
        ...
        $this->map = "from company in docs.Companies " .
                     "select new { company.Name, company.Phone }";

        $this->index("Name", FieldIndexing::search());
        $this->index("Phone", FieldIndexing::search());
    }
}
```
<br/>        
...
```php
$companies = $session->advanced()
    ->documentQuery(Company::class, "Companies/ByNameAndPhone")
...
To keep query terms unchanged, pass `true` for the `exact` parameter in the client API, or wrap `lucene()` with `exact(...)` in RQL.
...
```php
class Companies_ByNameExact extends AbstractIndexCreationTask
{
    public function __construct()
    {
        parent::__construct();
        ...
        $this->index("Name", FieldIndexing::exact());
    }
}
```
<br/>    
...
$companies = $session->advanced()
    ->documentQuery(Company::class, "Companies/ByNameExact")
    ->whereLucene("Name", "Bistro*", true) // exact: true
    ->toList();
```
</TabItem>
<TabItem value="RQL" label="RQL">
...
Indexing a field with `FieldIndexing::exact()` uses the [default exact analyzer](../../../indexes/using-analyzers.mdx#using-the-default-exact-analyzer).  
...
Use `andAlso()`, `orElse()`, and `not()` to compose filters outside the Lucene clause:    
...
<TabItem value="Query" label="Query">
```php
// The PHP string "\\(" becomes \( in the Lucene clause
$companies = $session->advanced()
    ->documentQuery(Company::class)
...
* **Document query only**  
  In the client API, `whereLucene` is available on document queries
  (`session->query()` / `session->advanced()->documentQuery()`). In raw RQL, use `lucene()`.    
