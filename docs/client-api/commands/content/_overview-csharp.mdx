...
* RavenDB's Client API is structured in layers.  
  At the highest layer, you interact with the [document store](../../what-is-a-document-store.mdx) and the [document session](../../session/what-is-a-session-and-how-does-it-work.mdx),  
...
* Beneath this high-level interface are Operations and Commands:
    ...
    * **Operations**:
        ...
        * Learn more about Operations in [what are Operations](../../operations/what-are-operations.mdx).
    ...
    * **Commands**:
        ...
        * All commands can be executed using either the [Store's _Request Executor_](#execute-command---using-the-store-request-executor)
          or the [Session's _Request Executor_](#execute-command---using-the-session-request-executor),  
...
* In this page:
    * [Execute command - using the Store Request Executor](#execute-command---using-the-store-request-executor)
    * [Execute command - using the Session Request Executor](#execute-command---using-the-session-request-executor)
...
(For examples of creating a subscription using higher-level methods, see [subscription creation examples](../../data-subscriptions/creation/examples.mdx)).
...
(For loading a document using higher-level methods, see [loading entities](../../session/loading-entities.mdx)).
...
* Note that the transaction created for the HTTP request when executing the command  
  is separate from the transaction initiated by the session's [SaveChanges](../../session/saving-changes.mdx) method,  
  even if both are called within the same code block.

* Learn more about transactions in RavenDB in [Transaction support](../../faq/transaction-support.mdx).
...
* **The following low-level commands, which inherit from `RavenCommand`, are available**:
    ...
    * [DeleteDocumentCommand](../documents/delete.mdx)  
    ...
    * [GetDocumentsCommand](../documents/get.mdx)  
    ...
    * [PutDocumentCommand](../documents/put.mdx)  
    * PutSecretKeyCommand  
    * QueryCommand  
    * QueryStreamCommand  
    * SeedIdentityForCommand
    * [SingleNodeBatchCommand](../batches/how-to-send-multiple-commands-using-a-batch.mdx)  
