...
* Covered so far in Getting Started:
   ...
   * [Install and activate](install-and-activate.mdx) - After this step, Quill is running on your machine, and its
...
* Quill reads `wal_level` when it lists your tables, in the
  [Verify your schema](connect-your-database.mdx#verify-your-schema) step, and reports an error unless the value
  is `logical`.
...
* While the Agent service is stopped, the capture tables stay empty. Quill connects and lists your tables as
  ...
  The warning appears in the [Verify your schema](connect-your-database.mdx#verify-your-schema) step below,
...
[Verify your schema](connect-your-database.mdx#verify-your-schema) step, and reports an error for any of the three
...
step, where Quill reads from the tables you select.  
...
Start **Quill's management dashboard** (as explained in the previous step, [Install and activate](install-and-activate.mdx)).  
...
The wizard opens on its first step:
...
9. **Next**  
   Click for the next step, **Verify your schema**.
...
The wizard's second step lists all the tables Quill discovered in your source database, and allows you to select
the tables that the app will work with.  
Only the tables you select are carried into the mapping and into the app's configuration; unselected tables, and
relationships that point to them, are ignored.
...
7. **The selection bar**  
   ...
   * Click **Verify schema** to check that CDC can run on the selected tables.
     ...
     * Quill removes the objects it created, leaving the source database as it found it.
     ...
     A passing verification shows as:  
     ![Schema verified](./assets/connect-your-database_schema-verified.png)

     A failing verification reports:  
     ![Replication privilege error](./assets/connect-your-database_replication-privilege-error.png)

     * **A. The error**: explains what blocked the verification, and carries a statement that can be executed by a
       database administrator to fix it.
     * **B. The table list**: stays empty until the verification passes.

     </Admonition>

8. **Next**  
   Click to advance to the mapping steps, covered in [Map your tables](map-your-tables.mdx).  
   Clicking **Next** runs the same verification as **Verify schema** above, ensuring that
   [CDC](connect-your-database.mdx#prerequisites) can run on the selected tables before advancing to the next step.
