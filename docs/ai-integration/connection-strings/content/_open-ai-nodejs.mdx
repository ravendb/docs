...
3. **Reasoning effort** (optional)  
   ...
   * The Node.js Client API's `OpenAiReasoningEffort` type suggests the listed values but also accepts any string.
...
```js
// OpenAiReasoningEffort accepts any string.
// Suggested values: "none" | "minimal" | "low" | "medium" | "high" | "xhigh" | "max"
...
class OpenAiSettings
{
    ...
    // Suggested values are 'none', 'minimal', 'low', 'medium', 'high', 'xhigh', and 'max',
    // but custom strings are also accepted.
    // RavenDB converts the exact legacy values 'Minimal', 'Low', 'Medium', and 'High' to lowercase.
