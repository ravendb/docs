...
* In this article:  
  * [Why use ETL](#why-use-etl)  
  * [Defining ETL Tasks](#defining-etl-tasks)  
...
* The following ETL tasks can be defined:  
  * [RavenDB ETL](./raven.mdx) - send data to another _RavenDB database_  
  * [SQL ETL](./sql.mdx) - send data to an _SQL database_  
  * [Snowflake ETL](./snowflake.mdx) - send data to a _Snowflake warehouse_  
  * [OLAP ETL](./olap.mdx) - send data to an _OLAP destination_  
  * [Elasticsearch ETL](./elasticsearch.mdx) - send data to an _Elasticsearch destination_  
  * [Kafka ETL](./queue-etl/kafka.mdx) - send data to a _Kafka message broker_  
  * [RabbitMQ ETL](./queue-etl/rabbit-mq.mdx) - send data to an _RabbitMQ exchange_  
  * [Azure Queue Storage ETL](./queue-etl/azure-queue.mdx) - send data to an _Azure Queue Storage message queue_  
  * [Amazon SQS ETL](./queue-etl/amazon-sqs.mdx) - send data to an _Amazon SQS message queue_  
...
* [Extract](#extract) - Extract the documents from the database  
* [Transform](#transform) - Transform & filter the documents data according to the supplied script (optional)  
* [Load](#load) - Load (write) the transformed data into the target destination  
...
See the [Predefined JavaScript functions](../../kb/javascript-engine.mdx#predefined-javascript-functions) article for the complete reference.
...
|-----------------------------------------------|----------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `loadTo`                                      | function | Loads an object to the specified target. An object will only be sent to the destination if this method is called.<br/>This command has several syntax options,<br/>see [The loadTo method](#theloadtomethod) below. |
| **Attachments:**                              |          |                                                                                                                                                                                                             |
...
| `<doc>.addAttachment([name,] attachmentRef)`  | function | Adds an attachment to a transformed document (`<doc>`) that will be sent to the target. <br/> For RavenDB ETL specifics, see this [section](./raven.mdx#attachments).       |
...
* [`ETL.ExtractAndTransformTimeoutInSec`](../../configuration/etl-configuration.mdx#etlextractandtransformtimeoutinsec) (default: 30 sec)  
  Time-frame for the extraction and transformation stages (in seconds), after which the loading stage will start.

* [`ETL.MaxNumberOfExtractedDocuments`](../../configuration/etl-configuration.mdx#etlmaxnumberofextracteddocuments) (default: 8192)  
  Maximum number of extracted documents in an ETL batch.

* [`ETL.MaxNumberOfExtractedItems`](../../configuration/etl-configuration.mdx#etlmaxnumberofextracteditems) (default: 8192)  
  Maximum number of extracted items (documents, counters) in an ETL batch.

* [`ETL.MaxBatchSizeInMb`](../../configuration/etl-configuration.mdx#etlmaxbatchsizeinmb) (default: 64 MB)  
...
* In contrast to [Replication](../../clustering/replication/replication-overview.mdx), 
...
ETL errors and warnings are [logged to files](../../troubleshooting/logging.mdx) and displayed in the notification center panel.  
...
but it never crosses [`ETL.MaxFallbackTimeInSec`](../../configuration/etl-configuration.mdx#etlmaxfallbacktimeinsec) configuration (default: 900 sec).  
