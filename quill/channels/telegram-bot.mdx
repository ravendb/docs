...
* **The agent remembers a conversation for one day at most.**  
  ...
  can also make the agent forget sooner by sending `/clear`.
...
* **The bot does not reply to a photo, a document, a sticker, a voice message, or a contact card.**  
  Likely cause: the bot answers text-only messages; a contact card is accepted only when a parameter is bound to
  **Telegram phone number**.  
