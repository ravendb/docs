...
The delay in index procession increases the size of Item [batches](../server/configuration/indexing-configuration.mdx#indexing.mapbatchsize) 
...
### Server-Wide Index Throttling
 ...
 Index Throttling can be set server-wide using a designated [configuration option](../server/configuration/configuration-options.mdx#settings.json).  
...
`}
    ...
    the [Indexing.MapBatchSize](../server/configuration/indexing-configuration.mdx#indexing.mapbatchsize) property.  
