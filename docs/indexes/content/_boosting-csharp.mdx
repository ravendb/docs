...
* **Corax scoring for `exists()` and `where true`**:
  * In RavenDB 7.2.6 and later, when Corax calculates a score for a query, an `exists()` condition (`WhereExists()` in a `DocumentQuery`) 
  ...
  * For example, `.WhereExists(x => x.Fax).Boost(3)` (`boost(exists(Fax), 3)` in RQL) contributes `3` before any index-entry boost is applied.  
