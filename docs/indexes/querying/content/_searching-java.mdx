...
<TabItem>
```java
public static class Employees_ByNotes extends AbstractIndexCreationTask {
    ...
    public Employees_ByNotes() {
        ...
        index("employeeNotes", FieldIndexing.SEARCH);
        ...
        analyze("employeeNotes", "WhitespaceAnalyzer");
...
List<Employee> employees = session
     // Query the index
    .query(Employees_ByNotes.IndexEntry.class, Employees_ByNotes.class)
     // Call 'search':
...
<TabItem>
```java
public static class Employees_ByEmployeeData extends AbstractIndexCreationTask {
    ...
    public Employees_ByEmployeeData() {
        ...
        index("employeeData", FieldIndexing.SEARCH);
...
List<Employee> employees = session
    ...
    .query(Employees_ByEmployeeData.IndexEntry.class, Employees_ByEmployeeData.class)
    .openSubclause()
...
```java
public static class Employees_ByNotes_usingDefaultAnalyzer extends AbstractIndexCreationTask {
...
<TabItem value="Query" label="Query">
```java
Reference<Explanations> explanationsRef = new Reference<>();

List<Employee> employees = session
    .query(Employees_ByNotes_usingDefaultAnalyzer.IndexEntry.class,
    ...
    .includeExplanations(explanationsRef)
...
String employeeId = session.advanced().getDocumentId(employees.get(0));
String explanation = explanationsRef.value.getExplanations(employeeId)[0];

assert explanation.contains("employeeNotes:*rench");
```
</TabItem>
<TabItem value="RQL" label="RQL">
```sql
...
<TabItem value="Index" label="Index">
```java
public static class Employees_ByNotes_usingCustomAnalyzer extends AbstractIndexCreationTask {
    ...
    public Employees_ByNotes_usingCustomAnalyzer() {
        ...
        analyze("employeeNotes", "RemoveWildcardsAnalyzer");
    }
}
```
</TabItem>
...
// From the Java client you pass its source code to the server as a string.

String removeWildcardsAnalyzer =
    "using System.IO; " +
...
List<Employee> employees = session
    .query(Employees_ByNotes_usingCustomAnalyzer.IndexEntry.class,
...
assert explanation.contains("employeeNotes:french");
assert !explanation.contains("employeeNotes:*french");
```
</TabItem>
<TabItem value="RQL" label="RQL">
...
<TabItem value="Index" label="Index">
```java
public static class Employees_ByFirstName_usingExactAnalyzer extends AbstractIndexCreationTask {
...
List<Employee> employees = session
    .query(Employees_ByFirstName_usingExactAnalyzer.IndexEntry.class,
...
assert explanation.contains("firstName:Mich*");
```
</TabItem>
<TabItem value="RQL" label="RQL">
```sql
