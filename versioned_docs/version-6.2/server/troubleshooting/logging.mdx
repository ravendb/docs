...
The logging to files can be setup by [logs configuration](../configuration/logs-configuration.mdx) defined in the [settings.json](../configuration/configuration-options.mdx#json) file. By default, the logging
...
Modifying the [settings.json](../configuration/configuration-options.mdx#json) file requires the server restart. In order to avoid that you can modify the current log level by using the [CLI](../administration/cli.mdx). 
...
You can read more about the command [here](../administration/cli.mdx#log).
