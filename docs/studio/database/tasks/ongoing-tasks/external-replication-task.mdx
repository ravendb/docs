...
description: "Set up external database replication in RavenDB Studio with connection string configuration, filtering, and delay options."
sidebar_position: 1
---
...
* This ongoing task replicates **one-way**, from the source to the destination. For additional functionality,  
  such as filtration and two-way (master-master) replication, consider [Hub/Sink Replication](../../../../studio/database/tasks/ongoing-tasks/hub-sink-replication/overview.mdx).  

* To replicate between two separate, secure RavenDB servers,  
  you need to [pass a client certificate](../../../../studio/database/tasks/ongoing-tasks/external-replication-task.mdx#step-by-step-guide) from the source server to the destination.

* External replication can be a comfortable means of data migration into a [sharded](../../../../sharding/overview.mdx) database.  
  You can read more about this option in the sharding documentation [external replication](../../../../sharding/external-replication.mdx) and [migration](../../../../sharding/migration.mdx) sections.  
...
* In this page:  
  * [General Information about External Replication Task](../../../../studio/database/tasks/ongoing-tasks/external-replication-task.mdx#general-information-about-external-replication-task)
  * [Step-by-Step Guide](../../../../studio/database/tasks/ongoing-tasks/external-replication-task.mdx#step-by-step-guide)
  * [Definition](../../../../studio/database/tasks/ongoing-tasks/external-replication-task.mdx#definition)  
  * [Details in Tasks List View](../../../../studio/database/tasks/ongoing-tasks/external-replication-task.mdx#details-in-tasks-list-view)  
...
* Ongoing tasks
   ...
   * [Backup](../../../../backup/create/periodic-tasks/database-backup#creating-and-managing-periodic-backups-via-studio)
...
1. **Pass Certificate from Source Server to Destination Server**  
  ...
  * **Via RavenDB Studio:**  
    Navigate from the "Manage Server" tab (left side) &gt; "Certificates" to open the [Certificate Management](../../../../server/security/authentication/certificate-management.mdx) view.  
  ...
  * **Via API:**  
     ...
     * To generate and configure a client certificate from the source server:  
       **Via code**, see [CreateClientCertificateOperation](../../../../client-api/operations/server-wide/certificates/create-client-certificate.mdx).  
       **Via RavenDB CLI** PowerShell command, see [Client Certificate Usage](../../../../server/security/authentication/client-certificate-usage.mdx)  
...
2. **Create Target Database in Destination Server**  
  * Consider [creating an empty target database](../../../../studio/database/create-new-database/general-flow.mdx) 
    because data transfer can overwrite existing data.  
  * If the source database is [encrypted](../../../../studio/database/create-new-database/encrypted.mdx#creating-encrypted-database), 
...
3. **Define External Replication Task in the Source Database**  
    Learn more about [defining the task](../../../../studio/database/tasks/ongoing-tasks/external-replication-task.mdx#definition) in the dedicated section.  
  * **Optional Parameters**  
     - Task Name  
     - Delay Time  
     - Preferred Node  
...
3. **Set Replication Delay Time** (Optional)  
   ...
   * Having a delayed instance of a database allows you to "go back in time" and undo contamination to your data due to an attack, faulty patch script, or other human errors.  
     * This doesn't replace the need to [safely backup your databases](../../../../backup/create/periodic-tasks/database-backup#creating-and-managing-periodic-backups-via-studio), but it does provide a way to stay online while repairing.  
...
5. **Create a new RavenDB connection string**  
  ...
  * The connection string defines the external database and its server URL to replicate to.  
    ...
    See [Offline Behavior - When the destination node is down](../../../../studio/database/tasks/ongoing-tasks/external-replication-task.mdx#offline-behavior).  
