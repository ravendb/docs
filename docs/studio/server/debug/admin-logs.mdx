...
2. <a id="set-displayed-logs-minimum-level"/>**Min level**  
   The minimum [logging level](../../../monitoring/logs/overview.mdx#available-logging-levels) from which log entries are displayed on this view.  
...
5. **Settings**  
   ...
   Only entries that match both the selected min level (see no. 2 above) and the filter's conditions will be written to the logs. Existing entries remain unaffected.  
      ...
      * <a id="define-logs-on-disk-minimum-level"/>**A.** **Current Minimum Level**  
        The minimum [logging level](../../../monitoring/logs/overview.mdx#available-logging-levels) from which log entries will be stored in log files.  
  ...
  4. **Log entry**  
        * Each log entry follows this pipe-separated (|) pattern:  
          ...
          All fields are always present (empty if not applicable).   
            ...
            * **Level** - The entry's [logging level](../../../monitoring/logs/overview.mdx#available-logging-levels).
            ...
            * **Logger** - The full class name of the originating [logger](../../../monitoring/logs/overview.mdx#your-own-loggers).
