...
Each query in RavenDB must be expressed by [RQL](../../../querying/rql/what-is-rql.mdx), our query language. Each query must match an index in order to return the results. The full query flow is as follows:
...
3. `select`
  - From each record, the server extracts the appropriate fields. It always extracts the `id()` field ([stored](../../storing-data-in-index.mdx) by default).   
  ...
  - If a query indicates that [projection](../projections.mdx) should be used, then all results that were not filtered out are processed by that projection. Fields defined in the projection are extracted from the index (if stored).
...
You can read more about filtering [here](../../../querying/filtering-query-results/filter-query-results.mdx).
...
You can read more about paging [here](../paging.mdx).

### Example IV - Querying a Specified Index

In the above examples, we **did not** specify an index that we want to query. RavenDB will try to locate an appropriate index or create a new one. You can read more about creating indexes [here](../../creating-and-deploying.mdx).
...
You can check the API reference for the `DocumentQuery` [here](../../../querying/document-query/what-is-document-query.mdx).
</Admonition>
