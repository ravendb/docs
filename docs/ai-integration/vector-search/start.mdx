...
* Vector search can be done using:  
  [Dynamic queries](./vector-search-using-dynamic-query.mdx) - suitable for ad-hoc or infrequent searches.  
  [Querying static-indexes that index vector fields](./vector-search-using-static-index.mdx) - suitable for optimized and repeated searches.  
...
* The embeddings used for vector search can be created in several ways:  
  ...
  Learn more in the [Vector search - Overview](./overview.mdx) section.
  
* Vector search queries also empower other AI features in RavenDB such as [GenAI](../gen-ai-integration/start.mdx) and [AI Agents](../ai-agents/start.mdx).   
