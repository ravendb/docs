...
* Use `PutCompareExchangeValueOperation` to **update the _value_ and/or _metadata_** of an existing compare-exchange item.
  This operation is also used to create new compare-exchange items, see [Create compare-exchange item](../create-cmpxchg-items.mdx).
...
| **index**    | `number` | The current version of the item when updating an existing item.<br/>Pass `0` to [create a new key](../create-cmpxchg-items.mdx). |
