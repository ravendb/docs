...
see_also:
  ...
  - title: "How to Create a Data Subscription"
    link: "data-subscriptions/creating-subscription/creating-subscription_api"
    source: "docs"
    path: "Data Subscriptions > Creating a data subscription"
  - title: "How to Consume a Data Subscription"
    link: "data-subscriptions/consuming-subscription"
    source: "docs"
    path: "Data Subscriptions"
---
...
In this article we will tackle the problem of processing invoices in an asynchronous manner using the [RavenDB Data subscriptions](https://docs.ravendb.net/data-subscriptions/overview) feature.  

We will create a [data subscription](https://docs.ravendb.net/data-subscriptions/overview) on `Orders` collection, and use a [Subscription Worker](https://docs.ravendb.net/data-subscriptions/consuming-subscription) to process the newly added `Orders` documents. In this particular article we are going to process in an ongoing fashion, but since `Subscriptions` state is persisted, it can be a process that runs on a schedule, like overnight or on weekends.  

In the subscription batch processing we will calculate the overall products cost, prepare the invoice PDF file and store it as an [Attachment](https://docs.ravendb.net/document-extensions/attachments/overview) to the `Invoices` document.
...
The subscription worker is opened using default [SubscriptionWorkerOptions](https://docs.ravendb.net/data-subscriptions/consuming-subscription), which means the subscription worker [strategy](https://docs.ravendb.net/data-subscriptions/consuming-subscription) is `OpenIfFree`.

The code here is simply creation of a subscription worker. Additional examples can be found in the [Subscription Consumption Examples](https://docs.ravendb.net/data-subscriptions/consuming-subscription) documentation article.

In the batch processing code, we check `order.InvoiceCreated` and if it is `true` we skip the item. The reason for that is that in case of [subscription connection failover](https://docs.ravendb.net/data-subscriptions/overview) we might receive an item a second time, meaning we could get an `Order` that we already created an invoice for.  

After checking the value of `order.InvoiceCreated`, we open a [Session](https://docs.ravendb.net/client-api/session/what-is-a-session-and-how-does-it-work) and load the related `Products` that were ordered.  
