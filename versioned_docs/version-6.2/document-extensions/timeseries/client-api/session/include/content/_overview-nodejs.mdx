...
* Time series data can be _included_ when -  
   * [Loading a document using `session.load`](../with-session-load.mdx)  
   * [Loading a document by query via `session.query`](../with-session-query.mdx)  
   * [Loading a document by raw query via `session.advanced.rawQuery`](../with-raw-queries.mdx)  
