...
* **Atomic Guards** are [compare-exchange key/value items](../overview.mdx)
...
1. These are **custom compare-exchange items**, created by the user for any purpose,  
   as described in [Create compare-exchange items](../create-cmpxchg-items.mdx).
...
* Since atomic guards are implemented as compare-exchange items,  
  ...
  Learn more in [why compare-exchange items are not replicated](../overview.mdx#why-compare-exchange-items-are-not-replicated-to-external-databases).
...
* You can still take this manual approach by disabling the automatic use of atomic guards in a cluster-wide session,
  and managing the required [compare-exchange key/value pairs](../overview.mdx) yourself,
  as shown in this [example](../overview.mdx#example-i-email-address-reservation).
