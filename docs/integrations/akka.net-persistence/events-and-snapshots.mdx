...
description: "Store and retrieve Akka.NET persistent actor events and snapshots using RavenDB as the journal and snapshot storage backend."
...
**Atomic-guards usage**:  
  ...
  * As with every document created using a cluster-wide transaction in RavenDB, 
    the server creates an [Atomic-Guard](../../compare-exchange/atomic-guards.mdx) for each event or snapshot document that is stored to prevent concurrent modifications.
