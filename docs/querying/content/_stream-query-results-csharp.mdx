...
* **Query limitations**:  
  ...
  * A streaming query does not wait for indexing by design.  
    So calling [WaitForNonStaleResults](../customize-query.mdx#waitfornonstaleresults) is Not supported and will result in an exception.   
...
| - | - | - |
| **query** | [IQueryable](../overview.mdx#session-query), [IDocumentQuery](../overview.mdx#session-advanced-documentquery) or [IRawDocumentQuery](../overview.mdx#session-advanced-rawquery) | The query for which to stream results |
