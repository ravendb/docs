...
* By default, optimistic concurrency checks are **disabled**. Changes made outside of the session object will be overwritten. 
  ...
  with the default configuration of the [session](../../what-is-a-session-and-how-does-it-work.mdx).
...
* In this page:
  * [Enable for specific session](#enable-for-specific-session)
  * [Enable globally](#enable-globally)
  * [Disable for specific document (when enabled on session)](#disable-for-specific-document-when-enabled-on-session))
  * [Enable for specific document (when disabled on session)](#enable-for-specific-document-when-disabled-on-session)) 
...
[Transaction support in RavenDB](../../../faq/transaction-support.mdx)
...
* Use the [store.conventions.use_optimistic_concurrency](../../../configuration/conventions.mdx#useoptimisticconcurrency) convention to enable globally.
...
* This is done by passing `None` as a change vector value to the [store](../../storing-entities.mdx) method.
...
* This is done by passing an empty `str` as the change vector value to the [store](../../storing-entities.mdx) method.  
