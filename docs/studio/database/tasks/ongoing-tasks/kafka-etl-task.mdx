...
* In this article:
  ...
  * [Creating a Kafka ETL task](#creating-a-kafka-etl-task)
    * [Open Kafka ETL Task View](#open-kafka-etl-task-view)
    * [Define Kafka ETL Task](#define-kafka-etl-task)
    * [Use RavenDB Certificate](#use-ravendb-certificate)
    * [Options Per Topic](#options-per-topic)
  ...
  * [Prerequisites for a secure Kafka server](#prerequisites-for-a-secure-kafka-server)
    ...
    * [Reading the transactional IDs from Studio](#reading-the-transactional-ids-from-studio)
...
otherwise enter via [Add new connection option](#define-kafka-etl-task).
...
* Saved tasks are listed in the Ongoing Tasks view; expand a task bar to find its scripts' transactional IDs.
    ...
    ![Ongoing tasks view: Saved task](./assets/kafka_ongoing-tasks-view_saved-task.png)

* Copy each transactional ID.

    ![Expanded task bar: Transactional ID](./assets/kafka_transactional-id.png)
