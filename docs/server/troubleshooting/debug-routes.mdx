...
| /admin/debug/node/ping | GET | <ul><li>(Optional) <strong>url</strong><br /></li><li>(Optional) <strong>node</strong></li></ul>Specifies the node by url or node tag | Test the ability to reach the server | |
...
| /databases/*/admin/debug/cluster/txinfo | GET | <ul><li>(Optional) <strong>from</strong><br />Number of results to skip</li><li>(Optional) <strong>take</strong><br />Number of results to take</li></ul> | List the incomplete [cluster transaction commands](../clustering/cluster-transactions.mdx#cluster--cluster-wide-transactions) | |
