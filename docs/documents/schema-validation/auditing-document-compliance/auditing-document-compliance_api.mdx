...
* Add an index map with a call to a validation method (`Schema.GetErrorsFor()` if this is a C# static index, or `schema.getErrorsFor()` if this is a JavaScript index).  
  ```csharp
...
using (var session = store.OpenAsyncSession())
{
  ...
  var results = await session.Query<Result>("Orders_WithValidation_JS")
      .Select(x => new
      {
