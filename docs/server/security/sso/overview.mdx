...
* In this article:
   ...
   * [Next steps](../../../server/security/sso/overview.mdx#next-steps)
...
- [Deploying the SSO application](../../../server/security/sso/deploying-sso-app.mdx) - interactive `install.sh` / `install.ps1` installer (recommended), plus the equivalent manual Docker Compose setup.
- [SSO application configuration](../../../server/security/sso/sso-configuration.mdx) - full reference of `settings.json` keys and `RAVENDBSSO_*` environment variables.
- [Configuring RavenDB for SSO](../../../server/security/sso/configuring-ravendb-for-sso.mdx) - registering SSO server certificates and SSO user entries in your cluster.
