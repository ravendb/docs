...
* In this article:
    ...
    * [Why choose RavenDB as your vector database](#why-choose-ravendb-as-your-vector-database)
    * [Vector search in RavenDB](#vector-search-in-ravendb)
      * [Vector search using a dynamic query](#vector-search-using-a-dynamic-query)
      * [Vector search using a static-index](#vector-search-using-a-static-index)
...
* The raw data is converted into embeddings using [transformers](https://huggingface.co/docs/transformers).  
  To optimize storage and computation, transformers can encode embeddings with lower-precision data types, such as 8-bit integers, through a technique called [quantization](./vector-search-using-dynamic-query.mdx#quantization-options).
...
* An index can consist of multiple index-fields, each having a distinct type, such as a standard field, a spatial field, a full-text search field, or a **vector field**.
  ...
  An example is available in [Indexing multiple field types](./vector-search-using-static-index.mdx#indexing-multiple-field-types).

* Document [attachments](./indexing-attachments-for-vector-search.mdx) can also be indexed as vector fields,
...
* **Textual input**:  
  ...
  [Embeddings generation tasks](../generating-embeddings/overview.mdx).
...
* **Numerical arrays input**:  
  ...
  Use RavenDB's dedicated data type, [RavenVector](./data-types-for-vector-search.mdx#ravenvector), to store these embeddings in your document entities.  
...
* **Optimize storage via quantization**:  
  ...
  Learn more in [Quantization options](./vector-search-using-dynamic-query.mdx#quantization-options).

* **Perform vector search**:  
  Leverage RavenDB's [Auto-indexes](./vector-search-using-dynamic-query.mdx)
  and [Static indexes](./vector-search-using-static-index.mdx) to perform a vector search,  
...
* ✔ **Query raw text**:    
   ...
   * the search term itself  
   ...
   Learn more in: [Dynamic vector search - querying TEXT](./vector-search-using-dynamic-query.mdx#dynamic-vector-search---querying-text).

* ✔ **Query pre-generated TEXT embeddings (created by RavenDB embeddings tasks)**:  
   If your database already contains embeddings generated by a RavenDB [Embeddings generation task](../generating-embeddings/overview.mdx),  
   ...
   * Providing a text search term, which will be embedded using the same model for comparison.
   ...
   Learn more in: [Querying pre-made embeddings generated by tasks](./vector-search-using-dynamic-query.mdx#querying-pre-made-embeddings-generated-by-tasks).
...
* ✔ **Query pre-generated TEXT or NUMERICAL embeddings (created externally)**:   
   ...
   This option is suitable for non-textual content, such as **images, audio, or video**, which RavenDB cannot embed automatically. 
   Store the vectors in your documents as numeric arrays (e.g., _float[]_) and provide an embedding vector as the search input.  
    
   Learn more in: [Dynamic vector search - querying NUMERICAL content](./vector-search-using-dynamic-query.mdx#dynamic-vector-search---querying-numerical-content).

<Admonition type="info" title="">    
The full description of how to perform a **dynamic vector search query** is available in article:  
[Vector Search using a Dynamic Query](./vector-search-using-dynamic-query.mdx). 
...
* ✔ **Index embeddings from raw text**:  
   ...
   * <u>When querying the index</u>:  
     You provide a text search term, which is embedded using the same model and compared against the indexed vectors.
       ...
       Learn more in: [Indexing vector data - TEXT](./vector-search-using-static-index.mdx#indexing-vector-data---text).
    
* ✔ **Index embeddings from pre-generated TEXT embeddings (via RavenDB embeddings tasks)**:  
   * <u>At indexing time</u>:  
     If your documents contain embeddings generated by a RavenDB [Embeddings generation task](../generating-embeddings/overview.mdx),  
   ...
   * <u>When querying the index</u>:    
     You provide a text search term, which is embedded using the same task (i.e., same model), and compared against the stored vectors.
       ...
       Learn more in: [Indexing pre-made text-embeddings](./vector-search-using-static-index.mdx#indexing-pre-made-text-embeddings).
    
* ✔ **Index embeddings from pre-made TEXT or NUMERICAL embeddings (e.g., images)**:  
    * <u>At indexing time</u>:  
      If you have numerical embeddings generated outside RavenDB (e.g., from text, images, videos, etc.),  
      you can store them in your documents either:  
    ...
    * <u>When querying the index</u>, you can search using:  
      ...
      * A numeric vector, provided directly as the search term  
      ...
      Learn more in: [Indexing vector data - NUMERICAL](./vector-search-using-static-index.mdx#indexing-vector-data---numerical).

* ✔ **Index embeddings from attachment content**:    
   You can index attachments using a static-index to enable vector search on their content.  
   Learn more in: [Indexing attachments for vector search](./indexing-attachments-for-vector-search.mdx).    
 
<Admonition type="info" title="">    
The full description of how to perform a **vector search query using a static-index** in RavenDB is available in article:
[Vector Search using a Static-Index](./vector-search-using-static-index.mdx).
