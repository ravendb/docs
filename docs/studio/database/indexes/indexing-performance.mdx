...
![Index Types](./assets/index-types.png)
 ...
 3. **ReplacementOf**  
    ...
    * When an existing index definition is changed, RavenDB uses the old version until the new version is completely built, then saves the old definition in the [index history](./index-history.mdx).  
      Until the new version completely replaces the old, the two are in a state called [side-by-side indexing](./indexes-list-view.mdx#index-list-view---side-by-side-indexing).
...
- [Map Index](./create-map-index.mdx)  
- [Multi-Map Index](./create-multi-map-index.mdx)  
- [Map-Reduce Index](./create-map-reduce-index.mdx)  
...
If a configuration is specific to an index, it can be [set in the Studio](./create-map-index.mdx#configuration).  
...
* [Low Memory](../../../server/configuration/memory-configuration.mdx#memorylowmemorylimitinmb) resources can slow down your system and result in [batch stops](#common-indexing-issues). 
  ...
  * **Cloud server**  
     ...
     * On a cloud instance, you can [upgrade](/cloud/portal/cloud-portal-products-tab) your instance, [scale](/cloud/cloud-scaling), 
      [externally replicate](../tasks/ongoing-tasks/external-replication-task.mdx) to new instances, and/or optimize your indexes.  
