...
[Starting Quill](starting-quill.mdx) - After this step, Quill is running on your machine, and its
...
* This page covers the third getting started step, **Connecting your database**, walking you through pointing
  Quill at your source PostgreSQL, SQL Server, or MySQL database, and settling the tables it may read.
...
* Quill's internal database stays current by [following the changes](../overview.mdx#mirroring) your source
  ...
  See [Prerequisites](connecting-your-database.mdx#prerequisites) below for details.

* In this article:
   * [Prerequisites](connecting-your-database.mdx#prerequisites)
      * [PostgreSQL](connecting-your-database.mdx#postgresql)
      * [SQL Server](connecting-your-database.mdx#sql-server)
      * [MySQL and MariaDB](connecting-your-database.mdx#mysql-and-mariadb)
   * [Connecting to your source database](connecting-your-database.mdx#connecting-to-your-source-database)
      * [Starting the add-app wizard](connecting-your-database.mdx#starting-the-add-app-wizard)
      * [Naming your app and entering the connection details](connecting-your-database.mdx#naming-your-app-and-entering-the-connection-details)
   * [Verifying your schema](connecting-your-database.mdx#verifying-your-schema)
      * [Choosing the tables](connecting-your-database.mdx#choosing-the-tables)
      * [When schema discovery fails](connecting-your-database.mdx#when-schema-discovery-fails)
...
* **Quill must be able to reach the source database from inside its Docker container.**
   ...
   * To find the address to use, see the
     [Connect to your source database](connecting-your-database.mdx#connecting-to-your-source-database) section below.
...
* Quill reads `wal_level` when it lists your tables, in the
  [Verify your schema](connecting-your-database.mdx#verifying-your-schema) stage, and reports an error unless the value
  is `logical`.
...
* While the Agent service is stopped, the capture tables stay empty. Quill connects and lists your tables as
  ...
  The warning appears in the [Verify your schema](connecting-your-database.mdx#verifying-your-schema) stage,
...
[Verify your schema](connecting-your-database.mdx#verifying-your-schema) stage, and reports an error for any of the three
...
* Quill needs `binlog_format` to be set to `ROW` because changed rows are what it copies into its internal
  database.
* MariaDB differs here. See [On MariaDB](connecting-your-database.mdx#on-mariadb) at the end of this section.
...
A missing `SELECT` privilege surfaces in the [Verify your schema](connecting-your-database.mdx#verifying-your-schema)
...
* [binlog_format](connecting-your-database.mdx#binlog_format) - A MariaDB server's default value is `MIXED`, so this
  variable always needs to be set to `ROW`.
* [gtid_mode](connecting-your-database.mdx#gtid_mode) - A MariaDB server has no such variable. Transaction identifiers
...
After [making your source database reachable and ready to record its changes](connecting-your-database.mdx#prerequisites),
Quill needs a name for your app, and the credentials it will use to reach the source database.

<ContentFrame>

### Starting the add-app wizard

Open **Quill's management dashboard** (as explained in the previous step, [Starting Quill](starting-quill.mdx)).  
...
8. **Next**  
   Click to advance to the mapping stages, covered in [Mapping your tables](mapping-your-tables.mdx).  
   Clicking **Next** runs the same verification as **Verify schema**, described above, ensuring that
   [CDC](connecting-your-database.mdx#prerequisites) can run on the selected tables before advancing to the next stage.
...
[Prerequisites](connecting-your-database.mdx#prerequisites) that the source database does not meet. In the example
