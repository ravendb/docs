...
* In this page:
    * [LoadBalanceBehavior options](#loadbalancebehavior-options)
    * [Initialize LoadBalanceBehavior on the client](#initialize-loadbalancebehavior-on-the-client)
    * [Set LoadBalanceBehavior on the server:](#set-loadbalancebehavior-on-the-server)
        * [By operation](#set-loadbalancebehavior-on-the-server---by-operation)
        * [From Studio](#set-loadbalancebehavior-on-the-server---from-studio)
...
* Requests will be handled based on the `ReadBalanceBehavior` configuration.  
  See the conditional flow described in [Client logic for choosing a node](../overview.mdx#client-logic-for-choosing-a-node).  
   * **_Read_** requests:  
     The client will calculate the target node from the configured [ReadBalanceBehavior Option](../read-balance-behavior.mdx#readbalancebehavior-options).  
   * **_Write_** requests:  
     Will be sent to the [preferred node](../overview.mdx#the-preferred-node).  
...
* **Load-balance**
  ...
  * All **_Read & Write_** requests made on the session (i.e a query or a load request, etc.)  
    ...
    _Read & Write_ requests that are made on the store (i.e. executing an [operation](../../../operations/what-are-operations.mdx))  
...
* This setting can be **overriden** by setting 'LoadBalanceBehavior' on the server, see [below](#set-loadbalancebehavior-on-the-server).
...
default_context_string = contextvars.ContextVar("default_context_string", default=None)

def get_default_context(db_name: str) -> str:
    # Get the string set by the web application elsewhere.
    ctx = default_context_string.get()
    if ctx is None:
        # return a safe default
        return db_name

    # Return the context set elsewhere or default to the db_name
    return ctx
`}
</CodeBlock>
</TabItem>
...
with document_store.open_session() as session:
    # Call 'context' to assign a dedicated context string for this session.
    # For example: "team/9-A", representing the team that the employees loaded in this session belong to.
    session.advanced.session_info.context = "team/9-A"
    ...
    employee2 = session.load("employees/3-A", Employee)
`}
</CodeBlock>
</TabItem>
...
* The `LoadBalanceBehavior` configuration can be set **on the server** by sending an [operation](../../../operations/what-are-operations.mdx).
...
* Once configuration on the server has changed, the running client will get updated with the new settings.  
  See [keeping client up-to-date](../overview.mdx#keeping-the-client-topology-up-to-date).
...
* In scenarios where [optimistic concurrency](../../../session/configuration/how-to-enable-optimistic-concurrency.mdx) is enabled,
