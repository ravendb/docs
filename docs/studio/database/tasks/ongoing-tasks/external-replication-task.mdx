...
* This ongoing task replicates **one-way**, from the source to the destination. For additional functionality,  
  such as filtration and two-way (master-master) replication, consider [Hub/Sink Replication](./hub-sink-replication/overview.mdx).  

* To replicate between two separate, secure RavenDB servers,  
  you need to [pass a client certificate](#step-by-step-guide) from the source server to the destination.
...
* The External Replication task **does _not_ create a backup** of your data and indexes.  
  See more in [Backup -vs- Replication](../../../../backup/faq.mdx#is-an-external-replication-task-a-good-substitute-for-a-backup-task)

* In this page:  
  * [General Information about External Replication Task](#general-information-about-external-replication-task)
  * [Step-by-Step Guide](#step-by-step-guide)
  * [Definition](#definition)  
  * [Details in Tasks List View](#details-in-tasks-list-view)  
...
* Ongoing tasks
   ...
   * [Backup](../../../../backup/create/periodic-tasks/database-backup.mdx#creating-and-managing-periodic-backups-via-studio)
   * [Hub/Sink Replication](./hub-sink-replication/overview.mdx)
...
2. **Create Target Database in Destination Server**  
  * Consider [creating an empty target database](../../create-new-database/general-flow.mdx) 
    because data transfer can overwrite existing data.  
  * If the source database is [encrypted](../../create-new-database/encrypted.mdx#creating-encrypted-database), 
...
3. **Define External Replication Task in the Source Database**  
    Learn more about [defining the task](#definition) in the dedicated section.  
  * **Optional Parameters**  
     - Task Name  
     - Delay Time  
     - Preferred Node  
...
3. **Set Replication Delay Time** (Optional)  
   ...
   * Having a delayed instance of a database allows you to "go back in time" and undo contamination to your data due to an attack, faulty patch script, or other human errors.  
     * This doesn't replace the need to [safely backup your databases](../../../../backup/create/periodic-tasks/database-backup.mdx#creating-and-managing-periodic-backups-via-studio), but it does provide a way to stay online while repairing.  

4. **Set Preferred Responsible Node** (Optional)  
  * Select a preferred mentor node from the [Database Group](../../settings/manage-database-group.mdx) to be the responsible node for this External Replication Task  
...
5. **Create a new RavenDB connection string**  
  ...
  * The connection string defines the external database and its server URL to replicate to.  
    ...
    See [Offline Behavior - When the destination node is down](#offline-behavior).  
