...
description: "Force an immediate RavenDB license update from the license server to apply renewed or upgraded license keys without restarting the server."
sidebar_label: "Force license update"
sidebar_position: 4
---
...
* RavenDB contacts the License server at `api.ravendb.net` every 24 hours (counted from server startup), to check if there are any updates in your current license that need to be applied.  
   ...
   * **And** the [DisableAutoUpdate](./configuration.mdx#licensedisableautoupdate) configuration option is set to **false**.  
  </Admonition>
...
2. **Force update**  
   ...
   <Admonition type="note" title="">
   Set the [DisableAutoUpdateFromApi](./configuration.mdx#licensedisableautoupdatefromapi) configuration option to determine where the license is updated from:  
   * When **true**, the license is updated from the [configuration key](./activate-license.mdx#activate-your-license-using-a-configuration-key).  
