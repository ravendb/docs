...
description: "Replicate RavenDB documents to Elasticsearch indexes using ETL tasks with custom JavaScript transformations and field mapping."
sidebar_position: 5
---
...
* An Elasticsearch ETL task uses an **Elasticsearch connection string** to connect to its destination Elasticsearch cluster.  
  The connection string stores one or more Elasticsearch node URLs and, when needed, the authentication settings required by those nodes.
...
* An Elasticsearch connection string can be created in any of these ways:
  * **From the Client API**:
  ...
  * **From Studio**:
    ...
    or while creating an [Elasticsearch ETL task](../../../studio/database/tasks/ongoing-tasks/elasticsearch-etl-task.mdx#define-the-elasticsearch-etl-task).

* The example below adds a **per-database** Elasticsearch connection string from the **Client API**.  
  The connection string is stored in the current database record and can be used by tasks in that database.

<Tabs>
<TabItem value="sync" label="Sync" default>
```csharp
// Define an Elasticsearch connection string
// =========================================
var elasticSearchConnectionString = new ElasticSearchConnectionString
{
    // The name used by the Elasticsearch ETL task configuration
    Name = "elasticsearch-connection-string-name",

    // One or more Elasticsearch node URLs
...
var putConnectionStringOp =
    new PutConnectionStringOperation<ElasticSearchConnectionString>(elasticSearchConnectionString);
...
| **Nodes** | `string[]` | One or more Elasticsearch node URLs. |
| **Authentication** | `Authentication` | Optional authentication settings. Omit this property when the Elasticsearch destination does not require authentication. |

`Authentication`:

| Property | Type | Description |
|---|---|---|
| **Basic** | `BasicAuthentication` | Authenticate with a username and password. |
| **ApiKey** | `ApiKeyAuthentication` | Authenticate with an API key. |
| **Certificate** | `CertificateAuthentication` | Authenticate with a certificate. |
...
| Property | Type | Description |
|---|---|---|
| **ApiKeyId** | `string` | The API key identifier. Used together with `ApiKey` when `EncodedApiKey` is not supplied. |
| **ApiKey** | `string` | The API key secret. Used together with `ApiKeyId` when `EncodedApiKey` is not supplied. |
| **EncodedApiKey** | `string` | A Base64-encoded API key value. If supplied, RavenDB uses this value directly. |

`CertificateAuthentication`:

| Property | Type | Description |
|---|---|---|
| **CertificatesBase64** | `string[]` | One or more certificates encoded as Base64 strings. |
