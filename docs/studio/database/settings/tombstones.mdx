...
* A **tombstone** is the marker RavenDB keeps when a document is deleted, so the processes that rely on it  
  (replication, ETL, indexes, and periodic backup) can handle the deletion.  

* RavenDB periodically [cleans up](../../../glossary/tombstone.mdx#tombstone-cleanup) all the tombstones that are no longer needed by any process.
...
* In this article:
   * [The Tombstones view](../../../studio/database/settings/tombstones.mdx#the-tombstones-view)
   * [Reading the Tombstones tables](../../../studio/database/settings/tombstones.mdx#reading-the-tombstones-tables)
      * [Per Collection table](../../../studio/database/settings/tombstones.mdx#per-collection-table)
      * [Per Task table](../../../studio/database/settings/tombstones.mdx#per-task-table)
      * [Example: a disabled index blocking cleanup](../../../studio/database/settings/tombstones.mdx#example-a-disabled-index-blocking-cleanup)

</Admonition>

<Panel heading="The Tombstones view">
...
4. **Force cleanup**  
   ...
   set by [Tombstones.CleanupIntervalInMin](../../../server/configuration/tombstone-configuration.mdx#tombstonescleanupintervalinmin).  
...
5. **Minimum Etags**  
   ...
   a single position for the whole database: replication, periodic backup, and any ETL task or
...
6. **Per Collection - Max Etags that can be deleted**  
   ...
   See [Per Collection table](../../../studio/database/settings/tombstones.mdx#per-collection-table).  
...
7. **Per Task - Max Etag that can be deleted**  
   ...
   See [Per Task table](../../../studio/database/settings/tombstones.mdx#per-task-table).  
