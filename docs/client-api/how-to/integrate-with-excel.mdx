...
description: "Export RavenDB query results directly into Excel using the dedicated streaming endpoint that outputs data in CSV format."
sidebar_position: 3
---
...
In order to take advantage of this feature, you need to specify a valid query according to [RQL syntax](../../querying/rql/what-is-rql.mdx).
...
In some cases it might be cumbersome to use the URL to send the query or the query might be too long. Please see our [dedicated section](#dealing-with-long-query-urls-in-excel) that deals with that problem.
