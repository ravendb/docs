...
* In this article:
   ...
   * [Define agent tools](#define-agent-tools)
      * [Add query tools](#a-add-query-tools)
      * [Add action tools](#b-add-action-tools)
...
1. **AI Hub**  
   Click to open the [AI Hub view](../../ai-tasks-list-view.mdx).  
...
4. **Connection String**  
      ...
      **Select** an existing [connection string](../../connection-strings/overview.mdx) 
...
[Read more about agent parameters](../overview.mdx#query-parameters).
...
* [Find an AI agent usage flow chart here](../overview.mdx#ai-agent-usage-flowchart)
...
7. **Sample parameters object** and **Parameters JSON schema**  
   ...
   [Read more about query parameters](../overview.mdx#query-parameters)  
...
8. **Advanced settings**  
    ...
    * **Allow model queries**  
      ...
      <Admonition type="note" title="">
      When disabled, the LLM will not be able to trigger this tool - but if the tool is set as an [initial-context query](../overview.mdx#initial-context-queries) the agent will still be able to execute it when it is started.  
      </Admonition>
    ...
    * **Add to initial context**  
      Enable to set this tool as an [initial-context query](../overview.mdx#initial-context-queries).  
...
[Learn more about sub-agents](../multi-agents.mdx)
