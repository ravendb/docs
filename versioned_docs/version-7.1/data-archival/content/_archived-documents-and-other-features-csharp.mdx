...
* Once you have archived documents in your database (see how to [enable](../enable-data-archiving.mdx) and [schedule](../schedule-document-archiving.mdx) document archiving),  
...
* **Configuring indexing behavior - Static indexes**:
    * **At the database level or server-wide**:  
      ...
      set the [Indexing.Static.ArchivedDataProcessingBehavior](../configuration.mdx#indexingstaticarchiveddataprocessingbehavior)
...
* **Configuring indexing behavior - Auto indexes:**
    * **At the database level or server-wide**:  
      ...
      set the [Indexing.Auto.ArchivedDataProcessingBehavior](../configuration.mdx#indexingautoarchiveddataprocessingbehavior) configuration key (default `ExcludeArchived`).
    * **Per index**:  
...
This setting will **override** the global configuration defined by the [Indexing.Static.ArchivedDataProcessingBehavior](../configuration.mdx#indexingstaticarchiveddataprocessingbehavior) configuration key.
...
* **Dynamic queries (auto-indexes)**:
  * When making a dynamic query, RavenDB creates an auto-index to serve it.
    Whether that index processes archived documents depends on the value of the [Indexing.Auto.ArchivedDataProcessingBehavior](../configuration.mdx#indexingautoarchiveddataprocessingbehavior) configuration key at the time the query is made.
...
* **Querying static-indexes**:  
  * When querying a static-index, the results will include, exclude, or consist solely of archived documents depending on how the static-index was configured.
    The index behavior is determined by:  
      * the value of the [Indexing.Static.ArchivedDataProcessingBehavior](../configuration.mdx#indexingstaticarchiveddataprocessingbehavior)  configuration key at the time the static-index was created, or -
...
* **Configuring the subscription task behavior**:
  * **At the database level or server-wide**:  
    ...
    set the [Subscriptions.ArchivedDataProcessingBehavior](../configuration.mdx#subscriptionsarchiveddataprocessingbehavior) configuration key at either the database level or server-wide
...
This setting will **override** the global configuration defined by the [Subscriptions.ArchivedDataProcessingBehavior](../configuration.mdx#subscriptionsarchiveddataprocessingbehavior) configuration key.
...
* Patching can be used to **schedule** multiple documents for archiving. See the dedicated sections:   
  [Schedule multiple documents for archiving - from the Studio](../schedule-document-archiving.mdx#schedule-multiple-documents-for-archiving---from-the-studio).  
...
* Patching is used to **unarchive** documents.  
  See the dedicated article [Unarchiving documents](../unarchiving-documents.mdx).   
