...
* To restrict the creation of JavaScript indexes to database admins (and above),  
  set the [Indexing.Static.RequireAdminToDeployJavaScriptIndexes](../server/configuration/indexing-configuration.mdx#indexing.static.requireadmintodeployjavascriptindexes) configuration to `true`.
...
* To define a JavaScript index using inline string compilation,  
  you must set the [Indexing.AllowStringCompilation](../server/configuration/indexing-configuration.mdx#indexing.allowstringcompilation) configuration key to _true_.
