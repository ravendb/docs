...
* Define the backup configuration as explained in the [periodic database-backup task](./database-backup.mdx#a-defining-basic-task-options) article  
  ...
  * **Backups storage**  
    ...
    Each database task will automatically store its backups in a [subfolder named after the database](../../overview.mdx#backups-storage), under the root storage location.  
