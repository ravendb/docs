...
<Admonition type="note" title="">
**Automatic renewal**:  
If your server has an active connection to RavenDB's [License Server](../licensing/overview.mdx#license-server),  
and if the [DisableAutoUpdate](../licensing/configuration.mdx#licensedisableautoupdate) configuration is Not set to true,  
...
* Replace the existing license key with the new one from Studio, as described [here](../licensing/replace-license.mdx#replace-license-from-studio).

**Or**:  

* Set [DisableAutoUpdateFromApi](../licensing/configuration.mdx#licensedisableautoupdatefromapi) to true.

* Activate the new license key using the [configuration keys](../licensing/activate-license.mdx#activate-license-with-configuration-keys).
