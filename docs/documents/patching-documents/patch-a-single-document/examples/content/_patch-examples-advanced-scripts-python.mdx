...
patch_request.values = {"lowStock": 10, "discount": 0.8}
...
session.advanced.defer(PatchCommandData("companies/1-A", None, patch_request))
...
store.operations.send(PatchOperation("companies/1-A", None, patch_request))
```
</TabItem>
</Tabs>
...
<TabItem value="Session_defer_syntax" label="Session API - defer">
```python
encryption_key = os.environ.get("PATCH_ENCRYPTION_KEY")

if not encryption_key:
    raise RuntimeError("PATCH_ENCRYPTION_KEY must contain a Base64-encoded 256-bit AES key.")
...
patch_request.script = """
    if (this.SensitiveData != null) {
        ...
        // The key comes from patch_request.values; do not store it in the document.
...
patch_request.values = {"encryptionKey": encryption_key}
