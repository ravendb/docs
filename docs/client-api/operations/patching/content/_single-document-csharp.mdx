...
* Since the operation is executed in a single request to the database,  
  the patch command is performed in a single write [transaction](../../../faq/transaction-support.mdx).  

* The current page covers patch operations on single documents.

* Patching has three possible interfaces: [Session API](#session-api), 
[Session API using Defer](#session-api-using-defer), 
and [Operations API](#operations-api).

* Patching can be done from the [client API](#examples) as well as in the [studio](../../../../studio/database/documents/patch-view.mdx).  
...
* [API overview](#api-overview)  
  ...
  * [List of script methods](#list-of-script-methods)
* [Examples](#examples)  
  * [Change value of single field](#change-value-of-single-field)  
  * [Change values of two fields](#change-values-of-two-fields)  
  * [Increment value](#increment-value)  
  * [Add or increment](#add-or-increment)  
  * [Add or patch](#add-or-patch)  
  * [Add or patch to an existing array](#add-or-patch-to-an-existing-array)  
...
<TabItem value="Operation_syntax" label="Operation_syntax">
```csharp
store.Operations.Send(new PatchOperation("companies/1-A", null, new PatchRequest
{
    Script = @"
        if (!this.ExternalId) {
            this.ExternalId = crypto.randomUUID();
...
<TabItem value="Operation_syntax" label="Operation_syntax">
```csharp
store.Operations.Send(new PatchOperation("users/1", null, new PatchRequest
{
    Script = @"
        // this.SensitiveData is the document field you want to encrypt
        if (this.SensitiveData) {
