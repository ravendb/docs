...
* An **Incremental Time Series** is a [time series](../../../document-extensions/timeseries/overview) 
  whose entries can be incremented and decremented, much like [Counters](../../../document-extensions/counters/overview) 
  embedded in time series entries.  

* Similar to time series, incremental time series are composed of consecutive [entries](../../../document-extensions/timeseries/overview#time-series-entries).  
   ...
   * Each entry value can be used as a counter (incremented and decremented).  
   * Multiple entries can share a timestamp, indicating updates from multiple sources.  
     ...
     the counter's counter values both by its uniresults both the unified value, and the values results by their origin.  
  Incremental time series entries, however,  are marked by time stamps.  
  And as with time series, incremental time series entries values The values of an incremental time series **Entry**, marked by a particular timestamp, can be incremented 
  and decremented by multiple clients and nodes without conflict.  
  The values of an incremental time series **entry** from different nodes and client are aggregated, and you can retrieve both their aggregated value 
  and their 


* Using incremental time series operations, time series values can be easily used as counters.  

* Innate to RavenDB’s distributed environment, multiple clients can simultaneously increase or 
  decrease such time series counters without conflict.  

* A simple scenario that demonstrates the usability of incremental time series operations is a time series 
  that collects the number of sales made by a webshop. A time series counter can be continuously updated with 
  the number of sales made over the last hour. When the hour changes, the next counter will be updated.  

* Incremental Time series can be managed and utilized using [API methods](../../document-extensions/timeseries/client-api/overview) 
  or the [Studio](../../studio/database/document-extensions/time-series).  
