...
description: "Understand how RavenDB counters replicate across cluster nodes using CRDT semantics to handle concurrent increments without conflicts."
...
* In this article:  
  * [Counter value modification](#counter-value-modification)  
     * [Value modification and replication flow](#value-modification-and-replication-flow) 
     * [Reading a Counter's value](#reading-a-counters-value)  
  * [Counter name modification](#counter-name-modification)  
  * [Concurrent value modification](#concurrent-value-modification)  
  * [Concurrent `Delete` and `Increment`](#concurrent-delete-and-increment)  
     * [In a single-node system](#in-a-single-node-system)  
...
After a Counter’s value is modified on a node, that node [replicates](#value-modification-and-replication-flow)
...
As explained in the [Value modification and replication flow](#value-modification-and-replication-flow) section,  
...
* Concurrent value modifications do not cause conflicts.
  For more details, see [Counters and conflicts](./overview.mdx#counters-and-conflicts).  

</Panel>

<Panel heading="Concurrent&nbsp;`Delete`&nbsp;and&nbsp;`Increment`">

A sequence of Counter actions is [cumulative](./overview.mdx#cumulative-counter-actions), 
as long as the Counter is not [Deleted](./delete.mdx).  
When a _Delete_ is involved, the order of execution becomes significant.  

If [Increment](./create-or-modify.mdx) and [Delete](./delete.mdx) are called concurrently,  
