...
* By creating a server-wide backup task, you can **define the backup configuration once** and apply it to **multiple databases** as per-database backup tasks, instead of manually creating an individual backup task for each database.  
...
A server-wide backup task is a **backup configuration template** that is automatically applied to all your databases as multiple **per-database backup tasks**.  
