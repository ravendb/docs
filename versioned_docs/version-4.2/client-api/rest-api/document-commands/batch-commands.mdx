...
| **Type** | Set to `PUT` | Yes |
| **ForceRevisionCreationStrategy** | When updating an existing document, set to `Before` to make a [revision](../../../server/extensions/revisions.mdx) of the document before it is updated. | No |
...
Add an [attachment](../../../client-api/session/attachments/what-are-attachments.mdx) to a document, or update an existing attachment.  
...
The string used as a separator must not appear elsewhere in the request body - i.e. "ChangeVector" or "[%7B" are not valid separators. | Yes |
| **Id** | Document ID | Yes |
