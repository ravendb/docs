...
* In this page:  
    ...
    * [Projection behavior with a static-index](#projection-behavior-with-a-static-index)  
    * [OfType](#oftype)  
...
* **Type of projection fields**:  
  ...
  * If you wish to specify fields from the original 'Employee' class type then follow [this example](#oftype) that uses `OfType`.  

* **Source of projection fields**:  

  * Since the index-fields in this example are not [Stored in the index](../../storing-data-in-index.mdx), and no projection behavior was defined,  
    resulting values for `FirstName` & `LastName` will be retrieved from the matching Employee document in the storage.
  
  * This behavior can be modified by setting the [projection behavior](#projection-behavior-with-a-static-index) used when querying a static-index.
...
The `SelectFields` method can only be used by a [Document Query](../../../querying/document-query/what-is-document-query.mdx).  
...
* **By default**, when querying a static-index and projecting query results,  
  the server will try to retrieve the fields' values from the fields [stored in the index](../../storing-data-in-index.mdx).  
