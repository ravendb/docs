...
* Quill reads from your database only to mirror part of your data: you select the tables it may draw answers from, and
  Quill keeps a live copy of their data in an internal database.  
...
* Agents carry two kinds of tools, and apply both freely during conversations: **query tools** and **actions**.  
  Quill drafts an agent's query tools for you; you can adjust them, add your own, and define the agent's actions.
...
* Direct access to [Quill's internal database](#internal-database) requires a client certificate.
...
* **Reading the mirrored database directly**
  ...
  * Your applications can connect to this database using RavenDB client libraries and a client certificate that you
