...
Signing up starts a 90-day free trial. No credit card or sales call is required, and WRU usage is unlimited during the trial.  
For current plan details, see [Quill pricing](https://ravendb.net/quill/pricing).
...
3. **Docker command**  
   ...
   * The license key retrieves the setup package for the registered domain.  
   * The dashboard API key, by contrast, belongs to the signup account.  
     The same key is issued for every Quill domain registered under the same email address.  
     See [One key per account](../security-and-architecture/operator-authentication.mdx#one-key-per-account).  

   Learn more about the Docker command in [Starting Quill](../getting-started/starting-quill.mdx).
