...
description: "Set up RavenDB as the persistence backend for Akka.NET actors — configure the journal, snapshot store, and connection settings."
...
* In this page:
   * [Overview](#overview)
   * [Akka.Persistence.RavenDB](#akkapersistenceravendb)
   * [Installing the RavenDB persistence plugin](#installing-the-ravendb-persistence-plugin)
   * [Configuring the RavenDB persistence plugin with HOCON](#configuring-the-ravendb-persistence-plugin-with-hocon)
       * [Configuration keys](#configuration-keys)
   * [Configuring the RavenDB persistence plugin with Hosting](#configuring-the-ravendb-persistence-plugin-with-hosting)
       * [Syntax](#syntax)
...
With the RavenDB plugin your application can:  
  ...
  * Persist and recover Events to/from a **Journal store**. Learn more in [Events](./events-and-snapshots.mdx#storing-events).
  * Persist and recover Snapshots to/from a **Snapshot store**. Learn more in [Snapshots](./events-and-snapshots.mdx#storing-snapshots). 
  * Query the stored events. Learn more in [Queries](./queries.mdx).
...
* The following is a sample HOCON configuration under the `<akka>` section.  
  See the full description of each configuration key [below](#configuration-keys).
...
* Use method `WithRavenDbPersistence` to configure all relevant parameters.  
  See the available parameters and method overloads in the syntax section [below](#syntax).
