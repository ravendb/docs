...
description: "Rachis is RavenDB's implementation of the Raft consensus algorithm that coordinates cluster-wide operations and leader election."
sidebar_position: 0
---
...
* It allows you to execute an ordered set of operations across your entire cluster.  
  ...
  So any series of events (called a [Raft Commands](./consensus-operations.mdx#raft-commands-implementation-details)) will be executed in the _same_ order on each node.  

* The [Leader Node](./cluster-topology.mdx#leader) accepts all the Raft Commands requests for the cluster and handles committing them cluster-wide.  
...
* Support for voting & non-voting cluster members, see [Cluster Topology](./cluster-topology.mdx).   
