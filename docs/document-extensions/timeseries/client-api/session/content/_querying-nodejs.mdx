...
* Note:  
  The RavenDB client translates the query to [RQL](../../../../../querying/rql/what-is-rql.mdx) before transmitting it to the server for execution.
### Query examples
...
* In this example, we retrieve a company's stock trade data.  
  ...
  This example is based on the sample entries that were entered in [this example](../append.mdx#append-entries-with-multiple-values).  
<Tabs groupId='languageSyntax'>
...
| `TimeSeriesAggregationResult[]` | The returned value for [aggregated data](../../../querying/aggregation-and-projections.mdx) |
...
The return value is the same as listed under the [query syntax](#query-syntax).
