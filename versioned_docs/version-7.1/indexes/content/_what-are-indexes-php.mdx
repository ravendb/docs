...
* This page provides a [Basic example](#basic-example) of creating, deploying, and querying an index.
  Additional examples can be found in [Creating and deploying indexes](../creating-and-deploying.mdx), [Map Indexes](../map-indexes.mdx),
...
* The indexing process iterates over the raw documents, creating an **index-entry** for each document that is processed.
  (Usually a single index-entry is created per raw document, unless working with a [fanout index](../indexing-nested-data.mdx#fanout-index---multiple-index-entries-per-document)).
* Each index-entry contains **index-fields**, and each index-field contains content (**index-terms**) that was generated from the raw documents,
  as defined by the index definition and depending on the [analyzer](../using-analyzers.mdx) used.
...
* The indexing process does not block or interrupt database operations, such as writing data or running queries,
  though queries may temporarily return [stale results](../stale-indexes.mdx) until the index is fully updated.
**Separate storage**:
...
* The first step is to define the index.  
  ...
  Learn more in [Define a static-index using a custom class](../creating-and-deploying.mdx#define-a-static-index-using-a-custom-class).
...
* Additional methods for deploying static-indexes are described in [Deploy a static index](../creating-and-deploying.mdx#deploy-a-static-index).
...
* For detailed guidance on querying with an index, refer to the [Querying an index](../querying/query-index.mdx).
...
This behavior can be changed by applying [Projections](../querying/projections.mdx),  
