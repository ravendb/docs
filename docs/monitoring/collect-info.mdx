...
* In this article:
  ...
  * [Provide incident description](../monitoring/collect-info.mdx#provide-incident-description)
  * [Create debug package](../monitoring/collect-info.mdx#create-debug-package)
     * [Create the package](../monitoring/collect-info.mdx#create-the-package)
     * [Verify the package](../monitoring/collect-info.mdx#verify-the-package)
  * [Enable logs for ongoing issues](../monitoring/collect-info.mdx#enable-logs-for-ongoing-issues)
  * [Download logs](../monitoring/collect-info.mdx#download-logs)
     * [Download the logs](../monitoring/collect-info.mdx#download-the-logs)
     * [Verify the logs](../monitoring/collect-info.mdx#verify-the-logs)
  * [Reproduce scenario](../monitoring/collect-info.mdx#reproduce-scenario)
  * [Create failing test](../monitoring/collect-info.mdx#create-failing-test)
...
(if not already enabled) to capture helpful information, before [downloading the log files](../monitoring/collect-info.mdx#download-logs).
...
To preserve your settings even through a restart, either:
  ...
  * **Save the settings** via Studio:  
    [Open each log type dialog](../monitoring/collect-info.mdx#enable-logs-for-ongoing-issues), 
  ...
  * or **Set the relevant configuration key** as described in the [configuration overview](../server/configuration/configuration-options.mdx):
    * For **Server logs**: set the [Logs.MinLevel](../monitoring/logging/configuration.mdx#logsminlevel) configuration key.  
    * For **Microsoft logs**: set the [Logs.Microsoft.MinLevel](../monitoring/logging/configuration.mdx#logsmicrosoftminlevel) configuration key.
...
* **Log files location** is set by the [Logs.Path](../monitoring/logging/configuration.mdx#logspath) configuration key.

* **Logs deletion** is controlled by the following configuration keys:
  * [Logs.MaxArchiveDays](../monitoring/logging/configuration.mdx#logsmaxarchivedays)
  * [Logs.MaxArchiveFiles ](../monitoring/logging/configuration.mdx#logsmaxarchivefiles)
...
* See how to enable the logs in [Enable logs](../monitoring/collect-info.mdx#enable-logs-for-ongoing-issues).
