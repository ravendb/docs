...
If enabled, RavenDB will export to the target machine/s the cluster-wide certificate defined during setup, and secure its connection with them.
...
* **Note**, however, that to complete the process you still need to register RavenDB's exported certificate  
...
When the target Kafka cluster uses ACLs (Access Control Lists), the credentials  
in the connection string must be granted explicit permissions on the Kafka side  
before the task can produce messages:

* **WRITE** on each target topic (one per `loadTo<TopicName>` call).
* **WRITE** and **DESCRIBE** on the producer's transactional ID (one per  
  transformation script).

Each transformation script has its own transactional ID. In the Kafka ETL  
task's **distribution panel**, the **Transactional ID** row shows the value  
with a copy-to-clipboard button.

For the full rule set and an explanation of why the grants are required, see  
