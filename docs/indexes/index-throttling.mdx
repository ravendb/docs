...
description: "Delay RavenDB indexing operations by a configurable interval to reduce resource consumption during high-throughput write periods."
sidebar_position: 22
---
...
* The gaps in indexing activity and the procession of 
  [larger batches](../indexes/index-throttling.mdx#throttling-and-batches) 
...
* In this page:  
  * [Why Use Index Throttling?](../indexes/index-throttling.mdx#why-use-index-throttling?)  
  * [Index Throttling and Batches](../indexes/index-throttling.mdx#throttling-and-batches)  
  * [Setting Index Throttling](../indexes/index-throttling.mdx#setting-index-throttling)  
     * [Server-Wide](../indexes/index-throttling.mdx#server-wide-index-throttling)  
     * [Per-Database](../indexes/index-throttling.mdx#index-throttling-per-database)  
     * [Per-Index](../indexes/index-throttling.mdx#index-throttling-in-an-index-definition)  
...
Throttled indexes are kept [stale](../indexes/stale-indexes.mdx#indexes-stale-indexes) 
...
The delay in index procession increases the size of Item [batches](../server/configuration/indexing-configuration.mdx#indexingmapbatchsize) 
...
### Server-Wide Index Throttling
 ...
 Index Throttling can be set server-wide using a designated [configuration option](../server/configuration/configuration-options.mdx#settingsjson).  
...
[Server-Wide](../indexes/index-throttling.mdx#server-wide-index-throttling) default.  
...
Setting throttling in an index definition overrides [server-wide](../indexes/index-throttling.mdx#server-wide-index-throttling) 
and [database](../indexes/index-throttling.mdx#index-throttling-per-database) settings configuration.  


* **From Code**:  
  Set throttling for a specific index using the `Indexing.Throttling.TimeIntervalInMs` property.  
<TabItem value="throttlingInterval" label="throttlingInterval">
...
`}
    ...
    the [Indexing.MapBatchSize](../server/configuration/indexing-configuration.mdx#indexingmapbatchsize) property.  
<TabItem value="batchSize" label="batchSize">
<CodeBlock language="csharp">
{`Configuration = new IndexConfiguration
\{
    \{ "Indexing.Throttling.TimeIntervalInMs", "2000" \},
...
* **From Studio**:  
  Use the [Configuration tab](../studio/database/indexes/create-map-index.mdx#configuration) of an index definition 
