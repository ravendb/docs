...
description: "RavenDB replicates documents across cluster nodes automatically, providing high availability with configurable conflict resolution strategies."
sidebar_position: 0
---
...
* __Conflicts__, which may be involved when the data replicates,  
  are resolved according to the defined [conflict resolution](./replication-conflicts.mdx) policy on the receiving end.

* In this page:  
    * [Replication types](#replication-types)  
      * [Internal replication](#internal-replication)
      * [External replication](#external-replication)
      * [Hub/Sink replication](#hubsink-replication)
    * [What is replicated](#what-is-replicated)  
    * [How replication works](#how-replication-works)  
    * [Replication & transaction boundary](#replication--transaction-boundary)  
...
* __Replicate between__: Two databases that are typically set on different clusters.   
  __Handled by__: Handled by the ongoing [External Replication Task](../../ongoing-tasks/external-replication.mdx) defined by the user.  
...
* __Replicate between__: Multiple Sinks that connect to a single Hub on different clusters.  
  __Handled by__: Handled by the ongoing [Hub/Sink Replication Tasks](../../ongoing-tasks/hub-sink-replication.mdx) defined by the user.  
...
* __With internal replication__:  
  ...
  then consistency between the database instances in the database-group is achieved by the [Raft Protocol](../rachis/what-is-rachis.mdx).
...
* A [cluster-wide transaction](../../../client-api/session/cluster-transaction/overview.mdx), which is implemented by the [Raft Protocol](../rachis/consensus-operations.mdx#consensus-operations),  
...
* After a cluster consensus is reached, the Raft command to be executed is propagated to all nodes.  
  When the command is executed locally on a node, if the items that are persisted are of the [type that replicates](#what-is-replicated), 
