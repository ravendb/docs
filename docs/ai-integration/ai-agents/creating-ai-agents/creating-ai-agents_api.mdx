...
* In this article:
   ...
   * [Conversations](../../../ai-integration/ai-agents/creating-ai-agents/creating-ai-agents_api#conversations)
      * [Continuous conversations](../../../ai-integration/ai-agents/creating-ai-agents/creating-ai-agents_api#continuous-conversations)
      * [Stored conversations Prefix and IDs](../../../ai-integration/ai-agents/creating-ai-agents/creating-ai-agents_api#stored-conversations-prefix-and-ids)
      * [Set and start the conversation](../../../ai-integration/ai-agents/creating-ai-agents/creating-ai-agents_api#set-and-start-the-conversation)
         * [Handle action tools requests](../../../ai-integration/ai-agents/creating-ai-agents/creating-ai-agents_api#handle-action-tools-requests--)
         * [Conversation reply](../../../ai-integration/ai-agents/creating-ai-agents/creating-ai-agents_api#conversation-reply--)
         * [Set user prompt and run the conversation](../../../ai-integration/ai-agents/creating-ai-agents/creating-ai-agents_api#set-user-prompt-and-run-the-conversation--)
...
* **Example**  
   ...
   * The second query tool will be used by the LLM when it needs to retrieve all the 
     orders that were sent to a particular country. It uses the "country" agent parameter.  
...
* You can mark a query tool as an [initial context query](../../ai-integration/ai-agents/ai-agents_overview#initial-context-queries) using its `Options.AddToInitialContext` property, to execute the query immediately when the agent is started and provide the LLM with the query results as part of the initial context.  
   * An initial context query is **not allowed** to use LLM parameters, since the LLM 
     will not have the opportunity to fill them with values before the query is executed.  
...
Alternatively, you can set the agent configuration's `SampleObject` or `OutputSchema` 
...
If [agent parameters](.../../../ai-integration/ai-agents/creating-ai-agents/creating-ai-agents_api#add-agent-parameters) 
...
* **Example**  
  ...
  // A user would pick a country, or enter "everywhere" for all countries.
...
#### Handle action tools requests - 
Handle an action tool request by passing the chat's `Handle` method the name of 
the action tool you want to handle. When the LLM sends your agent an action request, 
any data included in the request will reach the handler and the client will be able to use it.  
...
The agent in this example helps a human experience manager to reward employees.  
...
public async Task createAndRunAiAgent_full()
{
    ...
    //  Set agent parameters
    agent.Parameters.Add(new AiAgentParameter("country", "A specific country that orders were shipped " +
                                 "to, or \"everywhere\" to look for orders shipped to all countries"));
...
You can set the agent to [stream the LLM's response to the client](../../../ai-integration/ai-agents/ai-agents_overview#stream-llm-responses) in real time as the LLM generates it, using the `StreamAsync` method, instead of using [RunAsync](../../../ai-integration/ai-agents/creating-ai-agents/creating-ai-agents_api#set-user-prompt-and-run-the-conversation--) which sends the whole response to the client when it is fully prepared.  
