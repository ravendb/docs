...
* All comparison operators are supported (`=`, `!=`, `<`, `<=`, `>`, `>=`).    
  For example, the following query returns orders whose _RequiredAt_ date is **after** the current server time:
    ...
    List<Order> orders = session
        .Query<Order>()
        .Where(o => o.RequiredAt > RavenQuery.Now())
        .ToList();
    ```
    </TabItem>
    <TabItem value="Query_async" label="Query_async">
    ...
    List<Order> orders = session.Advanced
        .DocumentQuery<Order>()
        .WhereGreaterThan("RequiredAt", RavenDocumentQuery.Now())
        .ToList();
    ```
    </TabItem>
    <TabItem value="DocumentQuery_async" label="DocumentQuery_async">
    ...
    ```csharp
    List<Order> orders = session.Advanced
        .RawQuery<Order>("from Orders where RequiredAt > now()")
        .ToList();
    ```
    </TabItem>
    <TabItem value="RawQuery_async" label="RawQuery_async">
    ```csharp
    List<Order> orders = await asyncSession.Advanced
        .AsyncRawQuery<Order>("from Orders where RequiredAt > now()")
        .ToListAsync();
    ```
    </TabItem>
    <TabItem value="RQL" label="RQL">
...
* **For example**:  
  ...
  The generated timestamp is truncated to `00:00:00.000` UTC, discarding the hour, minute, second, and millisecond components.
      ...
      **So this predicate**: `RequiredAt <= now('+7d')`  
      **effectively becomes**: `RequiredAt <= 2026-05-17T00:00:00Z`

* The following query returns orders whose _RequiredAt_ value is earlier than or equal to 7 days from now,  
  rounded down to the day:
...
List<Order> orders = session.Query<Order>()
    .Where(o => o.RequiredAt <= RavenQuery.Now("+7d"))
    .ToList();
```
</TabItem>
<TabItem value="Query_async" label="Query_async">
...
List<Order> orders = session.Advanced
    .DocumentQuery<Order>()
    .WhereLessThanOrEqual("RequiredAt", RavenDocumentQuery.Now("+7d"))
    .ToList();
```
</TabItem>
<TabItem value="DocumentQuery_async" label="DocumentQuery_async">
...
// Using raw RQL - the offset string is passed directly to now()
List<Order> orders = session.Advanced
    .RawQuery<Order>("from Orders where RequiredAt <= now('+7d')")
    .ToList();

// Using parameter - the offset string is supplied via AddParameter().
// Note: with a parameter, repeated calls won't return 304 Not Modified.
List<Order> ordersParam = session.Advanced
    .RawQuery<Order>("from Orders where RequiredAt <= now($offset)")
    .AddParameter("offset", "+7d")
    .ToList();
```
</TabItem>
...
// Using raw RQL - the offset string is passed directly to now()
List<Order> orders = await asyncSession.Advanced
    .AsyncRawQuery<Order>("from Orders where RequiredAt <= now('+7d')")
    .ToListAsync();

// Using parameter - the offset string is supplied via AddParameter().
// Note: with a parameter, repeated calls won't return 304 Not Modified.
List<Order> ordersParam = await asyncSession.Advanced
    .AsyncRawQuery<Order>("from Orders where RequiredAt <= now($offset)")
    .AddParameter("offset", "+7d")
    .ToListAsync();
```
</TabItem>
...
// Offset string is passed as a literal string:
from Orders
where RequiredAt <= now('+7d')

// Offset string is passed as a parameter:
// Note: with a parameter, repeated calls won't return 304 Not Modified.
from Orders
where RequiredAt <= now($offset)
{ "offset": "+7d" }
```
</TabItem>
</Tabs>
...
* **Unsupported query contexts**  
  Both `now()` and `today()` are not supported in:
  * [Exploration queries](../../../indexes/querying/exploration-queries.mdx)
  * [Subscription queries](../../../client-api/data-subscriptions/creation/examples.mdx)    
  ...
  For example, the following queries are not allowed:
    ...
    <TabItem>       
    ```sql     
    // Not allowed: now() / today() are not supported in subscription RQL   
    from Employees
    where HiredAt <= now()
    ```
    </TabItem>
  ...
  [Filtering by an explicit date](../../../querying/filtering-query-results/filter-by-date-and-time.mdx#filter-by-an-explicit-date) is **fully supported** in both exploration and subscription queries.    
...
* **When the request runs inside an [Aggressive caching](../../../client-api/configuration/aggressive-caching.mdx) scope**,  
