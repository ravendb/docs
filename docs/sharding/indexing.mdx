...
* In this article:  
  * [Indexing in a sharded database](#indexing-in-a-sharded-database)  
  * [Map-Reduce indexes in a sharded database](#map-reduce-indexes-in-a-sharded-database)  
...
* Querying a sharded index is coordinated by the orchestrator, which combines results from all shards.    
  ...
  Learn more in [Client-server communication](./overview.mdx#client-server-communication).  
...
2. **At query time**:  
   ...
   See [order by and limit in a Map-Reduce query](./querying.mdx#order-by-and-limit-in-a-map-reduce-query) for details.
  
<Admonition type="note" title="">
Learn more about querying map-reduce indexes in a sharded database in [Sharding: querying](./querying.mdx).  
</Admonition>
...
* **Custom sorters**:  
  [Custom sorters](../querying/sorting-query-results/custom-sorters/overview.mdx) are not supported in a sharded database.  
...
* **Loading a document from another shard**:  
  ...
  handle this _null_ case **explicitly** in your index definition, as shown below:    
    ...
    by using the `$` syntax. Learn more in [Anchoring documents to a bucket](./administration/anchoring-documents.mdx).  
    </Admonition>
