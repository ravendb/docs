...
In both the JavaScript and RQL tabs above, the outer quotation marks delimit the string literal.  
...
const employees = await session
    ...
    .search("Notes", "College German", "OR")
    .all();
...
| **operator**      | string   | Logical operator to use between multiple terms in the same Search method. <br/> Can be `AND` or `OR`. <br/> <strong>Default</strong> is `OR`  |
