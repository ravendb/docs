...
* By default, RavenDB uses **internal configuration keys** to adjust logging.  
  ...
  * See a [list of logging configuration keys](configuration.mdx).
...
* To **determine whether to use an NLog configuration file or internal configuration keys**,  
  set the [Logs.ConfigPath](configuration.mdx#logsconfigpath) configuration key with -  
...
* **Temporary changes** can be applied at runtime, without requiring a server restart, using:
  ...
  * Studio's [Admin Logs view](admin-logs.mdx)  
  * the Client API [`SetLogsConfigurationOperation`](client-api.mdx#change-the-logging-configuration-at-runtime) method
...
To use a different level, set the [Logs.MinLevel](configuration.mdx#logsminlevel) configuration key.
...
To store log files in a different path, set the [Logs.Path](configuration.mdx#logspath) configuration key.  
...
See the full list of keys and their default values in the [logging configuration keys](configuration.mdx) page.
...
using the Client API [`SetLogsConfigurationOperation`](client-api.mdx#change-the-logging-configuration-at-runtime) method.
...
To use a custom file, set the [Logs.ConfigPath](configuration.mdx#logsconfigpath) configuration key to the full path of your NLog configuration file.
...
* Note: once a configuration file is used, the logging settings defined in it override all [internal configuration keys](configuration.mdx) values.  
...
To do this, define the plugin's NuGet package URL as a property of the [Logs.NuGet.AdditionalPackages](configuration.mdx#logsnugetadditionalpackages)
...
The log messages generated by the server can be viewed in real time from within Studio's [Admin Logs view](admin-logs.mdx).  
This view provides the following capabilities:

* Displays a continuous stream of RavenDB log entries in real time.
* Allows you to define the settings that determine the log entries written to disk.
