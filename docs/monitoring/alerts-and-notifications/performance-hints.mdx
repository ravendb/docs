...
* Most hints are triggered by configurable thresholds, set by the `PerformanceHints.*`
  [configuration keys](../../server/configuration/performance-hints-configuration.mdx).  
...
[configuration key](../../server/configuration/performance-hints-configuration.mdx#performancehintsmaxnumberofresults)
(`2048` by default).
...
[configuration key](../../server/configuration/performance-hints-configuration.mdx#performancehintstoolongrequestthresholdinsec)
...
[configuration key](../../server/configuration/performance-hints-configuration.mdx#performancehintsdocumentshugedocumentsizeinmb)
(`5` MB by default).
...
* **High fanout ratio**  
  ...
  [configuration key](../../server/configuration/performance-hints-configuration.mdx#performancehintsindexingmaxindexoutputsperdocument)
  (`1024` by default).  
...
* **Source document included in the index output**  
  ...
  [configuration key](../../server/configuration/performance-hints-configuration.mdx#performancehintsindexingalertwhensourcedocumentincludedinoutput)
  (`true` by default).  
...
* **Many `let` clauses**  
  ...
  [configuration key](../../server/configuration/performance-hints-configuration.mdx#performancehintsindexingmaxdepthofrecursioninlinqselect)
  (`32` by default).  
...
The server raises an indexing-references hint when an index loads the same
[related document](../../indexes/indexing-related-documents.mdx) or compare-exchange value
more times than the threshold set by the
`PerformanceHints.Indexing.MaxNumberOfLoadsPerReference`
[configuration key](../../server/configuration/performance-hints-configuration.mdx#performancehintsindexingmaxnumberofloadsperreference)
(`1024` by default).
...
* Unused-capacity hints are Info notifications, re-raised at most once a week while the
  condition persists, and dismissed automatically once the server uses all the machine's
  cores.  
...
* The `PerformanceHints.*` configuration keys, named in the sections above, are listed on
  the [Performance Hints Configuration](../../server/configuration/performance-hints-configuration.mdx)
  page.  
