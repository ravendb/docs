...
* Use a [server-wide backup task](../../overview.mdx#server-wide-backup) to define a **backup configuration** once and have RavenDB automatically apply it to **all your databases** as per-database backup tasks.
...
* This article explains how to create and manage the parent server-wide backup configuration and its derived database tasks.  
  For detailed guidance on backup configuration options (such as backup types, scope, scheduling, storage, retention policy, and encryption), see the [periodic database-backup task](./database-backup.mdx) article.  
...
* Define a **backup configuration** using a `ServerWideBackupConfiguration` instance.  
   - `ServerWideBackupConfiguration` inherits from [PeriodicBackupConfiguration](#backup-configuration-classes), which defines all configuration options used by both server-wide and regular database backup tasks.  
     See the [periodic database backup task](./database-backup.mdx) article for guidance about these options.  
   - `ServerWideBackupConfiguration` adds the following server-wide-specific property:  
     - `ExcludedDatabases` - A list of databases to [exclude](../../overview.mdx#excluding-databases-from-the-backup) from the server-wide backup.  
...
- The retrieved status reflects the **most recent completed backup run**, whether it was scheduled or [triggered manually](./database-backup.mdx#triggering-an-immediate-backup-operation).
...
[See GetPeriodicBackupStatusOperation syntax](./database-backup.mdx#task-management)
...
For syntax details on the operations used in the examples above to trigger, delay, and abort backup operations (`StartBackupOperation`, `DelayBackupOperation`, `KillOperationCommand`), see the [database backup operations syntax](./database-backup.mdx#operations).
...
<Admonition type="note" title="">
For syntax details on all configuration classes, enums, and backup destination settings used in the properties above, see the [database backup syntax](./database-backup.mdx#classes).
</Admonition>
...
* Define the backup configuration as explained in the [periodic database-backup task](./database-backup.mdx#a-defining-basic-task-options) article  
  ...
  * **Backups storage**  
    ...
    Each database task will automatically store its backups in a [subfolder named after the database](../../overview.mdx#server-wide-backups-storage), under the root storage location.  
  ...
  * **Encryption settings**  
    ...
    * When the database is **encrypted**:  
      **Snapshots** and **logical backups** are both **always encrypted using the database key**.  
      ...
      [See the server-wide encryption section in the overview for additional details.](../../overview.mdx#server-wide-encryption-settings)
      </Admonition>
