...
- When set to **Default**, backups are **stored locally** and exported from the local directory.  
  ...
  <Admonition type="note" title="">
  * If a [local folder](../backup/create/periodic-tasks/database-backup#e-choosing-where-to-store-the-backups) was selected to store backups locally, RavenDB will use it both for the transfer and as its permanent local backup location.  
  * If a local folder wasn't selected, RavenDB will use the 
    folder defined in its [Storage.TempPath](../server/configuration/storage-configuration.mdx#storagetemppath) setting.  
