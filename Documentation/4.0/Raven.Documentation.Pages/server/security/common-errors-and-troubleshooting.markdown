...
Firefox **sometimes** fails to import a certificate which is not password protected.
To overcome this issue, use the RavenDB Cli to [generate a password protected certificate](../../server/administration/cli#generateclientcert) or add a password to the current certificate by importing it to the OS store and exporting it back with a password.

Firefox **sometimes** fails to import a perfectly good certificate for no good reason and fails to give you a proper error. You can try to generate a new password protected certificate using the RavenDB Cli and import that instead.

If it didn't help, you can use any other client certificate (not generated by RavenDB) and have RavenDB trust it. See [trusting an existing certificate](../../server/administration/cli#trustclientcert).

If it still doesn't work, contact Mozilla or use another browser.
