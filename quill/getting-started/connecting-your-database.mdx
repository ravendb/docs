...
* Quill's internal database stays current by [following the changes](../overview.mdx#mirroring) your source
  ...
  Your database needs to be recording those changes. Quill turns change recording on where it can, and reports
  anything left to set up.  
...
* **The source database must be recording its changes.**
   * On PostgreSQL, a database administrator turns change recording on in the server's configuration. Quill then
     ...
     holds the `REPLICATION` attribute.
   * On SQL Server, Quill turns change recording on for you, as long as the database user is a member of the
     `db_owner` role.
...
[select the tables](#verifying-your-schema) that your app works with.
...
4. **Table name**  
   ...
   On PostgreSQL, a table is listed with a warning beside its name when its rows cannot be identified in delete
   events, which happens when the table has no primary key.
