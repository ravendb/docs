...
description: "RavenDB provides full ACID transactions per session SaveChanges call, with support for cluster-wide transactions across nodes."
sidebar_position: 1
---
...
* In this page:
  * [ACID storage](#acid-storage)  
  * [What is and what isn't a transaction](#what-is-and-what-isnt-a-transaction)  
  * [Working with transactions in RavenDB ](#working-with-transactions-in-ravendb)
     * [Single-node model](#single-node-model)
     * [Multi-master model](#multi-master-model)
     * [Cluster-wide transactions](#cluster-wide-transactions)
  * [ACID for document operations](#acid-for-document-operations)
  * [BASE for query operations](#base-for-query-operations)
...
* RavenDB doesn't support a transaction spanning __multiple__ HTTP requests. Interactive transactions are not implemented by RavenDB 
  (see [below](#no-support-for-interactive-transactions) for the reasoning behind this decision). 
  RavenDB offers [optimistic concurrency](../session/configuration/how-to-enable-optimistic-concurrency.mdx) feature to achieve similar behavior.  

* The [Client API Session](../session/what-is-a-session-and-how-does-it-work.mdx) is a pure Client API object and does not represent a transaction, 
...
In this mode, a user can perform all requested operations (read and/or write) in a single request.  
  ...
  * __Multiple writes__:  
  A batch of multiple write operations will be executed atomically in a single transaction when calling [SaveChanges()](../session/saving-changes.mdx).
  Multiple operations can also be executed in a single transaction using the low-level [SingleNodeBatchCommand](../commands/batches/how-to-send-multiple-commands-using-a-batch.mdx).  
  In both cases, a single HTTP request is sent to the database.

  * __Multiple reads & writes__:  
  Performing interleaving reads and writes or conditional execution can be achieved by [running a patching script](../operations/patching/single-document.mdx).  
  ...
  If you only need to modify a document in a transaction, [JSON Patch syntax](../operations/patching/json-patch-syntax.mdx) allows you to do that.
...
* __Multiple requests__:  
  ...
  Instead, users are expected to utilize [optimistic concurrency](../session/configuration/how-to-enable-optimistic-concurrency.mdx) to achieve similar behavior.  
...
conjunction with [optimistic concurrency](../session/configuration/how-to-enable-optimistic-concurrency.mdx), with much better performance.
...
then you must enable [optimistic concurrency](../session/configuration/how-to-enable-optimistic-concurrency.mdx) (turned off by default) across all sessions that modify those documents.
...
To achieve consistency across the entire cluster please refer to the [Cluster-wide transactions](#cluster-wide-transactions) section below.  

#### Replication conflicts

This is an important observation because you can get into situations where two clients wrote (even with [optimistic concurrency](../session/configuration/how-to-enable-optimistic-concurrency.mdx) turned on)
to the same document and both of them committed successfully (each one to a separate node). 
RavenDB attempts to minimize this situation by designating a [preferred node](../configuration/load-balance/overview.mdx#the-preferred-node) for writes for each database,  
...
However, handling replication conflicts is a consideration you have to take into account when using single-node transactions in RavenDB (see below for running a [cluster-wide transaction](#cluster-wide-transactions)).
...
If you care about avoiding lost updates, you need to ensure you have the conflict resolution script defined accordingly or use a [cluster-wide transaction](#cluster-wide-transactions).
...
RavenDB also supports [cluster-wide transactions](../session/cluster-transaction/overview.mdx).  
...
Cluster-wide transactions have the notion of [atomic guards](../../compare-exchange/atomic-guards.mdx) to prevent an overwrite of a document modified in a cluster transaction by a change made in another cluster transaction.
...
All of these constraints are guaranteed for each individual request made to the database when using a [Session](../session/what-is-a-session-and-how-does-it-work.mdx).  
In particular, every `Load` call is a separate transaction, and the [`SaveChanges`](../session/saving-changes.mdx) 
