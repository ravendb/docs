...
| **sample_object** | `str` | A [sample response object](../../../ai-integration/gen-ai-integration/overview.mdx#the-elements_json-schema) to format the AI model's replies by <br/> If both a `sample_object` and a `json_schema` are provided the schema takes precedence |
| **json_schema** | `str` | A [JSON schema](../../../ai-integration/gen-ai-integration/overview.mdx#the-elements_json-schema) to format the AI model's replies by <br/> If both a `sample_object` and a `json_schema` are provided the schema takes precedence |
