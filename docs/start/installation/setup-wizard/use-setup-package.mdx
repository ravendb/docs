...
description: "Initialize a RavenDB instance using a setup package ZIP file generated from a previous wizard session for consistent multi-node deployment."
sidebar_position: 8
---
...
* The **Use setup package** flow is the third option available in the [Choose setup method](./choose-setup-method.mdx) step.  
  It allows you to initialize a RavenDB instance using a configuration ZIP file generated in a previous session.
    
* In this article:
  * [When to use](#when-to-use)  
  * [Using an existing setup package](#using-an-existing-setup-package)
...
* When you are ready to apply a configuration package to your target server that was created using one of the following methods:
  * The [Create a package for external setup](./choose-setup-method.mdx#create-a-package-for-external-setup) flow
...
Run `run.ps1` (Windows) or `run.sh` (Linux) and accept the EULA, as explained in [How to run the Setup Wizard](./overview.mdx#how-to-run-the-setup-wizard).
...
In the [Choose setup method](./choose-setup-method.mdx) step, select the "Use setup package" option. 
