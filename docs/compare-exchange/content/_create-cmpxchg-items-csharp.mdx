...
* To create a new compare-exchange item, you must provide:  
  ...
  * You can optionally add **metadata** (a valid JSON object) to store extra information with the item.    
    ...
    Learn more in [Set expiration for compare-exchange items](../cmpxchg-expiration.mdx).  
 
* To modify an existing compare-exchange item, see: [Update compare-exchange item](../update-cmpxchg-item.mdx). 
...
* Note:  
   _PutCompareExchangeValueOperation_ is used for both **creating** and [modifying](../update-cmpxchg-item.mdx) compare-exchange items.  
