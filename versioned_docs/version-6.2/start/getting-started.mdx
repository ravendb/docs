...
The RavenDB server uses a [settings.json](../server/configuration/configuration-options.mdx#settings.json) file in each node `Server` folder to store the server-wide configuration options.  
...
- The folder where RavenDB server is running (to update [settings.json](../server/configuration/configuration-options.mdx#settings.json) by the [Setup Wizard](../start/installation/setup-wizard.mdx))
- The data folder ([`DataDir`](../server/configuration/core-configuration.mdx#datadir) setting)
- The logs folder ([`Logs.Path`](../server/configuration/logs-configuration.mdx#logs.path) setting)
