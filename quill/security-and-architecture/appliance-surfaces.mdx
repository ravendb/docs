---
title: "Appliance Surfaces"
sidebar_label: "Appliance Surfaces"
description: "How Quill routes and protects its network surfaces, including TLS, authentication, direct RavenDB access, and network limitations."
sidebar_position: 1
---
...
* Quill is a self-contained Docker appliance.   
  Its dashboard, API, public chat pages, and RavenDB server are all reached through a single TLS entry point  
  on port `443`.

* This article explains what runs inside the appliance, how incoming connections reach the correct service,  
  and how each network surface is protected.

* In this article:
  * [What is inside the appliance](#what-is-inside-the-appliance)
  * [How a connection is routed](#how-a-connection-is-routed)  
  * [The appliance hostnames](#the-appliance-hostnames)  
...
<Panel heading="What is inside the appliance">
...
* **nginx**  
  Acts as the appliance's reverse proxy and external TLS entry point.  
...
* **The web hostnames** (`dashboard.*`, `api.*`, and `public.*`) route to the Quill web application.  
  nginx terminates TLS using the appliance's wildcard server certificate and forwards the request as plain HTTP over the container's internal network.
...
<Panel heading="The appliance hostnames">
...
| --------------------------- | ------------------------------------------------------------- |
| `GET /healthz`              | Reports appliance health                                      |
...
* Quill does not currently provide an in-appliance command for rotating or revoking the Dashboard API key.
...
* Before activation, RavenDB starts without TLS on port `8080` and listens on all container interfaces.  
  The default configuration does not publish this port, so it is not part of the appliance's public surface.  
  Do not publish port `8080` or allow untrusted workloads to access the appliance's Docker network while activation is pending.
...
The appliance's setup package contains a wildcard server certificate for `*.<domain>`.  
This certificate covers the four appliance hostnames and RavenDB's `a.<domain>` routing alias.
...
If renewal cannot complete before the certificate expires, clients will reject TLS connections to every appliance hostname.
...
* nginx waits for the server certificate and does not start until the certificate is available.  
  A newly started appliance therefore does not accept connections on port `443` while activation is still in progress.
...
`db.<domain>` gives your applications direct access to the appliance's RavenDB server.  
...
* presents the appliance's wildcard server certificate;
...
* Direct access through `db.<domain>` therefore uses RavenDB's HTTPS API on port `443`.  
  Features that require RavenDB's native TCP protocol, such as data subscriptions, cannot connect from outside the appliance.
...
A Quill appliance runs one RavenDB server with a separate database for each app.  
...
### The appliance's admin certificate
...
* Four appliance hostnames expose the dashboard, API, public chat, and direct RavenDB surfaces.  
...
* The default appliance surface does not expose RavenDB's native TCP listener,  
  so features such as data subscriptions are unavailable externally.
* One wildcard server certificate covers the appliance hostnames and the `a.` alias.
* Quill's admin client certificate remains internal to the appliance and is not exposed to callers.
