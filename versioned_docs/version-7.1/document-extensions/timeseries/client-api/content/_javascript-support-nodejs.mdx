...
* Time series capabilities can be achieved via JavaScript when using the following methods:
  * [session.advanced.defer](../session/patch.mdx) - perform patch via the _session_
  * [PatchOperation](../operations/patch.mdx#patchoperation) - perform patch via a _store_ operation
  * [PatchByQueryOperation](../operations/patch.mdx#patchbyqueryoperation) - perform query & patch via a _store_ operation

* The server treats timestamps passed in the scripts as **UTC**, no conversion is applied by the client to local time.

* In this page:  
  * [JavaScript time series API methods](#javascript-time-series-api-methods)  
     * [timeseries - choose a time series](#section)  
     * [timeseries.append - append an entry](#section-1)  
     * [timeseries.delete - delete entries](#section-2)  
     * [timeseries.get - get entries](#section-3)  
  * [Examples](#examples)  
...
* This example shows a script that appends 100 entries to time series "HeartRates" in document "Users/john".  
  The script is passed to method [session.Advanced.Defer](../session/patch.mdx).
...
* This example shows a script that deletes time series "HeartRates" for documents that match the specified query.
  The script is passed to the [PatchByQueryOperation](../operations/patch.mdx#patchbyqueryoperation)  operation.
