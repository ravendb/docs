...
Each transformation script has its own transactional ID. When an ACL grant is  
missing, RavenDB raises an exception whose message names the missing resource  
and the operation, so the easiest way to read off the transactional ID's value  
is from the exception text.
