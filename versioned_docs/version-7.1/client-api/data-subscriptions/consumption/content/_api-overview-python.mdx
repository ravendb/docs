...
* In this page:  
   ...
   * [`SubscriptionBatch[_T]`](#subscriptionbatch_t)
   * [`SubscriptionWorker[_T]`](#subscriptionworker_t)
...
| **strategy**                             | `SubscriptionOpeningStrategy`<br/>(enum) | Configures how the server handles connection attempts from workers to a specific subscription task.<br/>Learn more in [worker strategies](../how-to-consume-data-subscription.mdx#worker-strategies).<br/>Default: `OPEN_IF_FREE`.                                                                                                                                      |
...
To start processing, you need to call the `run` function of the [SubscriptionWorker](#subscriptionworkert).
...
[failover](../../../configuration/load-balance/overview.mdx) to another node 
...
| `run`                                           | `Future[None]` | Call `run` to begin the worker's batch processing.<br/>Pass the batch processing delegates to this method<br/>(see [above](#run-the-subscription-worker)). |
### Events:
