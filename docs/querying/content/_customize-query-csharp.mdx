...
* Use `Customize()` to set the following customization options on a specific [Query](../overview.mdx).   
  Can be set for both **dynamic** and **index** queries.

* Each such customization can also be implemented via the [DocumentQuery](../document-query/what-is-document-query.mdx) API.
...
* Customization methods available:
  ...
  - [AfterStreamExecuted](#afterstreamexecuted)
  - [NoCaching](#nocaching)
...
* Learn more about streaming queries in [how to stream query results](../stream-query-results.mdx).
...
* For additional ordering options, see [Sort query results](../sorting-query-results/sort-query-results.mdx).      
...
* All queries in RavenDB provide results using an index, even when you don't specify one.  
  See detailed explanation in [Queries always provide results using an index](../overview.mdx#queries-always-provide-results-using-an-index).
...
* Note: This feature is Not available when [streaming the query results](../stream-query-results.mdx).  
