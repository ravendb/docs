...
5. Start the [Setup Wizard](./installation/setup-wizard/overview.mdx) by running `run.ps1` (or `run.sh` in Linux) in PowerShell or [disable the 'Setup Wizard' and configuring the server manually](./installation/manual.mdx).  
...
If you are interested in hosting the server in a Docker container, please read our [dedicated knowledge base](./containers/dockerfile/dockerfile-overview.mdx).
...
- [AWS Windows VM](./installation/setup-examples/aws-windows-vm.mdx)
- [AWS Linux VM](./installation/setup-examples/aws-linux-vm.mdx)
...
The [Setup Wizard](./installation/setup-wizard/overview.mdx) places it correctly automatically.  
...
- The folder where RavenDB server is running (to update [settings.json](../server/configuration/configuration-options.mdx#settingsjson) by the [Setup Wizard](./installation/setup-wizard/overview.mdx))
