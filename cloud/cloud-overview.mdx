...
{/* TODO: Screenshot needed — capture the login page (https://cloud.ravendb.net/user/login) showing the email field and the "Login via GitHub" and "Login via Google" buttons below it, separated by the "or" divider. */}
...
{/* TODO: Screenshot needed — capture the "Check your inbox" page that appears after attempting to sign in via GitHub or Google with an email that has no associated Cloud account. */}

**Possible errors**

If sign in cannot complete, a message is shown on the login page. The possible cases are:

| Message | Cause |
|---|---|
| **GitHub authentication failed** | The GitHub authorization step did not complete successfully. |
| **Google authentication failed** | The Google authorization step did not complete successfully. |
| **The selected account does not expose a verified email address** | The provider account does not expose a usable email address. Make sure the account has a verified primary email. |
| **This account is temporarily locked** | Too many recent failed sign-in attempts. Try again later. |
| **We could not sign you in** | A generic sign-in failure. Retry the sign in. |

!["Figure 10 - Login page with an OAuth error alert"](./assets/login-oauth-error-alert.png)

{/* TODO: Screenshot needed — capture the login page showing the warning alert displayed when an OAuth sign-in attempt fails (e.g. "GitHub authentication failed. Please try again."). */}
