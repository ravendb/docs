...
* Compare-exchange items are also suitable for storing shared or global values that aren't tied to a specific document -
  ...
  However, [unlike regular documents](#why-not-use-regular-documents-to-enforce-uniqueness),
...
* In this article:  
  ...
  * [Ways to create and manage compare-exchange items](#ways-to-create-and-manage-compare-exchange-items)  
  ...
  * [Why not use regular documents to enforce uniqueness](#why-not-use-regular-documents-to-enforce-uniqueness)  
...
* Each compare-exchange item contains: 
  ...
  * **Metadata** - Optional data that is associated with the compare-exchange item. Must be a valid JSON object.  
    ...
    Learn more in [compare-exchange expiration](../cmpxchg-expiration.mdx).  
...
* **Document Store Operations**  
  ...
  For example, see [Create items using a store operation](../create-cmpxchg-items.mdx#create-items-using-a-store-operation).
...
* **Cluster-Wide Sessions**  
  ...
  For example, see [Create items using a cluster-wide session](../create-cmpxchg-items.mdx#create-items-using-a-cluster-wide-session).     
...
* **Atomic Guards**  
  When creating _documents_ using a cluster-wide session, RavenDB automatically creates [Atomic Guards](../atomic-guards.mdx),  
...
* **Studio**  
  All compare-exchange items can also be managed from the **Compare-Exchange view** in the [Studio](../../studio/database/documents/documents-and-collections.mdx):  
...
* This compare-exchange item was [created as an operation](../create-cmpxchg-items.mdx#create-items-using-a-store-operation)
  rather than with a [cluster-wide session](../create-cmpxchg-items.mdx#create-items-using-a-cluster-wide-session).  
...
* When using cluster-wide sessions to handle documents, RavenDB automatically creates internal compare-exchange items,
  called [atomic guards](../atomic-guards.mdx), to enforce atomic document modifications.
