...
and [Restore from backup](../../backup/restore.mdx#restoring-from-backup-using-studio) operations.
...
If [Document revisions](../revisions/overview.mdx) are enabled, any addition or removal of attachments will create a new revision of the document.
...
It does not process or modify revisions.    
  * Revisions that reference an attachment as local will remain local forever, even if the attachment in the current (parent) document is
    [scheduled for remote uploading](./store-attachments/store-attachments-remote.mdx#schedule-existing-attachments-for-remote-upload) and later uploaded to remote storage.
