---
title: "RavenDB Kubernetes Operator: Secured Cluster Setup Guide"
...
see_also:
  - title: "Container Deployment Guide"
    ...
    source: "docs"
    path: "Getting Started > Containers"
  - title: "Container Deployment Guides"
    link: "start/containers/deployment-guides"
    source: "docs"
    path: "Getting Started > Containers"
  ...
  - title: "Certificate Renewal and Rotation"
...
**[Part 1: Setup & Operator Installation](#part-1-setup--operator-installation)**
...
**[Part 2: RavenDBCluster CRD](#part-2-ravendbcluster-crd)**
...
**[Part 3: External Access \- How Clients Reach the Cluster](#part-3-external-access---how-traffic-reaches-your-cluster)**
...
**[Part 8: Rolling Upgrades](#part-8-rolling-upgrades)**
...
<Image img={require("./assets/fromzero1.png")} alt="Architecture diagram showing the complete RavenDB Kubernetes deployment built throughout the guide series" />
...
<Image img={require("./assets/fromzero2.png")} alt="Architecture diagram showing Kubernetes lab setup with worker nodes distributed across availability zones" />
...
<Image img={require("./assets/fromzero3.png")} alt="Architecture diagram showing the RavenDB Operator installed with namespace, controller, webhooks, and CRD registered" />
...
With the `RavenDBCluster` contract now in place, we can move on to **Part 3**, where we focus on external access \- how traffic reaches your cluster, how URLs map to Kubernetes networking, and how the Operator wires ingress controllers or cloud load balancers into a secure RavenDB deployment.
...
### The Operator’s Perspective on External Access
...
### Ingress Controllers: One Entry Point, Many Routes
...
### Walking Through External Access
...
### MetalLB for Kind: Giving Your Ingress a Real External IP
...
<Tabs>
<TabItem value="aws-nlb" label="AWS (EKS)">
...
<Image img={require("./assets/fromzero4.png")} alt="AWS EC2 console showing three allocated Elastic IP addresses for RavenDB nodes A, B, and C" />
...
<Image img={require("./assets/fromzero5.png")} alt="Azure portal showing three public IP addresses allocated for RavenDB nodes A, B, and C" />
...
<Image img={require("./assets/fromzero6.png")} alt="Azure Load Balancer architecture diagram showing zone-redundant public IPs routing to Kubernetes services" />
...
<Image img={require("./assets/fromzero7.png")} alt="Architecture diagram showing completed external access configuration with load balancers and stable entry points" />
...
<Image img={require("./assets/fromzero8.png")} alt="Architecture diagram showing TLS configuration with Let's Encrypt certificates mapped to RavenDB nodes" />
...
<Image img={require("./assets/fromzero9.png")} alt="Architecture diagram showing DNS resolution configured for RavenDB nodes within the Kubernetes cluster" />
...
### Additional Volumes (Imports, Scripts, Backups)
...
<Image img={require("./assets/fromzero10.png")} alt="Architecture diagram showing storage layer fully defined with persistent volumes for data, logs, and auxiliary files" />
...
### Server certificate secrets (per-node)
...
<Image img={require("./assets/fromzero11.png")} alt="Architecture diagram showing all Kubernetes secrets created and ready for the RavenDBCluster manifest deployment" />
...
<Image img={require("./assets/fromzero12.png")} alt="Architecture diagram showing the operator reconciliation loop creating StatefulSets, Services, and other resources" />
...
### Watching the cluster form: the init job
...
<Image img={require("./assets/fromzero13.png")} alt="Architecture diagram showing completed cluster initialization with all nodes registered and cluster fully formed" />
...
<Image img={require("./assets/fromzero14.png")} alt="RavenDB Studio loaded in the browser showing the secured and clustered database management interface" />
...
<Image img={require("./assets/fromzero15.png")} alt="Architecture diagram showing the fully operational RavenDB cluster with certificates and bootstrap completed" />
...
### How the Operator Tells You the Truth
...
### Events: turning status changes into a timeline
...
<Image img={require("./assets/fromzero16.png")} alt="Kubernetes events timeline showing operator state changes from storage blocked through cluster ready" />
...
### What webhooks allow you to change
...
### The upgrade model: serial, gated, resumable
...
<Image img={require("./assets/fromzero17.png")} alt="RavenDB Studio About tab showing the current server version before the rolling upgrade" />
...
<Image img={require("./assets/fromzero19.png")} alt="RavenDB Studio About tab showing the updated server version after the rolling upgrade completed" />
...
<Image img={require("./assets/fromzero20.png")} alt="RavenDB Studio showing a healthy replicated database across all three nodes before sabotage" />
After Sabotage:

<Image img={require("./assets/fromzero21.png")} alt="RavenDB Studio showing the database in a degraded state after sabotaging nodes A and C" />
