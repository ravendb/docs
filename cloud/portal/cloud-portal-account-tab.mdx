...
You can add or remove new users with selected roles, as well as modifying existing users roles.

Users with role *Account with User Management* can modify roles of other users (except for *Account with User Management*).  
Users with role *Account with User Management* can **not** add or remove roles, that they are **not** assigned themselves.  

<Admonition type="note" title="Example" id="example" href="#example">
  User with roles **Account with User Management** and **Products** can assign the **Products** role to another user (that does not have a **Account with User Management** role).  
  User with roles **Account with User Management** and **Products** can not assign the **Billing** role to another user.
</Admonition>

Only *account owner* can modify roles of other users with *Account with User Management* role, as well as assign and remove this role.  
*Account owner* can modify any role to any user.
...
!["Account Tab - Invite a user"](./assets/portal-account-tab-invite-user-invite-button.png)
...
!["Account Tab - Remove a user"](./assets/portal-account-tab-invite-user-cancel-invitation-and-remove-user.png)
...
!["Account Tab - Edit a user"](./assets/portal-account-tab-edit-user-edit-button.png)

   **1**. To edit roles of an existing user, click the **Edit** button.

!["Account Tab - Edit a user"](./assets/portal-account-tab-edit-user-wizard-select-roles.png)

   **2**. Select new *account roles* for the existing user.

!["Account Tab - Invite a user"](./assets/portal-account-tab-edit-user-wizard-apply-changes.png)
