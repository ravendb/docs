...
The following example loads the `.pfx` into a `KeyStore`, connects to the app's RavenDB database,  
...
public final class QuillClientExample {
    public static void main(String[] args) throws Exception {
        String certificatePassword =
            System.getenv("QUILL_CLIENT_CERTIFICATE_PASSWORD");
        char[] certificatePasswordChars = certificatePassword == null
            ? new char[0]
            : certificatePassword.toCharArray();

        KeyStore clientStore = KeyStore.getInstance("PKCS12");
        try (FileInputStream certificateFile =
                 new FileInputStream("C:\\secrets\\quill-client.pfx")) {
            clientStore.load(certificateFile, certificatePasswordChars);
        }

        try (DocumentStore store = new DocumentStore()) {
            store.setUrls(new String[]{ "https://db.<domain>" });
            store.setDatabase("northwind-app");
            store.setCertificate(clientStore);
            store.setCertificatePrivateKeyPassword(
                certificatePasswordChars);
...
The `.pfx` in the downloaded archive is a PKCS#12 file, so `KeyStore.getInstance("PKCS12")` loads it directly.  
Pass the same password to both `KeyStore.load` and `setCertificatePrivateKeyPassword`. If the `.pfx` has no password, pass an empty character array to both.

The RavenDB Java client maps entity properties with Jackson's default naming, so `getName()` alone reads the JSON property `name`.  
Annotate the field with `@JsonProperty` whenever the mapped property name differs, as the capitalized `Name` does here.
...
The RavenDB Java client (`7.2.3`) does not define a CDC Sink member in `OngoingTaskType`,  
so `GetOngoingTaskInfoOperation` cannot request this task type from Java.  
