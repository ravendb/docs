...
* In this article:
   * [What happens when the server certificate expires](#what-happens-when-the-server-certificate-expires)
   * [Recover the server](#recover-the-server)
      * [Obtain a valid certificate](#obtain-a-valid-certificate)
      * [Install the new certificate](#install-the-new-certificate)
...
and `Enhanced Key Usage` fields. See [certificate requirements](./certificate-configuration.mdx#certificate-requirements)
for the full list.
...
cluster has no leader. The cluster elects a leader again once a [majority of its nodes](../../clustering/rachis/cluster-topology.mdx#leader)
...
Once the cluster is reachable again, see [certificate renewal and rotation](./certificate-renewal-and-rotation.mdx) to learn how to replace certificates across the cluster using Studio.
...
* RavenDB also publishes the certificate's expiration over [SNMP](../../administration/snmp/snmp-overview.mdx),
...
See [automatic Let's Encrypt renewals](./lets-encrypt-certificates.mdx).

If you provide your own certificate, renewing it before it expires is your responsibility.  
See [certificate renewal and rotation](./certificate-renewal-and-rotation.mdx).
