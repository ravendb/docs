...
![Data source](./assets/data-source-view-1.png)    
...
4. **Data source actions**  
   ...
   **Export configuration** appears only when Quill recognizes a supported source engine.  
   ...
   See [Data source actions](#data-source-actions) for instructions and the effects of each action.
      ...
      ![Data source](./assets/data-source-view-2.png)
...
![Data source - Sync performance](./assets/data-source-view-3.png)
...
2. **Recent writes**  
   ...
   For the app's database write usage over a selected period, see the **WRU** tile on the [Overview](./overview.mdx#activity) page.

3. **Errors**  
   Quill shows the higher of these two counts:  
     * Errors reported in the live batches.
     * Entries returned in the sync error list.

   The returned list contains up to 25 of the most recent errors, but the tile can exceed 25 when the live batches report more errors.  
    
   Some failures occur before a batch is created, so this tile can report errors even when every listed batch shows **Success**.  
...
![Data source - Sync errors](./assets/data-source-view-4.png)
...
![Data source - Collections](./assets/data-source-view-5.png)
