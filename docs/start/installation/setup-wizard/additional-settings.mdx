...
description: "Customize optional RavenDB setup settings including environment type, data directory, TCP ports, and certificate configurations."
sidebar_position: 5
---
...
* After configuring your cluster nodes, as described in [Node addresses](./configure-node-addresses.mdx),  
...
1. **Studio environment**:  
   ...
   **In the wizard, this option is available only when you choose a [Secure setup](./choose-security-option.mdx#choose-a-security-method) flow**.  
   If you choose the [Unsecured setup](./choose-security-option.mdx#don-t-use-certificate), this option will not appear in the wizard,  
...
2. **Admin client certificate expiration time**:  
   ...
   **This option is available only when you choose a [Secure setup](./choose-security-option.mdx#choose-a-security-method) flow** in the wizard.  
...
4. **Configure advanced options**:  
   ...
   Learn more below in [Configure advanced options](#configure-advanced-options).  
    
5. Click **"Continue"** to proceed to the next step: [Review setup](./review-setup.mdx).
    
6. Click **"Back"** to return to the previous step - [Node addresses](./configure-node-addresses.mdx).
...
2. **Setup certificate path**:  
   By default, RavenDB stores your **server certificate** (the `.pfx` file) directly under the extracted `Server` folder.
     ...
     When joining nodes to your cluster using a [setup package](./use-setup-package.mdx),  
...
3. **Logs directory**:    
   By default, RavenDB stores logs in the `Logs` directory under the extracted `Server` folder.
     ...
     This directory will be used on all nodes in the cluster.
     When joining nodes to your cluster using a [setup package](./use-setup-package.mdx),  
     make sure this directory exists on each node; otherwise, cluster setup may fail.  
    
     Learn more about the RavenDB logging system in [Logging](../../../server/troubleshooting/logging.mdx).      
