...
description: "Distribute RavenDB databases across cluster nodes with configurable replication factors, failover behavior, and node prioritization."
sidebar_position: 0
---
...
* In RavenDB, a database can be replicated across multiple nodes, depending on its [Replication Factor](#replication-factor).  
...
* Unless [Sharding](#sharding) is employed,
...
* In this page:  
  ...
  * [Dynamic Database Distribution](#dynamic-database-distribution)  
  * [Sharding](#sharding)  
...
Once the database is created by getting a [Consensus](../rachis/consensus-operations.mdx),  
the [Cluster Observer](./cluster-observer.mdx) begins monitoring the _Database Group_ in order to maintain this Replication Factor.
...
All `Members` have master-master [Replication](../replication/replication-overview.mdx) in order to keep the documents in sync across the nodes.
...
If any of the `Database Nodes` is down or partitioned, the [Cluster Observer](./cluster-observer.mdx) will recognize it and act as follows:  

1. If the time that is defined in [TimeBeforeMovingToRehabInSec](../../configuration/cluster-configuration.mdx#clustertimebeforemovingtorehabinsec) (default: 60 seconds) has passed  
   and the node is still unreachable, the node will be moved to a `Rehab` state.
   
2. If the node remains in `Rehab` for the time defined in [TimeBeforeAddingReplicaInSec](../../configuration/cluster-configuration.mdx#clustertimebeforeaddingreplicainsec) (default: 900 seconds),  
   a new database node will be automatically added to the database group to replace the `Rehab` node.

3. If the `Rehab` node is online again, it will be assigned a [Mentor Node](./highly-available-tasks.mdx#responsible-node) to update it with the recent changes.
