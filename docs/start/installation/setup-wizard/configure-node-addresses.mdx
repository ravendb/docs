...
description: "Define cluster topology and network addresses for each RavenDB node during setup, including HTTP and TCP endpoint configuration."
sidebar_position: 4
---
...
* After choosing how to secure your server, as described in [Choose security option](./choose-security-option.mdx),  
...
* In this article:
  * [Set up node addresses for a SECURE cluster](#set-up-node-addresses-for-a-secure-cluster)  
    * [Behind a firewall or load balancer](#behind-a-firewall-or-load-balancer)
    * [In a Docker container](#in-a-docker-container)
  * [Set up node addresses for an UNSECURED cluster](#set-up-node-addresses-for-an-unsecured-cluster) 
...
7. **External IP address/hostname**:    
   ...
   **Note**: You must ensure that traffic reaching this external address is properly forwarded to the node’s internal/private address; otherwise, the traffic will not reach your server.
...
10. **Add another node**:   
    ...
    Learn more in [Number of nodes](#number-of-nodes).  
11. **Save**:  
...
2. The label “(current node)” marks the node that will be started on this machine when you finish the Setup Wizard  
   (when in the [Set up a new cluster](./choose-setup-method.mdx#set-up-a-new-cluster) flow).  
   The remaining nodes can be started and joined to the cluster later using the
   [Use setup package](./choose-setup-method.mdx#use-an-existing-setup-package) method on their machines.
3. The number of nodes you can add depends on your license; when you reach the limit, this button is disabled.  
4. Click **"Continue"** to proceed to the next step - [Additional settings](./additional-settings.mdx).    
5. Click **"Back"** to return to the previous step - [Enter domain](./choose-security-option.mdx#enter-a-domain-name).
...
* Using the Setup Wizard to configure a Docker container introduces unnecessary networking and port-mapping complexities.
  Instead, it is much simpler to pass your configuration directly using Environment Variables or by mounting a pre-configured `settings.json` file.

* For detailed instructions on how to properly set up and configure a container, see [Docker image usage](../../containers/image-usage.mdx).
...
![Unsecured cluster](./assets/node-addresses-4.png)  
...
7. **Add another node**:   
    ...
    For production, it is recommended to start with at least 3 nodes.
    Learn more in [Number of nodes](#number-of-nodes).  
8. Click **"Continue"** to proceed to the next step - [Additional settings](./additional-settings.mdx).  
   The "Continue" button is enabled after you click "Save".
9. Click **"Back"** to return to the previous step - [Choosing a security option](./choose-security-option.mdx).
...
For a deeper look at deployment options and their trade-offs, see: [Deployment considerations](../deployment-considerations.mdx).   
