...
* A client with [Operator-level](../security/authorization/security-clearance-and-permissions.mdx) access can connect to the Traffic Watch WebSocket endpoint to receive a **live stream** of requests.  
...
* Independently, Traffic Watch can be configured to **log requests persistently to file** via the  
  [`TrafficWatch.Mode`](../configuration/traffic-watch-configuration.mdx#trafficwatchmode) configuration key (`Off` by default, set to `ToLogFile` to enable).  
  Both the live stream and log-to-file can be active at the same time.

* In this page:
  * [Accessing Traffic Watch](#accessing-traffic-watch)
...
**From Studio:**  
* Go to _Manage Server → Traffic Watch_ ([Operator-level](../security/authorization/security-clearance-and-permissions.mdx) access is required).  
...
<Admonition type="note" title="">
For the full list of configuration keys that control log-to-file filtering (by database, HTTP method, status code, change type, etc.), see [Configuration: Traffic Watch Options](../configuration/traffic-watch-configuration.mdx).
</Admonition>
...
* **CPU & memory overhead**: Negligible.  
  Traffic Watch logging is processed asynchronously by RavenDB's logging pipeline.  
  Request-handling threads are never blocked, so request latency and throughput are not affected.

* **I/O overhead**: Minor.  
  Traffic Watch entries are written to the same log file as all other RavenDB log entries.  
...
* **Disk storage**: Proportional to request volume.  
  Because Traffic Watch shares the log file with all other log data, high request volumes can increase log file size and cause more frequent log rollover.  
  When a log file reaches the size limit, a new log file is started. Old log files are deleted once they exceed the configured age or size limit.  
  These limits are set by the standard [logging configuration](../configuration/logs-configuration.mdx):  
  [`Logs.MaxFileSizeInMb`](../configuration/logs-configuration.mdx#logsmaxfilesizeinmb) (default: 128 MB),  
  [`Logs.RetentionTimeInHrs`](../configuration/logs-configuration.mdx#logsretentiontimeinhrs) (default: 72 hours),  
  [`Logs.RetentionSizeInMb`](../configuration/logs-configuration.mdx#logsretentionsizeinmb) (no limit unless set).  
  Old log files that exceed the configured retention limits are deleted automatically.

</ContentFrame>

<Admonition type="tip" title="">
Use the [filter configuration options](../configuration/traffic-watch-configuration.mdx) (databases, HTTP methods, status codes, change types, minimum duration, etc.)
