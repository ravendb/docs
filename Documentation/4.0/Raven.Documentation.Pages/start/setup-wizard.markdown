...
{WARNING: For production, you should be the owner of the domains and the certificates}
The automatic setup built into RavenDB is conviennent and easy, it takes care of all the details of setting up DNS, generating certificates,
renwals, etc. Because this automated process is managed by Hibernating Rhinos, it means that Hibernating Rhinos also has the ability to
generate valid certificates (and modify the DNS settings) for these domains. To be clear, that is not something that Hibernating Rhinos is
doing or ever planning to do, but it is a consideration to keep in mind while looking at the security of your system. 

The purpose of this feature is to make it easy for users to get setup and running with a minimum of fuzz, but we recommend that for actual
production deployments and for the highest level of security and control, you'll use your own certificates and domains, avoiding the need
to rely on third party for such a critical part of your security.
{WARNING/}
