...
A session created before the Dashboard API key is changed therefore remains valid until it expires after eight hours of inactivity.
Recreating the container with the same persistent volume does not invalidate it. See [Replacing an exposed key](#replacing-an-exposed-key).
...
The Dashboard API key issued during signup belongs to the signup account, not to an individual Quill domain.  
The same key is issued for every Quill domain registered under the same email address.

By default, each corresponding Quill instance is configured with the signup-issued key:
  ...
  * If the signup-issued key is exposed, treat every Quill instance that still accepts it as affected.  
    Replace the key and recreate the container on each affected instance. See [Replacing an exposed key](#replacing-an-exposed-key).
...
* With the default container layout, the web application stores its cookie-protection keys beneath  
  ...
  Recreating the container with that volume preserves the keys, so existing sessions remain valid until they expire after eight hours of inactivity.
  Continued activity renews the session.
...
* To replace an exposed key, update `QUILL_API_KEY` and recreate every affected container.  
  ...
  Neither restarting nor recreating the container with the same persistent volume invalidates existing browser sessions.
