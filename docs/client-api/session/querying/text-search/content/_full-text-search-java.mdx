...
In both the Java and RQL tabs above, the outer quotation marks delimit the string literal.  
...
List<Employee> employees = session
    .query(Employee.class)
     // * Pass 'SearchOperator.AND' as the third parameter
...
List<Employee> employees = session
    .query(Employee.class)
     // * Pass 'SearchOperator.OR' as the third parameter (or don't pass this param at all)
...
| **operator**      | `SearchOperator`   | Logical operator to use between multiple terms in the same Search method. <br/> Can be `SearchOperator.AND` or `SearchOperator.OR`. <br/> <strong>Default</strong> is `SearchOperator.OR` |

| Return Type            | Description                        |
|------------------------|------------------------------------|
| `IDocumentQuery<T>`    | The same object used for the query |
