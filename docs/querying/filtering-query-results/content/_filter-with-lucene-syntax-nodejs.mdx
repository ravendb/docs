...
* In the client API, use `whereLucene` from a query.  
...
* `whereLucene(fieldName, whereClause)` takes 2 parameters:
...
const companies = await session.advanced
    .rawQuery("from Companies where lucene(Name, $clause)")
...
const companiesWithSingleCharacterWildcard = await session
    .query({ collection: "Companies" })
...
In JavaScript, wrap the clause in single quotes (e.g. `'"sales representative"'`),  
...
const employeesProx = await session
    .query({ collection: "Employees" })
...
```javascript
class Companies_ByNameAndPhone extends AbstractJavaScriptIndexCreationTask {
    constructor() {
        super();
...
```javascript
const companies = await session
    .query({ indexName: "Companies/ByNameAndPhone" })
...
```javascript
class Companies_ByNameExact extends AbstractJavaScriptIndexCreationTask {
    constructor() {
        super();
...
const companies = await session
    .query({ indexName: "Companies/ByNameExact" })
...
Indexing a field with the `Exact` option uses the [default exact analyzer](../../../indexes/using-analyzers.mdx#using-the-default-exact-analyzer).  
...
<TabItem value="Query" label="Query">
```javascript
// The JavaScript string "\\(" becomes \( in the Lucene clause
const companies = await session
    .query({ collection: "Companies" })
