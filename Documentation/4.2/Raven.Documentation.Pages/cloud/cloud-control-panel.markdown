...
* In this page:  
  * [The Products Tab](../cloud/cloud-control-panel#the-products-tab)  
     ...
     - [Product Security](../cloud/cloud-control-panel#product-security)  
...
While there is only one Free product, you can pick your Development or Production product 
out of a whole tier of configurations.  
    ...
    - **Tier**  
      ...
      In short,  
       ...
       - A **Development instance** implements RavenDB's full feature set, and its hardware can be selected out of 7 configurations 
         ...
         a shared CPU and is unfit for production.  
...
* Entering your account  
  An account operator is given access to the account using a [single-access link](../cloud/cloud-overview#visiting-your-account), to make sure each visit is recorded.  

* Mutual Authentication  
  Accessing your database instance is always over HTTPS, using mutual authentication: the database identifies itself to 
  your client, and the client to the database using the provided X509 certificate.

  An [Operator certificate](../server/security/authorization/security-clearance-and-permissions#operator) is automatically 
  generated by the setup procedure when you create your instance, to allow this authentication and let you manage the instance.  
 ...
 {NOTE/}
  ...
  We recommend that you provide your applications with certificates of their own, with the permission they actually need, 
  and not use the operator certificate from your applications.

{NOTE: }
    When using a free instance, we recommend that you create your certificate locally and import it to your instance, 
    rather than let RavenDB generate it for you. This avoids consuming large amount of credits for the generation of
    the certificate.
{NOTE/}
