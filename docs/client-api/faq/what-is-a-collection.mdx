...
description: "Collections in RavenDB group documents by the @collection metadata property, enabling per-collection queries, indexes, and configuration."
sidebar_position: 0
---
...
* In this page:  
  * [Collection Name Generation](#collection-name-generation)
...
* The function that is responsible for tagging the documents can be overridden.  
  See: [Global Identifier Generation Conventions](../configuration/identifier-generation/global.mdx#findtypetagname-and-finddynamictagname). 
...
* **The @hilo Collection**
  * The ranges of available IDs values returned by [HiLo algorithm](../document-identifiers/hilo-algorithm.mdx) are per collection name.  
