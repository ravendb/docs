...
* To pause indexing for ALL indexes in the database use [StopIndexingOperation](../stop-indexing.mdx).

* In this page:
    * [Overview](#overview)
    * [Pause index example](#pause-index-example)
    * [Syntax](#syntax)
...
* When pausing the index from the **client**:  
  The index will be paused for the [preferred node](../../../../configuration/load-balance/overview.mdx#the-preferred-node) only, 
...
* Learn how to resume an index by a client here: [Resume index](../start-index.mdx)  
...
* Pausing the index is **Not a persistent operation**.  
  This means the paused index will resume upon either of the following:
    ...
    * The database is re-loaded (by disabling and then enabling it).  
      ...
      or using [ToggleDatabasesStateOperation](../../../server-wide/toggle-databases-state.mdx) by the client.

* [Resetting](../reset-index.mdx) a paused index will resume the normal operation of the index  
