...
* This article focuses on **storing attachments locally**.  
  To learn about remote storage, see [Store attachments in remote storage](../../todo).
...
* In this article
  ...
  * [Deduplicating attachments](../../../document-extensions/attachments/store-attachments/store-attachments-local#deduplicating-attachments)  
...
* To store multiple attachments, call the operation separately for each attachment you want to upload. 
...
<Panel heading="Deduplicating attachments">
    
* RavenDB avoids storing duplicate binary content by using content-based hashing.
   
* It computes a SHA-256 hash of the attachment's binary content to determine if the stream already exists on disk.    
  If multiple attachments (across any documents in the database) share the same hash, their binary stream is stored only once.
...
| **remoteParameters** | `RemoteAttachmentParameters` | Parameters for uploading the attachment to [Remote storage](../../../todo..).<br/>Set to _null_ or omit to store the attachment **locally**. |
| **changeVector**     | `string`                     | An optional change vector for concurrency control (`null` to skip check) |
