...
* In this page:
   * [Sample query with distinct](#sample-query-with-distinct)
   * [Paging with distinct](#paging-with-distinct)
   * [Count with distinct](#count-with-distinct)
      * [Performance cost and an alternative approach](#performance-cost-and-an-alternative-approach)
...
Please read the dedicated article about [paging through tampered results](../paging.mdx#paging-through-tampered-results).  
...
* Getting the distinct items' count can also be achieved by creating a [Map-Reduce](../../map-reduce-indexes.mdx) index 
...
* Using a Map-Reduce index is more efficient since computations are done during indexing time and not at query time. 
  The entire dataset is [indexed](../../creating-and-deploying.mdx) once,
