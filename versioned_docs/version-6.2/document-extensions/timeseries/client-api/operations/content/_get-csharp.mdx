...
* Use `GetTimeSeriesOperation` to retrieve entries from a **single** time series.  
  You can also retrieve entries from a single time series using the Session's [TimesSeriesFor.Get](../../session/get/get-entries.mdx) method.
...
| **returnFullResults** | `bool`      | This param is only relevant when [getting incremental time series data](../../../incremental-time-series/client-api/operations/get.mdx) |
...
* Details of class `TimeSeriesEntry ` are listed in [this syntax section](../../session/get/get-entries.mdx#syntax).
