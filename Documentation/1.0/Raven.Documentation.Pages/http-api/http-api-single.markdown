...
The only way a delete can fail is if [the etag doesn't match](http://ravendb.net/docs/http-api/http-api-comcurrency?version=1.0). If the document doesn't exist, a delete will still respond with a successful status code. 
