...
3. **Docker command**  
   ...
   * The dashboard API key, by contrast, belongs to the signup account and is normally shared by Quill instances
     registered under the same email address. See [One key per account](../security-and-architecture/operator-authentication.mdx#one-key-per-account).  
