...
description: "Define AI connection strings in RavenDB to connect to external AI providers for chat models and text embedding generation."
sidebar_position: 0
---
...
* While each task can have only one connection string,  
  you can define multiple connection strings to support different providers or configurations.  
  A single connection string can also be reused across multiple tasks in the database.

* AI connection strings can be created from **Studio** or from the **Client API**,  
  either as [Per-database connection strings](../../integrations/connection-strings/per-database/add-or-update-connection-string.mdx)
  or as [Server-wide connection strings](../../integrations/connection-strings/server-wide/add-or-update-connection-string.mdx).    
  For provider-specific configuration details, see the dedicated provider articles.    

---
 
* In this article:
  * [The AI connection strings view](../../ai-integration/connection-strings/overview.mdx#the-ai-connection-strings-view)
  * [Creating an AI connection string (from Studio)](../../ai-integration/connection-strings/overview.mdx#creating-an-ai-connection-string-from-studio)
...
3. Click **"Add new"** to create a new connection string.  
   See [Creating an AI connection string (from Studio)](../../ai-integration/connection-strings/overview.mdx#creating-an-ai-connection-string-from-studio) below.
...
* The **AI Connection Strings** view is used to define AI connection strings for the **current database**.  
* AI connection strings can also be defined **server-wide**, so the same provider and model configuration can be shared across databases.
  See [Add a server-wide connection string](../../integrations/connection-strings/server-wide/add-or-update-connection-string.mdx).

</Admonition>    

</Panel>

<Panel heading="Creating an AI connection string (from Studio)">
