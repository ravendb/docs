...
1. [Subscription Creation Options](../creation/api-overview.mdx#subscription-creation-options): The documents that will be received, it's filtering and projection.
...
It's possible to configure that a worker will wait for an existing connection to fail, and take it's place, or we can configure it to force close an existing connection etc. See more in [Workers interplay](../consumption/how-to-consume-data-subscription.mdx#workers-interplay).
