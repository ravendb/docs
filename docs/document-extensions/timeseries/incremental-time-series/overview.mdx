...
* **Incremental Time Series** are [time series](../overview.mdx) 
...
* Incremental Time series can be created and managed using dedicated [API methods](./client-api/session/overview.mdx) 
...
* **A Time Series That Counts**  
  ...
  [Counters](../../counters/overview.mdx) let us count, but not keep track of changes.  
  [Time series](../overview.mdx) let us record changes over time, 
...
* **Rollup Policies**  
   ...
   * Incremental:  
     ...
     It can be handled via [TimeSeriesFor](../client-api/session/append.mdx#timeseriesforappend), not 
     [IncrementalTimeSeriesFor](./client-api/session/overview.mdx#methods), 
     and its values can no longer be [Incremented](./client-api/session/increment.mdx).  
...
* The basic structure and behavior of incremental time series are similar 
  to those of [non-incremental time series](../overview.mdx#time-series-data).  
