...
* Read about the logic and rationale behind RavenDB's client certificates in [The RavenDB Security Authorization Approach](#the-ravendb-security-authorization-approach) to ensure that you configure them appropriately.  
...
* [Studio Certificate Management View](#studio-certificates-management-view)  
* [The RavenDB Security Authorization Approach](#the-ravendb-security-authorization-approach)  
  * [Authorization Levels in Client Certificates](#authorization-levels-in-client-certificates)  
  * [Partial Access to Database](#partial-access-to-database)  
* [Create and Configure Certificates](#create-and-configure-certificates)  
  * [List of Registered Certificates](#list-of-registered-certificates)  
  * [Generate Client Certificate](#generate-client-certificate)  
  * [Edit Certificate](#edit-certificate)  
* [Enabling Communication Between Servers: Importing and Exporting Certificates](#enabling-communication-between-servers-importing-and-exporting-certificates)  
  * [Export Server Certificates](#export-server-certificates)  
  * [Upload an Existing Certificate](#upload-an-existing-certificate)  
  * [Certificate Collections](#certificate-collections)  
  * [Private Keys](#private-keys)  
  * [Client Certificate Chain of Trust](#client-certificate-chain-of-trust)  
...
3. **Client certificate**  
   ...
   * **Generate client certificate**  
     [Create and configure](#generate-client-certificate) a new client certificate  
   * **Upload client certificate**  
     [Import a client certificate](#upload-an-existing-certificate) that was exported from another server so that they can communicate.  
4. **Server certificates**  
   ...
   * **Export server certificates**  
     [Download server certificates](#export-server-certificates) so that you can download and then import them into another server.  
   * **Replace server certificates**  
     [Replace server certificates](./certificate-renewal-and-rotation.mdx) by uploading another `.pfx` certificate.  
5. Status of the current server certificate. 
   * Click [Renew now](./certificate-renewal-and-rotation.mdx) to renew a server certificate's expiration period.  
...
6. Status of current client certificates active in this server. You can remove or [edit client certificates](#edit-certificate), 
   including configuring databases permissions and [authorization (security clearance) levels](../authorization/security-clearance-and-permissions.mdx#authorization-security-clearance-and-permissions) here. 
...
allowed.  This is why access levels of RavenDB databases in each cluster are highly customizable by [cluster admins](../authorization/security-clearance-and-permissions.mdx#cluster-admin) 
and [operators](../authorization/security-clearance-and-permissions.mdx#operator).  
...
* [Cluster Admin](../authorization/security-clearance-and-permissions.mdx#cluster-admin)  
  Full administrative access to clusters and databases within.  
* [Operator](../authorization/security-clearance-and-permissions.mdx#operator)  
  Admin access to databases, but not to modify the cluster.  
* [User](../authorization/security-clearance-and-permissions.mdx#user)  
  ...
  * ["User" authorization levels](#setting-user-access-levels) are also configured per database.  
 ...
 * [Using ETL for selective, one-way data transfer](#using-etl-for-selective-one-way-data-transfer)  
 * [Setting "User" Access Levels](#setting-user-access-levels)  
...
1. [Create](../../../studio/database/create-new-database/general-flow.mdx) a dedicated database that the public will be able to access.  
2. [Generate a client certificate](#generate-client-certificate) with "User" security clearance so that
...
3. If the dedicated database is on a different cluster than the source database: (This is optional. If both databases are on the same cluster, skip to step 4.)  
   * [Export (download) server certificates](#export-server-certificates) from the **destination server**.  
   * [Upload the .pfx certificate](#upload-an-existing-certificate) to the **source server** to enable the two to connect.  
    * While uploading, configure the certificate to give access to the target source database.  
4. Then set up an [ETL](../../ongoing-tasks/etl/raven.mdx) task from the source database to the exposed, destination database.  
   * Set up a Javascript [Transform script](../../ongoing-tasks/etl/basics.mdx#transform) in the ETL to automatically filter the information passed from the source to destination databases.  
   * After entering the script code, you can click the blue button to [test the script](../../ongoing-tasks/etl/test-scripts.mdx) before saving the ETL.  Once you click the red **Save** button, 
...
You can also control access by giving a client certificate a [User](../authorization/security-clearance-and-permissions.mdx#user) security clearance. 
With this clearance, you can set a different access level to each database. The three "User" access levels are:  

* User [Admin](../authorization/security-clearance-and-permissions.mdx#user)  
* [Read/Write](../authorization/security-clearance-and-permissions.mdx#section-1)  
* [Read-Only](../authorization/security-clearance-and-permissions.mdx#section-2)  
  * Learn more about the [Read-Only access level here](../../../studio/server/certificates/read-only-access-level.mdx).  

<Admonition type="note" title="">

This approach is similar to the HR Manager and Customers certificates in the example given [above](#authorization-levels-in-client-certificates). 
It enables developers to control access levels by configuring client certificates.  

</Admonition>

* To learn how to configure each client certificate's database permissions and authorization levels via the RavenDB Studio GUI, see [Create and Configure Certificates](#create-and-configure-certificates).  
* To learn how to configure client certificates via CLI, see [Authentication: Client Certificate Usage](./client-certificate-usage.mdx#authentication-client-certificate-usage)
...
you can grant different [access levels](../authorization/security-clearance-and-permissions.mdx#authorization-security-clearance-and-permissions) 
...
3. **Security Clearance**  
   [Authorization level](../authorization/security-clearance-and-permissions.mdx#authorization-security-clearance-and-permissions) 
...
3. **Security Clearance**  
   Set authorization level for this certificate. Read about [Security Clearance](../authorization/security-clearance-and-permissions.mdx#authorization-security-clearance-and-permissions) 
...
6. **Database Permissions**  
   Configure allowed databases and ["User" access level](../authorization/security-clearance-and-permissions.mdx#user) for each database.  
...
a. **Export** ([download](#export-server-certificates)) the server certificate **from the source server**.  
b. **Upload** ([import](#upload-an-existing-certificate)) the downloaded certificate **into the destination server**.  
...
a .pfx [collection](#certificate-collections) will be exported.
...
2. **Security Clearance**  
   Set authorization level for this certificate. Read about [Security Clearance](../authorization/security-clearance-and-permissions.mdx#authorization-security-clearance-and-permissions) to choose the appropriate level.  
3. **Certificate file**  
...
4. **Database permissions**  
   ...
   If you choose *User* security clearance, you can give access to specific databases on the server and configure [User](../authorization/security-clearance-and-permissions.mdx#user) authorization levels for this certificate.  
...
* All of the operations which are described below are also available in Command Line Interface (CLI).  
  - Be sure to configure the `SecurityClearance` for each client certificate because the default is [cluster admin](../authorization/security-clearance-and-permissions.mdx#cluster-admin) which has full access.
  - There are CLI-based means to [generate](./client-certificate-usage.mdx#example-i---using-the-ravendb-cli) and [configure client certificates in Windows](./client-certificate-usage.mdx#example-ii---using-powershell-and-wget-in-windows).  
  - [Linux](./client-certificate-usage.mdx#example-iii--using-curl-in-linux) developers can use this cURL command sample.  
### Private Keys
...
same certificate authority. This is accomplished using a [public key pinning hash](./certificate-renewal-and-rotation.mdx#implicit-trust-by-public-key-pinning-hash).  
</Admonition>
