        ...
        public async Task LoadingEntitiesXY()
		{
			using (var store = new DocumentStore())
			{
			    ...
			    using (var asyncSession = store.OpenAsyncSession())
			    {
			        ...
			        Dictionary<string, Employee> employees = await asyncSession.LoadAsync<Employee>(new[] { "employees/1", "employees/2", "employees/3" });
			    ...
			    using (var asyncSession = store.OpenAsyncSession())
			    {
			        ...
			        IAsyncEnumerator<StreamResult<Employee>> enumerator = await asyncSession.Advanced.StreamAsync<Employee>("employees/");
			    ...
			    using (var asyncSession = store.OpenAsyncSession())
			    {
			        ...
			        using (var outputStream = new MemoryStream())
			        {
			            await asyncSession.Advanced.LoadStartingWithIntoStreamAsync("employees/",outputStream);
			        }
