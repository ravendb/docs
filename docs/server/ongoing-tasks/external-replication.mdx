...
* This ongoing task replicates **one-way**, from the source to the destination.  
  For additional functionality such as filtration and two-way replication consider [Hub/Sink Replication](./hub-sink-replication.mdx).
...
* Server and cluster level features:
    ...
    * [Conflict resolution scripts](../clustering/replication/replication-conflicts.mdx#conflict-resolution-script)
    ...
    * [Identities](../kb/document-identifier-generation.mdx#identity-id)
    * Ongoing tasks
        * [ETL](./etl/basics.mdx)
...
* Two databases that have an External Replication task defined between them will detect and resolve document
  [conflicts](../clustering/replication/replication-conflicts.mdx) according to each database's conflict resolution policy.
* It is recommended to have the same [policy configuration](../clustering/replication/replication-conflicts.mdx#configuring-conflict-resolution-using-the-client)
