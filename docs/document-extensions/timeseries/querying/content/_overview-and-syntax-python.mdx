...
* Querying time series data is native to RavenDB's queries.  
  Clients can express time series queries in high-level LINQ expressions or directly in [RQL](../../../../querying/rql/what-is-rql.mdx).

* Queries can be executed as dynamic queries or over [time series indexes](../../indexing.mdx).  

* In this page:  
  * [Time series query capabilities](#time-series-query-capabilities)
  * [Server and client queries](#server-and-client-queries)  
  * [Dynamic and index queries](#dynamic-and-index-queries)  
  * [Scaling query results](#scaling-query-results)  
  * [RQL syntax](#rql-syntax)  
     * [`select timeseries` syntax](#section)  
     * [`declare timeseries` syntax](#section-1)  
  * [Combine time series and custom functions](#combine-time-series-and-custom-functions)  
  * [Use Studio To experiment](#use-studio-to-experiment)  
...
* [Choose a range of time series entries](../choosing-query-range.mdx) to query from.  
* [Filter](../filtering.mdx) time series entries by their tags, values and timestamps. 
* [Aggregate](../aggregation-and-projections.mdx)  time series entries into groups by a chosen time resolution,  
...
* Select entries by various criteria, e.g. by the min and max values of each aggregated group,  
  and [project](../aggregation-and-projections.mdx) them to the client.  
* Calculate [statistical measures](../statistics.mdx): the percentile, slope, or standard deviation of a time series.  
...
* **Dynamic queries**:  
  ...
  * Use dynamic queries when time series you query are not indexed,  
    or when you prefer that RavenDB would choose an index automatically. See [queries always use an index](../../../../querying/overview.mdx#queries-always-provide-results-using-an-index).  
    E.g. -
...
* **Index queries**:
  * Static time series indexes can be created by clients (or using Studio).  
    To learn how to create such indexes, see [indexing time series](document-extensions/timeseries/indexing.mdx).  
  * Examples of querying a static time series index can be found in [querying time series indexes](../using-indexes.mdx).
...
* Note the generated RQL, where the custom function is translated to a [custom JavaScript function](../../../../querying/rql/what-is-rql.mdx#declare).
