...
* RavenDB can also be monitored via protocols like [SNMP](../integrations/snmp/overview.mdx) and
  [OTLP](../integrations/open-telemetry.mdx) (used by OpenTelemetry), as well as monitoring tools like [Prometheus](../integrations/prometheus.mdx).  
...
* [Cluster Dashboard](../../monitoring/cluster-dashboard/overview.mdx): live per-node CPU, memory, traffic, indexing, and storage metrics.
...
* [IO Stats](../../monitoring/performance-views/io-stats.mdx#server-wide-io-stats): live charts of the server's disk writes, flushes, and syncs.
...
listed below or via the [client API](client-api.mdx).
...
* [Stats view](stats-view.mdx): the counts of the database's documents and document extensions, per-node details like the database ID and the size on disk, and the indexing counters of each index.
...
* [Storage Report](../../monitoring/performance-views/storage-reports.mdx): the database's disk space usage, broken down by component.
* [IO Stats](../../monitoring/performance-views/io-stats.mdx#database-io-stats): live charts of the database's disk writes, flushes, and syncs.
