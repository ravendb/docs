...
2. **Set the number of revisions to keep** <a id="limit-revisions"/>  
   ...
   When you define a new configuration, this option is enabled by default and set to 1000
   (keeping the latest 1000 revisions).  
   If your license limits the number of revisions that you can keep to a lower number than 1000,
   the lower limit becomes the default instead.  

   If no limit is set on the number of revisions to keep, Studio shows a warning:  
   ![Revisions number warning](./assets/document-revisions_revisions-number_warning.png)  

   An **age limit** alone does not clear this warning, since the age limit does not cap
