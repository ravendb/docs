...
See [Secret Key Management](./secret-key-management.mdx) for more details on this topic.

## Enabling Server Store Encryption

This is an offline operation which should be performed only once using the [rvn tool](../../administration/cli.mdx). 
...
[Secret Key Management](./secret-key-management.mdx) to learn about secret protection in RavenDB.
...
[here](./encryption-at-rest.mdx#how-does-it-work). You still need to explicitly setup each database as encrypted
