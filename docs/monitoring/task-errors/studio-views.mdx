...
* Hover an indicator to trigger a popup summary of tasks whose health currently matches 
  the selected state.  
* The summary details the nodes that each task runs on, and the number of errors each 
  task raised per node.  
...
5. **Errors table**  
    ...
    * **`Error step` column**  
      ...
      `Transformation`, `Load`, `Model Inference`, `Persistence`, or `Unknown`.  
