...
* **Numbers** - integer and floating-point values can be compared.  
  RavenDB converts between `Long` and `Double` when evaluating the condition,  
  so for example, `1` and `1.0` compare as equal.    

* **Strings** - all string value comparisons are **case-insensitive** (ordinal, ignore-case).  
  For example, a `$status` parameter equal to `"active"` matches the literal `'ACTIVE'`.    

* **Booleans** - only the equality operators `==` and `!=` are supported.  
  Using an inequality operator (`<`, `<=`, `>`, `>=`) on a boolean throws a `RavenException`.

    ```sql
    // Valid - RavenDB converts between Long and Double:
    where when($quantity >= 10, ...)

    // Valid - strings compare case-insensitively:
    where when($status == 'active', ...)

    // Valid - booleans with == / !=:
    where when($includeArchived == false, ...)

    // Throws RavenException - inequality on a boolean:
    where when($includeArchived > false, ...)

    // Throws RavenException - incompatible types ($quantity is a number):
    where when($quantity == 'ten', ...)
    ```
