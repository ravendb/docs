...
description: "RavenDB client runs speed tests across cluster nodes to determine the fastest node for read requests when using Fastest Node read balance behavior."
sidebar_position: 2
---
...
* In RavenDB Client API, if the [Read Balance Behavior](../configuration/load-balance/read-balance-behavior.mdx) is configured for the _Fastest Node_, 
