...
* RavenDB supports fast and efficient querying through indexes,
  which are powered by either [Lucene](http://lucene.apache.org/) or [Corax](../search-engine/corax.mdx),  
...
During the [indexing process](../../studio/database/indexes/indexes-overview.mdx#2-indexing-process),
...
enabling [Full-text search](../querying/searching.mdx).
...
When running a [Full-text search on a static-index](../querying/query-index.mdx),  
...
2. Behavior is also different when making a full-text search with wildcards in the search terms.  
   This is explained in detail in [Searching with wildcards](../querying/searching.mdx#searching-with-wildcards).
...
Learn more in [Sorting and Collation](../sorting-and-collation.mdx#collation).
...
* When no analyzer is explicitly assigned to an index-field in the index definition,  
  RavenDB will use its [Default Analyzers](#ravendb) to process and tokenize the content of a field.
...
* If you want RavenDB to use the default analyzers, see [RavenDB's default analyzers](#ravendb).
...
* The available **Indexing Behavior** values are:
    * `Exact`
    * `Search`
    * `No` - This behavior [disables field indexing](#disabling-indexing-for-index-field).
...
* Use the `No` indexing behavior option to disable indexing of a particular index-field.  
  In this case:
    ...
    * The field will still be accessible for extraction when [projecting query results](../querying/projections.mdx).

* This is useful when you need to [store the field data in the index](../storing-data-in-index.mdx) and only intend to use it for query projections.
