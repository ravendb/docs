...
* RavenDB uses **Indexes** in order to be able to answer queries about your data.
...
* **Static Indexes**:  
  ...
  These computations are run during the indexing-process and not at query time.  
{NOTE/}
...
* In a multiple-nodes cluster, indexing can be configured to occur either in _Rolling deployment Mode_ (one node at a time) 
