...
* Use `Customize()` to set the following customization options on a specific [Query](../how-to-query.mdx).   
...
* A query can also be customized on the Store or Session level by subscribing to `OnBeforeQuery`.  
  Learn more in [Subscribing to Events](../../how-to/subscribe-to-events.mdx). 
...
* Learn more in [how to stream query results](../how-to-stream-query-results.mdx).
...
* Learn more in [disable caching per session](../../configuration/how-to-disable-caching.mdx).
...
* By default, the [Session](../../what-is-a-session-and-how-does-it-work.mdx) tracks all changes made to all entities that it has either loaded, stored, or queried for.

* You can use the `NoTracking` customization to disable entity tracking.  

* See [disable entity tracking](../../configuration/how-to-disable-tracking.mdx) for all other options.
...
* Note:  
  ...
  * [Project dynamic query results](../how-to-project-query-results.mdx)
...
* More ordering options are available in this [Sorting](../sort-query-results.mdx) article.  
...
* Learn more in [how to include query timings](../debugging/query-timings.mdx).
...
* All queries in RavenDB provide results using an index, even when you don't specify one.  
  See detailed explanation in [Queries always provide results using an index](../how-to-query.mdx#queries-always-provide-results-using-an-index).
...
* Note: This feature is Not available when [streaming the query results](../how-to-stream-query-results.mdx).  
