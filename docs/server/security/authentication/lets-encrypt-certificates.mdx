...
description: "Secure your RavenDB server with free Let's Encrypt TLS certificates, including automatic renewal and DNS challenge configuration."
sidebar_position: 4
---
...
The [Setup Wizard Walkthrough](../../../start/installation/setup-wizard/choose-security-option.mdx#generate-let-s-encrypt-certificate) explains how to obtain a free Let's Encrypt certificate for your server or cluster.
...
You should also download the appropriate DNS plugin for certbot. This example uses [Amazon's Route53](https://certbot-dns-route53.readthedocs.io/en/stable/), but [many other services](https://certbot.eff.org/docs/using.html#dns-plugins) are supported.
...
the standard output. [See this example](../../../server/security/authentication/certificate-configuration.mdx) of how to write a file to standard output in 
Powershell.  
...
If two different RavenDB clusters are communicating securely, and the source cluster has its certificate renewed, the destination cluster could still trust this new certificate - provided that the new certificate is signed with the same private key as the original, and was issued by the same certificate authority.  
This is accomplished using a [public key pinning hash](../../../server/security/authentication/certificate-renewal-and-rotation.mdx#implicit-trust-by-public-key-pinning-hash).  
</Admonition>

## Manual Renewal

When your server uses a Let's Encrypt certificate obtained via the Setup Wizard, you can trigger the certificate renewal process manually in two ways:

* Go to the certificate view in Studio and click `Renew` on the server certificate.
* Run the following command via [`rvn admin-channel`](../../../server/administration/cli.mdx#rvn):

```
ravendb> triggerCertificateRefresh [-replaceImmediately]
```

Both methods trigger the same certificate replacement process described [above](../../../server/security/authentication/lets-encrypt-certificates.mdx#automatic-renewal-for-lets-encrypt-certificates-obtained-via-ravendb).

If a node is down when renewal is triggered, the cluster will complete the operation without that node. **When bringing that node up, the certificate must be replaced manually.**
