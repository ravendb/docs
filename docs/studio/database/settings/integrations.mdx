...
description: "Define credentials that third-party clients, like Power BI connecting over the PostgreSQL protocol, must provide to access a RavenDB database, in the Studio Integrations view."
sidebar_position: 14
---
...
* Use the **Integrations view** to define credentials that third-party clients must provide 
  when connecting to RavenDB.  

* Providing these credentials is required only when RavenDB runs as a 
  [secure server](../../../server/security/overview.mdx).  
...
Define the credentials that clients connecting over the 
