...
* In this page:
    * [Add a RabbitMQ connection string](#add-a-rabbitmq-connection-string)
        * [Example](#example)
        * [Syntax](#syntax)
    * [Add a RabbitMQ ETL task](#add-a-rabbitmq-etl-task)
    ...
    * [The transformation script](#the-transformation-script)
        ...
        * [Available method overloads](#available-method-overloads)
...
<TabItem value="add_rabbitMq_connection_string" label="add_rabbitMq_connection_string">
...
<TabItem value="rabbitMq_con_str_settings" label="rabbitMq_con_str_settings">
<CodeBlock language="csharp">
...
* For more details about the script and the `loadTo` method overloads, see the [transformation script](#the-transformation-script) section below.
...
The [basic characteristics](../basics.mdx) of a RabbitMQ ETL script are similar to those of other ETL types.  
...
The two methods are equivalent, offering alternative syntax:
  ...
  * **`loadTo('ExchangeName', obj, 'routingKey', {attributes})`**  
    ...
    * Separating the target name from the `loadTo` command makes it possible to include symbols like `'-'` and `'.'` in target names.
      ...
      | **attributes**   | object  | An object with [CloudEvents attributes](./overview.mdx#cloudevents).                |
