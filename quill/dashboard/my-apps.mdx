...
5. **Add app**  
   Click **Add app** to open the [wizard](../getting-started/connecting-your-database.mdx#starting-the-add-new-app-wizard)
   for connecting a source database and creating a new app.
...
2. **Conversations**  
   The number of conversations users started with agents across all apps.  
...
5. **WRU**  
   **Write Request Unit (WRU)** is the billing unit Quill uses for metered write activity.  
   This tile totals WRU reported for the RavenDB databases belonging to the current apps.
     * It does not include **@system** configuration-storage usage, usage from other Quill deployments sharing the license, or historical databases that no longer belong to a current app.
     * For the billing rules and examples of writes that count, see [Write usage (WRU)](usage.mdx#write-usage-wru).
...
3. **Agents**  
   ...
   See [App: Agents view](../dashboard/manage-an-app/app-agents-view.mdx).
...
7. **Add app**  
   Click **Add app** to open the wizard for [connecting a source database](../getting-started/connecting-your-database.mdx#starting-the-add-new-app-wizard),
   [mapping the tables](../getting-started/mapping-your-tables.mdx) Quill will mirror,  
