...
4. The _destination_ sends back a heartbeat message with both the latest `ETag` he got from the _source_ and the current [Change Vector](./change-vector.mdx) of the database.  
...
RavenDB [Database Group](../distribution/distributed-database.mdx#distributed-database) is a fully connected graph of replication channels, meaning that if there are `n` nodes in a `Database Group` there are `n*(n-1)` replication channels.  
