...
* RavenDB queries are handled through indexes.  
  When a query does not match an existing index, RavenDB can create an [auto-index](../creating-and-deploying.mdx#auto-indexes) for it.  
  The selected search engine determines which engine is used when new auto or [static indexes](../creating-and-deploying.mdx#static-indexes) are created.
...
* **The default search engine depends on your license.**  
  ...
  * All other license types, such as  _Professional_ and _Enterprise_, default to **Lucene**.    
  ...
  Explicitly [selecting the search engine](#selecting-the-search-engine) overrides this license-based default.
    
---
    
* In this article:  
   * [Selecting the search engine](#selecting-the-search-engine)  
      * [Server wide](#select-search-engine-server-wide)  
      * [Per database](#select-search-engine-per-database)  
      * [Per static index](#select-search-engine-per-static-index)  
   * [Unsupported features](#unsupported-features)  
   * [Handling complex JSON objects](#handling-complex-json-objects)  
   * [Compound fields](#compound-fields)  
   * [Limits](#limits)  
   * [Index training: Compression dictionaries](#index-training-compression-dictionaries)
...
Use these configuration options to select the search engine:
   ...
   * For additional Corax configuration options, see [Configuration options](#configuration-options).
...
* To make an existing index use a different engine, [reset the index](../../studio/database/indexes/indexes-list-view.mdx#indexes-list-view---actions) after changing the relevant search engine setting.
...
Some features are available only with Corax:    

* **Vector search**  
  [Vector search](../../ai-integration/vector-search/overview.mdx) is supported only by Corax.  
...
* **Static indexes with vector fields**  
  A static index that defines vector fields must use Corax.  
  If such an index is configured to use Lucene, deploying it fails with a `NotSupportedException`.

* **Auto-index to static-index conversion**  
  When RavenDB converts an auto-index with vector fields to a static index definition,  
  the resulting static index is set to Corax as well, regardless of the default search engine configuration.
    
* **Control null placement when sorting**  
  The `order by ... nulls first` and `order by ... nulls last` clauses are supported only by Corax.  
  RavenDB does not automatically select Corax for these clauses;  
  running such a query against a Lucene index throws an `InvalidQueryException`.
    
    Learn more in [Control position of null values](../../querying/sorting-query-results/sort-query-results.mdx#control-position-of-null-values).  
    For spatial sorting, see [Control null placement when sorting by distance](../../client-api/session/querying/how-to-make-a-spatial-query.mdx#control-null-placement-when-sorting-by-distance).
...
* Setting a [boost factor on an index-field](../boosting.mdx#assign-a-boost-factor-to-an-index-field) is not supported.  
  Note that [boosting the whole index-entry](../boosting.mdx#assign-a-boost-factor-to-the-index-entry)
  and [query-time boosting](../../client-api/session/querying/text-search/boost-search-results.mdx) with `boost()` **are supported**.
...
* [Custom sorters](../../querying/sorting-query-results/custom-sorters/overview.mdx) are not supported.  
    
#### Complex JSON properties:

Complex JSON properties cannot currently be indexed and searched by Corax.  
Read more about this in [Handling complex JSON objects](#handling-complex-json-objects) below.  

#### Unsupported `where` methods:

* [lucene()](../../querying/filtering-query-results/filter-with-lucene-syntax.mdx)  
* [intersect()](../querying/intersection.mdx) is not supported.  
...
* **Corax** does not support indexing the whole complex JSON object as a single text value,
  ...
  see [How Corax handles complex fields while indexing](#how-corax-handles-complex-fields-while-indexing) below.

    To work with complex objects, use one of the following approaches:
    
    1. [Index simple properties from the object](#1-index-simple-properties-from-the-object)
    2. [Store the complex field for projection only](#2-store-the-complex-field-for-projection-only)
    3. [Serialize the complex object explicitly](#3-serialize-the-complex-object-explicitly)
    4. [Use Lucene to index the whole object as JSON text](#4-use-lucene-to-index-the-whole-object-as-json-text)   
...
[disable indexing for the field](../using-analyzers.mdx#disabling-indexing-for-index-field) and store it.
[Projection queries](../querying/projections.mdx#projections-and-stored-fields) would be able to project it.
...
* [Query paging](../querying/paging.mdx) over Corax indexes supports skipping more than `int.MaxValue` results.  
...
* Compound fields have additional constraints, including exactly **2 fields** per compound field
  ...
  Learn more in [Compound fields: Constraints and value limits](#constraints-and-value-limits).
...
you can [reset the index](../../studio/database/indexes/indexes-list-view.mdx#indexes-list-view---actions) to train a new dictionary.
