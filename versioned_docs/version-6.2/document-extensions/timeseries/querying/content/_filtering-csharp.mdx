...
* In addition to limiting time series query results by specifying the [range of entries](../choosing-query-range.mdx) to retrieve,
...
* In this page:  
  ...
  * [Filter by referenced document](#filter-by-referenced-document)

</Admonition>
## Filter by value

* A time series entry can have up to 32 [values](../../overview.mdx#values).
...
* A time series entry can have an optional [tag](../../overview.mdx#tags).
...
* A time series entry's [tag](../../overview.mdx#tags) can contain the **ID of a document**.
