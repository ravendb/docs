...
The following example loads the `.pfx`, connects to the app's RavenDB database, and queries the mapped `Products` collection.
It uses ES modules and top-level `await`; save it as an `.mjs` file or set `"type": "module"` in the application's `package.json`:
...
const authOptions = {
    certificate: fs.readFileSync("C:\\secrets\\quill-client.pfx"),
    type: "pfx",
    password: process.env.QUILL_CLIENT_CERTIFICATE_PASSWORD
};
...
try {
    ...
    try {
        const products = await session
            .query({ collection: "Products", documentType: Product })
            .take(10)
            .all();
...
`type` selects the certificate format.
Set it to `"pfx"` for the `.pfx` in the downloaded archive, or to `"pem"` to supply the certificate and its private key in PEM form.

The explicit `collection` option makes the query target the mapped collection exactly.
Without it, the RavenDB Client API derives the collection name from the document type—for example, `query({ documentType: Customer })` targets `Customers` by default.

The `collection` option applies only to this query.
...
```js
import { GetOngoingTaskInfoOperation } from "ravendb";

const task = await store.maintenance.send(
...
The `"CdcSink"` task type and the `OngoingTaskCdcSink` result type require the RavenDB Node.js client `7.2.7` or later.
