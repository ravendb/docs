...
* **Query limitations**:
    ...
    * Using [include](../../client-api/session/loading-entities.mdx#load-with-includes) to load a related document into the session in a streaming query is Not supported.  
...
| `StartingWithOptions` | | |
| - | - | - |
| **matches** | `number` | Filter the ID part that comes after the specified prefix.<br/>Use '?' for any character, '*' any characters.<br/>Use '&#124;' to separate rules. |
