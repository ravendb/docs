...
<Panel heading="Prerequisites and Recommendations">

* **Prerequisites**  
   * Install [`.NET` Core runtime](https://dotnet.microsoft.com/en-us/download), 
     either manually or [along with a RavenDB full version](../embedded.mdx#setting-server-directory).  
...
<Panel heading="Starting the Server">
...
<Panel heading="Embedded Server Licensing">
...
Embedded server licensing configuration options are gathered in the `ServerOptions.LicensingOptions` class.  
After acquiring a license, it can be passed to the server either as a string or as a file.  

#### Pass to the server a license string:
To pass your license to the server as a string, use the `ServerOptions.LicensingOptions.License` configuration option.  
```csharp
EmbeddedServer.Instance.StartServer(new ServerOptions
{
...
#### Pass to the server a path to a license file:
To keep your license file in your file system and point the server to its path, use the `ServerOptions.LicensingOptions.LicensePath` configuration option.  
```csharp
EmbeddedServer.Instance.StartServer(new ServerOptions
{
  Licensing = new ServerOptions.LicensingOptions
...
* To pass your license to the server as a string, define or edit the environment variable `RAVEN_License`.  
  ...
  **Note**, however, that you must first **reformat** the license JSON that you acquired and turn it into a single line, removing all new-line characters.  
  <Tabs groupId='languageSyntax'>
  ...
  Find below a Bash script that reads the license from a file and compacts it to a single line.  
  ```plain
...
<Panel heading=".NET FrameworkVersion">
...
<Panel heading="Get Server URL and Process ID">
