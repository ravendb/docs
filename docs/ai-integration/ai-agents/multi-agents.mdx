...
The entire run is bounded by the root agent's `MaxModelIterationsPerCall` setting: it caps the number of tool-call rounds the model may produce per user message, regardless of whether those tools are queries, actions, or sub-agent invocations.  
...
Multi-agent setups do not introduce a new access-control surface: the scope limits already documented for AI agents apply at every level of the hierarchy. Sub-agents live in the same database as their parent, are created under the same administrative controls, and use the same connection strings.  
