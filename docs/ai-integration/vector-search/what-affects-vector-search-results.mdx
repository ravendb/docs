...
* This article explains why vector search results might not always return what you expect, even when relevant documents exist.
  It applies to both [Dynamic vector search queries](./vector-search-using-dynamic-query.mdx) and
  [Static-index vector search queries](./vector-search-using-static-index.mdx).
...
* If full accuracy is required, RavenDB also provides [Exact vector search](#using-exact-search),  
  which performs a full scan of all indexed vectors to guarantee the closest matches.
* In this article:
  * [The approximate nature of HNSW](#the-approximate-nature-of-hnsw)
  * [Indexing-time parameters](#indexing-time-parameters)
  * [Query-time parameters](#query-time-parameters)
  * [Using exact search](#using-exact-search)
...
* **Graph structure**:  
  ...
  * Each node (vector) is connected only to a limited number of neighbors, selected as the most relevant during indexing (graph build time).
    This limitation is controlled by the [Indexing-time parameters](#indexing-time-parameters) described below.
...
* **Greedy search**:  
  ...
  * The search is influenced by the [Query-time params](#query-time-parameters) described below.  
  ...
  * While HNSW offers fast search performance at scale and quickly finds points that are likely to be among the nearest neighbors,
    ...
    If full accuracy is critical, consider using [Exact search](#using-exact-search) instead.
...
* **Number of edges**:  
  ...
  * With **static-indexes** -  
    This param can be set directly in the index definition. For example, see this [index definition](./vector-search-using-static-index.mdx#indexing-raw-text).  
...
see [Parameters defined at index definition](./vector-search-using-static-index.mdx#parameters-defined-at-index-definition).
...
* **Number of Candidates at query time**:  
  ...
  * This param can be set directly in the query. For example, see this [Query example](./vector-search-using-dynamic-query.mdx#querying-raw-text).  
...
* **Search Method**:  
  ...
  * You can choose between two vector search modes:  
    ...
    * **Exact search**:  
      ...
      Learn more in [Using exact search](#using-exact-search) below.

For all parameters that can be defined at query time, see:  
Dynamic queries - [The dynamic query parameters](./vector-search-using-dynamic-query.mdx#the-dynamic-query-parameters).  
Static index queries - [Parameters used at query time](./vector-search-using-static-index.mdx#parameters-used-at-query-time).   
...
* Exact search can be used with both static index queries and dynamic queries.  
  For example, see [Dynamic vector search - exact search](./vector-search-using-dynamic-query.mdx#dynamic-vector-search---exact-search).
