...
* In this article:
    ...
    * [Order by metadata](../../../querying/sorting-query-results/sort-query-results.mdx#todo...)
    ...
    * [Performance notes ](../../../querying/sorting-query-results/sort-query-results.mdx#todo..)
...
* When ordering query results, you can specify where documents with `null` values should appear in the sorted results.
  ...
  * method expressions such as `id()` or `spatial.distance(...)`. See... []().
...
<Panel heading="Order by a metadata">

* Documents carry server-maintained metadata such as `@last-modified` and `@collection`.  
  You can sort by these fields directly in RQL, and via `DocumentQuery` in C#.

* Note: ordering by metadata uses **string** ordering by default. 
  For `@last-modified` the timestamp format is lexically sortable (`yyyy-MM-ddTHH:mm:ss.fffffffZ`), so default string ordering produces the correct chronological result.
...
* **Filter before you sort.** Sorting cost is proportional to the number of
  matched documents, not the size of the collection. A `Where` clause that
  reduces the candidate set materially cheapens the sort.

* **Sorting requires an indexed field.** Ordering by an arbitrary field on a
  dynamic query forces the auto-index to add that field, which may trigger an
  index update / extension. On hot paths, prefer a static index that already
  contains the sortable field.

* **`OrderingType` matters.** Using the wrong type works (`as string` on a
  numeric field will still return rows) but produces lexicographic order —
  `"10"` comes before `"9"`. Always pass `OrderingType.Long` /
  `OrderingType.Double` for numeric fields.
