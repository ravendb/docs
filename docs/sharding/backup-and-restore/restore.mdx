...
* In this page:  
  * [Restore](#restore)  
     * [Set paths to backup files locations](#set-paths-to-backup-files-locations)  
     * [Define a restore configuration](#define-a-restore-configuration)  
     ...
     * [Examples](#examples)  
  * [Restoring to a selected restore point](#restoring-to-a-selected-restore-point)  
  * [Restore options summary](#restore-options-summary)  
...
* We choose what configuration object to pass the restore operation, by 
  ...
  or remotely (in a cloud location).  
    ...
    | -------------------- | --------------- | --------------------- |
    | `RestoreBackupConfiguration` | Local shard storage | None (see [example](#examples)) |
    | `RestoreFromS3Configuration` | AWS S3 Bucket | `S3Settings` (see S3 [example](#examples)) |
    | `RestoreFromAzureConfiguration` | MS Azure Blob | `AzureSettings` (see Azure [example](#examples)) |
    | `RestoreFromGoogleCloudConfiguration` | Google Cloud Bucket | `GoogleCloudSettings` (see Google Cloud [example](#examples)) |
...
* Instead of `RestoreBackupConfigurationBase`, use the configuration object 
  you [prepared](#define-a-restore-configuration):  
...
| Restore from a [remote location](../../sharding/backup-and-restore/backup.mdx#backup-storage-local-and-remote) | **Yes** | Define a [restore configuration](#define-a-restore-configuration) with S3, Azure, or Google Cloud settings. |
