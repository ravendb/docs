...
## Conflict Detection
Each document in a RavenDB cluster has a [Change Vector](./change-vector.mdx) which is used for conflict detection.

When the server receives an incoming replication batch, it compares the [Change Vector](./change-vector.mdx) 
of the incoming document with the [Change Vector](./change-vector.mdx) of the local document. 
...
The [Comparison](./change-vector.mdx#change-vector-comparisons) of the [Change Vectors](./change-vector.mdx) may yield three possible results:  
