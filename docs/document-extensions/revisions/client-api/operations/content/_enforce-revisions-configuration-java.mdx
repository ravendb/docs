...
* On large datasets, use `setMaxOpsPerSecond(...)` to [throttle the operation](#throttle-the-operation)
...
By default, [force-created revisions](../../../overview.mdx#force-revision-creation)
...
| Parameter               | Type       | Description |
|-------------------------|------------|-------------|
| **includeForceCreated** | `boolean`  | `true` - [Force-created revisions](../../../overview.mdx#force-revision-creation)may also be removed by the operation.<br/>`false` (default) - Force-created revisions are not included in deletion paths that explicitly protect them. |
